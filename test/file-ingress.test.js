...
import { checkClamavRuntime, createEndpointIngressApi, hasExecutableJavaScriptSemantics, isCurrentClamavSignature, isJavaScriptParserInputWithinBounds, isJavaScriptRawInputWithinBounds, isSupportedInspectionNodeVersion, multipartParts, shutdownClamavRuntime, stageMultipartIngress, sweepExpiredFileIngress, validatePdfIngress } from "../dist/file-ingress-runtime.js";
...
test("the flat parser budget bounds every recursive Acorn expression grammar at one threshold", () => {
  ...
  assert.equal(hasExecutableJavaScriptSemantics("`" + "await new => ? ([{ ".repeat(1000) + "`"), false);
  assert.equal(hasExecutableJavaScriptSemantics("/await new => \\? \\( \\[ \\{/"), false);
  assert.equal(hasExecutableJavaScriptSemantics("/* " + "await new => ? ([{ ".repeat(1000) + " */ supportNote"), false);
});

test("raw JavaScript structure is bounded before Acorn tokenizes regular expressions", () => {
  const nestedRegex = (depth) => "/" + "(".repeat(depth) + "evidence" + ")".repeat(depth) + "/";
  assert.equal(isJavaScriptRawInputWithinBounds(nestedRegex(256)), true);
  assert.equal(isJavaScriptRawInputWithinBounds(nestedRegex(257)), false);
  assert.equal(hasExecutableJavaScriptSemantics(nestedRegex(256)), false);
  assert.equal(hasExecutableJavaScriptSemantics(nestedRegex(257)), true);
  assert.equal(isJavaScriptRawInputWithinBounds("/" + "\\(".repeat(1000) + "/"), true);
  assert.equal(isJavaScriptRawInputWithinBounds("/[" + "(".repeat(1000) + "]/"), true);
  assert.equal(isJavaScriptRawInputWithinBounds("/* /" + "(".repeat(1000) + ")".repeat(1000) + "/ */ evidence"), true);
  assert.equal(isJavaScriptRawInputWithinBounds("// /" + "(".repeat(1000) + ")".repeat(1000) + "/\nevidence"), true);
  assert.equal(isJavaScriptRawInputWithinBounds("'/" + "(".repeat(1000) + ")".repeat(1000) + "/'"), true);
  assert.equal(isJavaScriptRawInputWithinBounds('"/' + "(".repeat(1000) + ")".repeat(1000) + '/"'), true);
  assert.equal(isJavaScriptRawInputWithinBounds("`/" + "(".repeat(1000) + ")".repeat(1000) + "/`"), true);
  assert.equal(isJavaScriptRawInputWithinBounds("`evidence ${" + nestedRegex(257) + "}`"), false);
  assert.equal(isJavaScriptRawInputWithinBounds("`evidence ${" + "(".repeat(257) + "value" + ")".repeat(257) + "}`"), false);
