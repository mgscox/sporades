...
function xrefStreamPdf(compressed = false) { const objects = ["1 0 obj\n<</Type /Catalog /Pages 2 0 R>>\nendobj\n", "2 0 obj\n<</Type /Pages /Kids [3 0 R] /Count 1>>\nendobj\n", "3 0 obj\n<</Type /Page /Parent 2 0 R /MediaBox [0 0 1 1] /Contents 4 0 R>>\nendobj\n", "4 0 obj\n<</Length 0>>\nstream\n\nendstream\nendobj\n"]; let body = Buffer.from("%PDF-1.5\n"); const offsets = [0]; for (const object of objects) { offsets.push(body.length); body = Buffer.concat([body, Buffer.from(object)]); } const xrefOffset = body.length; offsets.push(xrefOffset); const entries = Buffer.alloc(6 * 7); entries[5] = 0xff; entries[6] = 0xff; for (let index = 1; index < 6; index += 1) { const at = index * 7; entries[at] = 1; entries.writeUInt32BE(offsets[index], at + 1); } const stream = compressed ? deflateSync(entries) : entries; return Buffer.concat([body, Buffer.from(`5 0 obj\n<</Type /XRef /Size 6 /Root 1 0 R /W [1 4 2]${compressed ? " /Filter /FlateDecode" : ""} /Length ${stream.length}>>\nstream\n`), stream, Buffer.from(`\nendstream\nendobj\nstartxref\n${xrefOffset}\n%%EOF\n`)]); }
function hybridXrefPdf() { const streamPdf = xrefStreamPdf(); const streamOffset = Number(/startxref\n(\d+)\n%%EOF/.exec(streamPdf.toString("latin1"))[1]); const streamEnd = streamPdf.indexOf(Buffer.from("\nstartxref\n"), streamOffset); const body = streamPdf.subarray(0, streamEnd); const offsets = [...body.toString("latin1").matchAll(/(\d+) 0 obj/g)].map((match) => match.index); const xrefOffset = body.length + 1; return Buffer.concat([body, Buffer.from(`\nxref\n0 6\n0000000000 65535 f \n${offsets.map((value) => `${String(value).padStart(10, "0")} 00000 n \n`).join("")}trailer\n<</Size 6 /Root 1 0 R /XRefStm ${streamOffset}>>\nstartxref\n${xrefOffset}\n%%EOF\n`)]); }
