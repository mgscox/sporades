...
test("sentence-shaped shell classification checks only regular executable filesystem entries", async () => {
  ...
  try {
    ...
    for (const command of ["Payload/run argument.", "./Payload/run argument.", `${executable} argument.`, "Payload/nested/run --flag value.", '"Payload/run" argument.', "'Payload/run' argument.", "Payload\\/run argument.", '"Payload/literal*" argument.', "Payload/literal\\* argument.", "Payload/linked argument.", "Payload/* argument.", "Payload/r?n argument.", "Payload/[r]un argument.", "Payload/{run,nope} argument.", "~/Payload/r*n argument.", "Runner argument.", "LinkedRunner argument."]) assert.equal(hasExecutableShellSemantics(command), true, command);
    for (const prose of ["Payload/evidence remains available.", "Payload/directory remains available.", "Payload/broken remains available.", "Payload/missing* remains unavailable.", "Payload/n?pe remains unavailable.", "Payload/[x]un remains unavailable.", "Payload/[z-a] remains documented.", "Payload/{nope,missing} remains unavailable.", '"Payload/*" remains documented.', "Payload/\\* remains documented.", "~/sporades-file-ingress-no-such-command-* remains unavailable.", "Evidence argument.", "Support request remains active.", "BrokenRunner argument.", "A * marker remains visible.", "A {draft,final} label remains visible."]) assert.equal(hasExecutableShellSemantics(prose), false, prose);
