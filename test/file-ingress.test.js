...
import { checkClamavRuntime, createEndpointIngressApi, hasExecutableJavaScriptSemantics, isCurrentClamavSignature, isJavaScriptParserInputWithinBounds, isSupportedInspectionNodeVersion, multipartParts, shutdownClamavRuntime, stageMultipartIngress, sweepExpiredFileIngress, validatePdfIngress } from "../dist/file-ingress-runtime.js";
...
test("content-policy-v1 structurally validates its allowlist and rejects executable or ambiguous evidence", async () => {
  ...
  try {
    ...
    const cases = [
      ...
      ["deep-parenthesized-prose-control", "note.txt", "text/plain", Buffer.from("(".repeat(255) + "supportNote" + ")".repeat(255)), "clean"], ["deep-parenthesized-call", "note.txt", "text/plain", Buffer.from("(".repeat(1000) + "alert(1)" + ")".repeat(1000)), "rejected"], ["deep-array-call", "note.txt", "text/plain", Buffer.from("[".repeat(1000) + "alert(1)" + "]".repeat(1000)), "rejected"], ["deep-unary-call", "note.txt", "text/plain", Buffer.from("!".repeat(4000) + "alert(1)"), "rejected"], ["deep-await-call", "note.txt", "text/plain", Buffer.from("await ".repeat(1000) + "alert(1)"), "rejected"], ["deep-new-call", "note.txt", "text/plain", Buffer.from("new ".repeat(1000) + "alert(1)"), "rejected"], ["deep-arrow-call", "note.txt", "text/plain", Buffer.from("a=>".repeat(1000) + "alert(1)"), "rejected"], ["deep-conditional-call", "note.txt", "text/plain", Buffer.from("a?".repeat(1000) + "alert(1)" + ":a".repeat(1000)), "rejected"],
