...
test("PDF action entries are classified from their owning dictionary without rejecting tagged layout attributes", async () => {
  ...
  const annotationAction = structuredPdf("", [
    "5 0 obj\n<</Type /Annot /Subtype /Link /Rect [0 0 1 1] /A 6 0 R>>\nendobj\n",
    "6 0 obj\n<</D [3 0 R /Fit]>>\nendobj\n",
  ], "/Annots [5 0 R]");
  const outlineAction = structuredPdf("/Outlines 5 0 R", [
    "5 0 obj\n<</Type /Outlines /First 6 0 R /Last 6 0 R /Count 1>>\nendobj\n",
    "6 0 obj\n<</Title (Unsafe) /Parent 5 0 R /A <</D [3 0 R /Fit]>>>>\nendobj\n",
  ]);

  assert.equal(await validatePdfIngress(taggedLayout), true);
  assert.equal(await validatePdfIngress(annotationAction), false);
  assert.equal(await validatePdfIngress(outlineAction), false);
});
