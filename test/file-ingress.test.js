...
function predictedXrefStreamPdf({ predictor = 12, filterArray = false, parmsArray = false, columns = 7, colors = 1, bits = 8, bitsKey = "BitsPerComponent", filterByte, params = "", mutate } = {}) {
  ...
  const encoded = []; let previous = Buffer.alloc(7);
  for (let row = 0; row < 6; row += 1) { const current = entries.subarray(row * 7, row * 7 + 7); if (predictor === 1) encoded.push(current); else if (predictor === 2) { const raw = Buffer.alloc(7); raw[0] = current[0]; for (let at = 1; at < 7; at += 1) raw[at] = current[at] - current[at - 1]; encoded.push(raw); } else { const kind = filterByte ?? (predictor === 15 ? row % 5 : predictor - 10); const raw = Buffer.alloc(7); for (let at = 0; at < 7; at += 1) { const left = at ? current[at - 1] : 0; const up = previous[at]; const upLeft = at ? previous[at - 1] : 0; const p = left + up - upLeft; const pa = Math.abs(p - left); const pb = Math.abs(p - up); const pc = Math.abs(p - upLeft); const predicted = kind === 0 ? 0 : kind === 1 ? left : kind === 2 ? up : kind === 3 ? Math.floor((left + up) / 2) : pa <= pb && pa <= pc ? left : pb <= pc ? up : upLeft; raw[at] = current[at] - predicted; } encoded.push(Buffer.from([kind]), raw); } previous = current; }
...
test("PDF xref streams decode bounded TIFF and PNG prediction parameters", async () => {
  ...
  for (const [name, pdf] of [
    ...
    ["wrong row width", predictedXrefStreamPdf({ predictor: 12, columns: 6 })],
