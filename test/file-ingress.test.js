...
function xrefStreamPdfWithCompressedObject(options = {}) { const tableObject = options.tableObject ?? 6; const objectPayload = `${tableObject} 0 <</Producer (compressed evidence)>>`; const decodedObjectStream = Buffer.from(objectPayload); const objectStreamBytes = options.flate ? deflateSync(decodedObjectStream) : decodedObjectStream; const objectStreamDictionary = options.ordinaryStream ? `/Length ${objectStreamBytes.length}` : `/Type /ObjStm /N ${options.count ?? 1} /First ${options.first ?? String(tableObject).length + 3}${options.flate ? " /Filter /FlateDecode" : ""} /Length ${objectStreamBytes.length}`; const objects = ["1 0 obj\n<</Type /Catalog /Pages 2 0 R>>\nendobj\n", "2 0 obj\n<</Type /Pages /Kids [3 0 R] /Count 1>>\nendobj\n", "3 0 obj\n<</Type /Page /Parent 2 0 R /MediaBox [0 0 1 1] /Contents 4 0 R>>\nendobj\n", "4 0 obj\n<</Length 0>>\nstream\n\nendstream\nendobj\n"]; let body = Buffer.from("%PDF-1.5\n"); const offsets = [0]; for (const object of objects) { offsets.push(body.length); body = Buffer.concat([body, Buffer.from(object)]); } offsets.push(body.length); body = Buffer.concat([body, Buffer.from(`5 0 obj\n<<${objectStreamDictionary}>>\nstream\n`), objectStreamBytes, Buffer.from("\nendstream\nendobj\n")]); const xrefOffset = body.length; offsets.push(0, xrefOffset); const entries = Buffer.alloc(8 * 7); entries.writeUInt32BE(options.freeHead ?? 0, 1); entries[5] = 0xff; entries[6] = 0xff; for (let index = 1; index <= 5; index += 1) { const at = index * 7; entries[at] = 1; entries.writeUInt32BE(offsets[index], at + 1); } const compressedAt = 6 * 7; entries[compressedAt] = 2; entries.writeUInt32BE(options.container ?? 5, compressedAt + 1); entries.writeUInt16BE(options.index ?? 0, compressedAt + 5); const selfAt = 7 * 7; entries[selfAt] = 1; entries.writeUInt32BE(xrefOffset, selfAt + 1); return Buffer.concat([body, Buffer.from(`7 0 obj\n<</Type /XRef /Size 8 /Root 1 0 R /W [1 4 2] /Length ${entries.length}>>\nstream\n`), entries, Buffer.from(`\nendstream\nendobj\nstartxref\n${xrefOffset}\n%%EOF\n`)]); }
