...
test("sentence-shaped shell classification checks only regular executable filesystem entries", async () => {
  ...
  try {
    ...
    for (const name of ["literal*", "run1", "run01", "runa", "1un"]) { await writeFile(path.join(dir, "Payload", name), "#!/bin/sh\nexit 0\n"); await chmod(path.join(dir, "Payload", name), 0o755); }
    ...
    for (const command of ["Payload/run argument.", "./Payload/run argument.", `${executable} argument.`, "Payload/nested/run --flag value.", '"Payload/run" argument.', "'Payload/run' argument.", "Payload\\/run argument.", '"Payload/literal*" argument.', "Payload/literal\\* argument.", "Payload/linked argument.", "Payload/* argument.", "Payload/r?n argument.", "Payload/[r]un argument.", "Payload/{run,nope} argument.", "Payload/run{1..2} argument.", "Payload/run{01..02} argument.", "Payload/run{a..b} argument.", "Payload/run{2..1} argument.", "Payload/run{b..a} argument.", "Payload/run{5..1..2} argument.", "Payload/run{5..1..-2} argument.", "Payload/run{1..5..2} argument.", "Payload/run{01..05..2} argument.", "Payload/run{a..e..2} argument.", "Payload/run{1..1000} argument.", "Payload/[[:alpha:]]una argument.", "Payload/[[:digit:]]un argument.", "Payload/[[:alnum:]]una argument.", "Payload/[![:digit:]]una argument.", "Payload/[[:digit:]r]una argument.", "~/Payload/r*n argument.", "~/Payload/run{1..2} argument.", "Runner argument.", "LinkedRunner argument."]) assert.equal(hasExecutableShellSemantics(command), true, command);
    for (const prose of ["Payload/evidence remains available.", "Payload/directory remains available.", "Payload/broken remains available.", "Payload/missing* remains unavailable.", "Payload/n?pe remains unavailable.", "Payload/[x]un remains unavailable.", "Payload/[z-a] remains documented.", "Payload/[[:digit:]]una remains unavailable.", "Payload/{nope,missing} remains unavailable.", "Payload/run{1..3..0} remains documented.", "Payload/run{1..x} remains documented.", "Payload/run{1...2} remains documented.", '"Payload/run{1..2}" remains documented.', "Payload/run\\{1..2\\} remains documented.", '"Payload/[[:alpha:]]una" remains documented.', '"Payload/*" remains documented.', "Payload/\\* remains documented.", "~/sporades-file-ingress-no-such-command-* remains unavailable.", "Evidence argument.", "Support request remains active.", "BrokenRunner argument.", "A * marker remains visible.", "A {draft,final} label remains visible.", "A {1..2} range remains visible.", "A [[:alpha:]] class remains visible."]) assert.equal(hasExecutableShellSemantics(prose), false, prose);
...
test("content-policy-v1 structurally validates its allowlist and rejects executable or ambiguous evidence", async () => {
  const dir = await mkdtemp(path.join(tmpdir(), "sporades-ingress-content-matrix-")); const originalHome = process.env.HOME; let database;
  try {
    ...
    for (const name of ["run", "run1", "run01", "runa", "1un"]) { const candidate = path.join(path.dirname(sentenceExecutable), name); await writeFile(candidate, "#!/bin/sh\nexit 0\n"); await chmod(candidate, 0o755); }
    ...
    const cases = [
      ...
      ["valid-shell-unmatched-posix-class", "note.txt", "text/plain", Buffer.from(`${path.dirname(sentenceExecutable)}/[[:digit:]]una remains unavailable.`), "clean"],
  ...
  } finally { process.env.HOME = originalHome; await database?.close(); await rm(dir, { recursive: true, force: true }); }
});
