...
test("PDF inspection requires the final cross-reference EOF boundary to be terminal", async () => {
  ...
  const additionalFreeEntry = Buffer.from(classic.replace("xref\n0 5", "xref\n0 6").replace("trailer\n", "0000000000 00123 f \ntrailer\n").replace("/Size 5", "/Size 6"), "latin1");
  assert.equal(await validatePdfIngress(additionalFreeEntry), true);
