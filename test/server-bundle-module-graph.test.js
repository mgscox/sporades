...
test("a generated server bundle lazily lists the current linked user's singleton Team", async () => {
  ...
  try {
    ...
    try {
      ...
      assert.equal(members.data.nextCursor, undefined);
...
const RUNTIME_SOURCE_CONSTANTS = [
  ...
  "CAPSULE_INGRESS_AUTH_USER_PREFIX",
  "REGISTRATION_ADMISSION_BYTE_LIMIT",
  ...
  "STRIPE_EVENT_PAYLOAD_CLEANUP_BATCH_SIZE",
...
test("every runtime constant reaches a booted Capsule with the value and the type its module declares", async () => {
  ...
  try {
    ...
    assert.equal(bundled.STRIPE_EVENT_PAYLOAD_RETENTION_MS.type, "number");
    assert.equal(bundled.STRIPE_EVENT_PAYLOAD_CLEANUP_BATCH_SIZE.type, "number");
