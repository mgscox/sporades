...
test("privileged Job cancellation propagates its signal", async () => {
 ...
 const db=await openDevDatabase(path.join(dir,"data.db"),"",{},{name:"jobs"},{jobs:{wait:job(async(ctx)=>{started(); await new Promise(r=>release=r); const e=new Error("abort");e.name="AbortError";throw e;})},mutations:{enqueue:mutation((ctx)=>ctx.privileged.run({operation:"jobs.enqueue",targetResourceKind:"job-queue"},p=>p.jobs.enqueue("wait",{}))),cancel:mutation((ctx,id)=>ctx.privileged.run({operation:"jobs.cancel",targetResourceKind:"job-queue"},p=>p.jobs.cancel(id))),get:mutation((ctx,id)=>ctx.privileged.run({operation:"jobs.get",targetResourceKind:"job-queue"},p=>p.jobs.get(id)))}});
