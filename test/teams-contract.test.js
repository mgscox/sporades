...
test("the published Teams contract defines the public boundaries and is reachable from the reference index", async () => {
  const [teams, reference, config, readme, clientTypes, serverTypes, clientApi, serverApi, aclTeamHelpersApi, sourceRuntime, generatedRuntime, generatedBin] = await Promise.all([
    ...
    readFile(new URL("../src/server-runtime-source.ts", import.meta.url), "utf8"),
    readFile(new URL("../dist/server-runtime-source.js", import.meta.url), "utf8"),
    readFile(new URL("../bin/sporades.js", import.meta.url), "utf8"),
  ]);
  ...
  for (const statement of [
    ...
    /bypass the joining user's ordinary row ACLs/i,
    /Concurrent joins for a final seat[\s\S]*serialize/i,
  ]) assert.match(teams, statement);
  assert.match(readme, /teams\.admitJoin[\s\S]*joining user's ordinary row ACLs/);
  ...
  assert.match(serverTypes, /bypass the joining user's ordinary row ACLs[\s\S]*Runtime tables and mutations are unavailable/);
  ...
  for (const runtime of [sourceRuntime, generatedRuntime, generatedBin]) {
    assert.match(runtime, /runTeamJoinAdmission/);
    assert.match(runtime, /teams\.join-admission/);
    assert.match(runtime, /withTrustedRead/);
  }
});
