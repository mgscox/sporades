...
test("sporades jobs uses active Dev adapter environment without an inspection token", async () => {
  const dir = await mkdtemp(path.join(tmpdir(), "sporades-jobs-dev-"));
  try {
    await mkdir(path.join(dir, ".sporades", "build"), { recursive: true });
    await writeFile(path.join(dir, ".sporades", "dev-session.json"), JSON.stringify({ pid: process.pid, inspectionToken: "must-not-pass", serviceEnv: { SPORADES_SERVICE_DATABASE_ENGINE: "libsql", SPORADES_SERVICE_DATABASE_URL: "http://adapter.test" } }));
    await writeFile(path.join(dir, ".sporades", "build", "server.mjs"), `if(process.env.SPORADES_SERVICE_DATABASE_ENGINE!=="libsql"||process.env.SPORADES_SERVICE_DATABASE_URL!=="http://adapter.test"||process.env.SPORADES_INSPECTION_TOKEN)process.exit(8);console.log(${JSON.stringify(JSON.stringify(envelope))})`);
