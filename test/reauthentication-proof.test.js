...
test("runtime-owned proof consumption commits atomically, preserves rejected proof, and has one concurrent winner", async () => {
  ...
  try {
    ...
    await database.close(); database = await openDevDatabase(file, "", {}, { name: definition.name }, definition); assert.equal(database.adapter.prepare("SELECT id FROM sporades_auth_reauthentication_proofs WHERE id = ?").get("proof-expired"), undefined, "restart sweeps expired proofs after the rejected mutation rolls back");
