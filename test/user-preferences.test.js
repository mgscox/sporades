...
test("User journey lifecycle is declaration-gated and bound to the enabling identity over the real transport", async () => {
  await withTempDir(async (dir) => {
    ...
    try {
      ...
      assert.equal(enabled.data.userId, auth.data.auth.userId); assert.equal(enabled.data.enabled, true); assert.equal(enabled.data.sessionId, undefined);
      ...
      assert.equal(linkedEnable.data.sessionId, undefined);
      ...
      assert.equal(signedInEnable.data.sessionId, undefined); assert.equal(anonymousEnable.data.sessionId, undefined);
      ...
      assert.equal(tokenReplacedEnable.data.sessionId, undefined);
...
test("Journey subscribers receive a Capsule-wide snapshot before complete realtime changes", async () => {
  await withTempDir(async (dir) => {
    ...
    try {
      ...
      assert.deepEqual(first.data.journey.metadata, { safe: true }, "explicit JSON-safe metadata remains unchanged");
      ...
      assert.equal(sameUserEnabled.data.sessionId, undefined); assert.equal(enabled.data.sessionId, undefined);
      ...
      assert.equal(second.data.journey.metadata, null, "set results must keep the complete Journey record shape when metadata is absent");
      const listed = await sendAndWait(sameUserPublisher, { id: "list", type: "journey.list" });
      assert.equal(listed.data.journeys.find((state) => state.sessionId === second.data.journey.sessionId).metadata, null, "list results must keep the complete Journey record shape when metadata is absent");
      ...
      assert.equal(events[0].states.find((state) => state.sessionId === second.data.journey.sessionId).metadata, null, "subscription snapshots must keep the complete Journey record shape when metadata is absent");
      ...
      assert.deepEqual(Object.keys(events[2].state).sort(), ["expiresAt", "metadata", "sessionId", "status", "updatedAt", "userId"]);
      assert.equal(events[2].state.metadata, null, "realtime changes must keep the complete Journey record shape when metadata is absent");
