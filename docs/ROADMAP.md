...
| Built-in Teams foundation | implemented | Runtime-owned Team storage gives every linked user one singleton Team and initial admin membership. Browser `teams` and trusted `ctx.teams` support explicit Team creation, rename, admin-scoped membership listing, email-bound Join links, joining, and the Team-admin lifecycle. Capsules declare bounded `teams.appRoles`, and exact-Team admins atomically reconcile membership-scoped assignments; inactive declarations fail closed while retained rows support rollback. Lifecycle writes re-check current authority in their transaction, emit bounded redacted audits, and do not create a current-Team state or automatic Capsule data authorization. Planning remains in `.scratch/built-in-teams/PRD.md`; Tickets 01–09 record these delivered slices. |
...
| Team ACL | candidate | Runtime-owned Team creation, membership listing, Join links, lifecycle management, and membership application roles are implemented through Ticket 09. Team ACL remains intentionally deferred to Ticket 10 (`.scratch/built-in-teams/issues/10-authorize-capsule-resources-through-team-acl.md`). |
