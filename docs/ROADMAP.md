...
| Job queue | design | Gives Capsule authors durable background work with retry behavior, failure visibility, and a SQLite-first default that can later grow through adapters. It should shape the queue and Job concepts before Job scheduling commits to whether recurring work builds on the queue or stays timer-only, and it depends on the implemented Privileged server role actor boundary for userless system-owned Jobs. | Needs PRD. Use `.scratch/post-v2-platform-hardening-and-ops/issues/08-add-job-queue.md`, the Ops And Automation roadmap notes, and the implemented Privileged server role boundary as source planning artifacts. |
| Job scheduling | design | Gives Capsule authors a first-class way to run recurring work with persistence, missed-run behavior, timezone handling, and duplicate-run protection. It can support platform needs such as automated backups, and it depends on the implemented Privileged server role actor boundary because recurring Jobs need an explicit execution actor when no live user session exists. | Needs PRD after Job Queue and recurring actor semantics are shaped. Use `.scratch/post-v2-platform-hardening-and-ops/issues/09-add-job-scheduling.md`, the Ops And Automation roadmap notes, and the implemented Privileged server role boundary as source planning artifacts. |
