...
| --- | --- | --- |
| User-owned scoped Access keys | implemented | Linked users can create immutable named credentials for programmatic access without synthetic Bot accounts. Capsules declare a central scope vocabulary and explicitly opt Custom endpoints or private File reads into Bearer admission; the real owner remains the authorization actor while `ctx.credential` preserves named-key attribution through logs and durable Jobs. Session-only owner management provides one-time issuance and rotation, while Privileged and CLI surfaces inspect or retire bounded metadata through a running Capsule without receiving plaintext. Planning and completion evidence remain in `.scratch/scoped-access-keys-implementation/`. |
...
| Job queue | implemented | Capsule server code declares durable server-only Jobs with `job()` and uses the runtime-owned queue for current-user and Privileged server role actors. Enqueue persists atomically with mutation, App message, and Custom endpoint app writes; dispatch begins after commit, including a guaranteed follow-up scan when a commit races an active worker's empty read. The queue provides one-time delayed availability, bounded retry, cancellation, lease recovery, at-least-once delivery, actor-scoped app inspection, bounded JSON-only administrator inspection, and orderly shutdown that drains HTTP work and settles scheduled Job work before storage closes. Implemented Job scheduling builds on this queue and leaves execution, retries, cancellation, leases, and results under its contract. Planning remains in `.scratch/job-queue/PRD.md` for traceability. |
...
| Capsule table uniqueness | implemented | Capsule tables declare quoted single-field and composite constraints with `.unique(...)` across SQLite, libSQL, and PostgreSQL. New tables enforce them immediately; existing tables add them through atomic rollback-safe migration; `insertOrIgnore` ignores only the exactly named declared constraint. Non-additive constraint changes remain unsupported, and duplicate migration details stay opaque without hiding unrelated database errors. |
...
| Built-in Teams foundation | implemented | Runtime-owned Team storage gives every linked user one singleton Team and initial admin membership. Browser `teams` and trusted `ctx.teams` support explicit Team creation, rename, count-only exact accepted-membership totals for current members, admin-scoped membership listing, email-bound Join links, joining, and the Team-admin lifecycle. Active Privileged callbacks have a separate audited, read-only exact-Team inspection projection without user scope, mutations, target emails, or recoverable capabilities. Capsules declare bounded `teams.appRoles`, and exact-Team admins atomically reconcile membership-scoped assignments; inactive declarations fail closed while retained rows support rollback. ACL rules can make constrained explicit-Team membership, admin, and declared-role decisions through `ctx.acl.teams`; no current-Team state or automatic Capsule data authorization exists. Planning remains in `.scratch/built-in-teams/PRD.md`; Tickets 01–10 record these delivered slices. |
