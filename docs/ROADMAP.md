...
| --- | --- | --- | --- |
| Multi-framework client toolchains | ready | Expands Capsule client support beyond React and Preact while preserving one Sporades-owned Bundle pipeline, deterministic Dev/Container/Hosted behavior, and framework-native authoring. The intended initial set is Vanilla TypeScript, React, Preact, Vue, Svelte, SolidJS, Lit, and Inferno. | Ready for implementation from `.scratch/multi-framework-client-toolchains/PRD.md` and its dependency-ordered issues. ADR 0032 supersedes ADR 0010 by preserving author-owned source HTML while replacing the fixed `/client.js` release assumption with a normalized public asset tree. The plan adds a client-toolchain adapter seam, an esbuild/Vite capability matrix, framework-neutral query/mutation/auth primitives, native hooks/composables/stores/signals/controllers, scaffold and dependency generation, structured build events, and Sporades-owned Dev refresh. Angular and server-owning meta-frameworks remain out of scope because they would replace rather than fit the Sporades project and runtime model. |
