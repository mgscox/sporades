...
| Privileged server role | design | Gives Capsule server code a server-only authority for trusted operations that intentionally run outside normal user rights, such as privileged runtime inspection, system-owned background work, or scheduled Jobs without a live user session. It must be auditable through the implemented Privileged audit event contract, must not appear in browser credentials or user/team membership, and must distinguish privileged execution from running as a captured user. | Needs PRD. Use the implemented Privileged audit event contract, the Data And Auth Helpers roadmap note, ADR 0022, Firebase/Appwrite-style server-admin precedent, and Job Queue / Job scheduling planning as source artifacts. |
