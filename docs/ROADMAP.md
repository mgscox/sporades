...
| Built-in Teams foundation | implemented | Runtime-owned Team storage now lazily gives every linked user one singleton Team and its initial admin membership. The caller can list only their safe, bounded Team presentations through public `teams.list()` or trusted-server `ctx.teams.list()`; the foundation has no enablement flag, current-Team state, or automatic Capsule data authorization. Planning remains in `.scratch/built-in-teams/PRD.md`; Ticket 01 records this delivered slice. |
...
| Team administration, Join links, application roles, and Team ACL | candidate | The runtime-owned singleton/current-user listing foundation is implemented. Additional Team creation and rename, member enumeration, and admin lifecycle remain in Tickets 03 (`.scratch/built-in-teams/issues/03-create-and-rename-additional-teams.md`), 04 (`.scratch/built-in-teams/issues/04-list-team-memberships-within-admin-scope.md`), and 08 (`.scratch/built-in-teams/issues/08-manage-the-team-admin-lifecycle.md`); email-bound Join links remain in Tickets 05 (`.scratch/built-in-teams/issues/05-create-and-manage-email-bound-join-links.md`), 06 (`.scratch/built-in-teams/issues/06-validate-join-links-after-authentication.md`), and 07 (`.scratch/built-in-teams/issues/07-join-a-team-through-a-validated-link.md`); membership application roles and Team ACL remain in Tickets 09 (`.scratch/built-in-teams/issues/09-declare-and-assign-membership-application-roles.md`) and 10 (`.scratch/built-in-teams/issues/10-authorize-capsule-resources-through-team-acl.md`). |
