...
| Email password management | implemented | The client SDK exposes `auth.setPassword(email, newPassword)` and the server context exposes `ctx.serverAuth.setEmailPassword(email, newPassword)` for updating existing email credentials. The runtime handles scrypt hashing with a fresh random salt and updates the internal `sporades_auth_email_credentials` table. This enables password reset flows in Capsule apps without direct access to the internal auth database. |
