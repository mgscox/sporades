...
| Built-in Teams foundation | implemented | Runtime-owned Team storage gives every linked user one singleton Team and initial admin membership. Browser `teams` and trusted `ctx.teams` now support explicit Team creation, rename, admin-scoped membership listing, email-bound Join links, joining, and the Team-admin lifecycle: promote, safely demote, remove another member, ordinary-member leave, and sole-member deletion. Lifecycle writes re-check current authority and the surviving-admin invariant in their transaction, emit bounded redacted audits, and do not create a current-Team state or automatic Capsule data authorization. Planning remains in `.scratch/built-in-teams/PRD.md`; Tickets 01–08 record these delivered slices. |
...
| Team application roles and Team ACL | candidate | Runtime-owned Team creation, membership listing, Join links, and the Team-admin lifecycle are implemented through Ticket 08. Membership application roles and Team ACL remain intentionally deferred to Tickets 09 (`.scratch/built-in-teams/issues/09-declare-and-assign-membership-application-roles.md`) and 10 (`.scratch/built-in-teams/issues/10-authorize-capsule-resources-through-team-acl.md`). |
