...
| --- | --- | --- | --- |
| User journey tracker | ready | Gives Capsule authors an explicitly declared way to understand what enabled Sporades users are doing across one or more live client sessions by converting safe navigation/focus signals, annotated semantic interactions, and manual updates into short-lived Journey state. | PRD ready at `.scratch/user-journey-tracker/PRD.md`; split it into implementation issues before beginning delivery. |
...
| User journey tracker | ready | Add a runtime-owned tracker that the Capsule must explicitly declare before `sporades/client` can read, subscribe, or publish. Within an enabled Capsule, each publishing client explicitly enables a Journey session with a unique session ID attached to its authenticated Sporades user ID. The session's latest bounded status and optional JSON metadata form Journey state buffered until expiry; expiry makes that session inactive without ending it, so it can publish again under the same session ID. One user may have multiple simultaneous Journey sessions, and the user's inactive state is derived when none has live state. All connected Capsule clients receive live updates in V1; future Team-based delivery filtering may restrict delivery to users who share an enabled Team. Planning is ready in `.scratch/user-journey-tracker/PRD.md`. |
