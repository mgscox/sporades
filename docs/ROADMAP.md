...
| Job queue | implemented | Capsule server code declares durable server-only Jobs with `job()` and uses the runtime-owned queue for current-user and Privileged server role actors. Enqueue persists atomically with mutation, App message, and Custom endpoint app writes; dispatch begins after commit. The queue provides one-time delayed availability, bounded retry, cancellation, lease recovery, at-least-once delivery, actor-scoped app inspection, bounded JSON-only administrator inspection, and orderly shutdown that settles scheduled Job work before storage closes. Implemented Job scheduling builds on this queue and leaves execution, retries, cancellation, leases, and results under its contract. Planning remains in `.scratch/job-queue/PRD.md` for traceability. |
