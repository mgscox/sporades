...
| --- | --- | --- |
| Mail sending | implemented | Capsule server code sends provider-independent mail through `ctx.mail.send(...)` over a generic SMTP transport. The implemented contract covers Dev sessions, local Container sessions, and Hosted Capsules; keeps credentials in Server env; validates Postmark, Mailgun, SMTP2GO, and generic SMTP header extensions; bounds transport lifecycles; and recommends durable Jobs plus application-level idempotency for important notifications. Planning remains in `.scratch/smtp-mail/PRD.md` for traceability. |
| Multi-provider OAuth | implemented | Runtime-owned provider auth supports Google, Microsoft, Apple, and Facebook behind the provider-neutral `auth.signIn(provider)` client intent. Provider-specific OAuth mechanics, stable `(provider, subject)` identities, secret-safe configuration, callbacks, and Session linking remain server-owned. Protocol, browser, generated-runtime, and Container gates are complete; real four-provider Hosted Capsule acceptance remains external-blocked pending registrations and credentials. Planning remains in `.scratch/multi-provider-oauth/PRD.md` for traceability. |
