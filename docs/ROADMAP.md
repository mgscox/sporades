...
| Privileged server role | design | Gives Capsule server code a server-only actor for trusted system-owned execution that intentionally runs without a Sporades user identity, such as scheduled Jobs or platform-owned maintenance inside a Capsule. Capsule admin authorization stays separate as Capsule roles checked through normal ACL rules. Privileged server role activity must be auditable through the implemented Privileged audit event contract, must not appear in browser credentials or user/team membership, and must distinguish userless system-owned execution from running as a captured user. | Needs PRD. Use the implemented Privileged audit event contract, ADR 0022, ADR 0026, ADR 0027, Capsule role terminology, and Job Queue / Job scheduling planning as source artifacts. |
| Job queue | design | Gives Capsule authors durable background work with retry behavior, failure visibility, and a SQLite-first default that can later grow through adapters. It should shape the queue and Job concepts before Job scheduling commits to whether recurring work builds on the queue or stays timer-only. | Needs PRD. Use `.scratch/post-v2-platform-hardening-and-ops/issues/08-add-job-queue.md` and the Ops And Automation roadmap notes as source planning artifacts. |
| Job scheduling | design | Gives Capsule authors a first-class way to run recurring work with persistence, missed-run behavior, timezone handling, and duplicate-run protection. It can support platform needs such as automated backups, but is blocked on Privileged server role because recurring Jobs need an explicit execution actor when no live user session exists. | Blocked by Privileged server role. Needs PRD after that actor boundary is clear. Use `.scratch/post-v2-platform-hardening-and-ops/issues/09-add-job-scheduling.md` and the Ops And Automation roadmap notes as source planning artifacts. |
...
| --- | --- | --- |
| Privileged audit event contract | implemented | Privileged audit events are implemented as a narrow platform-owned JSONL audit surface, not a new audit database or centralized logging system. The contract records actor kind, operation, surface, Capsule identity, target resource, outcome, safe error code, and redacted bounded metadata, and current coverage includes Sporades-controlled SSH configuration, lifecycle, and inspection events. App `ctx.log` and browser/client credentials cannot forge privileged audit events. Real `sshd` auth/session capture remains future scanner work. Planning remains in `.scratch/privileged-audit-event-contract/PRD.md` and `.scratch/privileged-audit-event-contract/ssh-daemon-session-log-scanner-spike.md` for traceability. |
