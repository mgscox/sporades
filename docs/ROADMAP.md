...
| User journey tracker | ready | Add a runtime-owned tracker that the Capsule must explicitly declare before `sporades/client` can read, subscribe, or publish. Client code explicitly enables page-runtime consent; the server lazily creates Journey sessions from accepted publication activity. Every new connection or configured inactivity gap creates a new session ID, while short-lived Journey state remains independently TTL-buffered. One user may have multiple simultaneous Journey sessions, and inactivity is derived when none has live state. All connected Capsule clients receive live updates in V1; future Team-based delivery filtering may restrict delivery to users who share an enabled Team. Planning is ready in `.scratch/user-journey-tracker/PRD.md`. |
