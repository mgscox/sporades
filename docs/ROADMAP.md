...
| --- | --- | --- |
| User journey tracker | implemented | Capsules can explicitly enable privacy-safe, client-only Journey publication with page-runtime consent, lazy server-owned sessions, reconnect and inactivity segmentation, bounded latest-state TTL buffering, flat multi-session records, and snapshot-first realtime observation. Planning remains in `.scratch/user-journey-tracker/PRD.md` for traceability. |
