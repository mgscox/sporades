...
ADR-0024 keeps this ADR's high-level `files` SDK model, Sporades-owned read
routes, and File version cache busting, but replaces the old user-scoped
`default` bucket wording. Current uploads use logical, Capsule-scoped File paths
and File metadata/ACL ownership semantics. The Default File bucket is only a
logical namespace fallback for omitted File paths, not a user bucket or policy
boundary.
...
Uploaded files are private by default through runtime-owned File metadata and authorization behavior. Apps can explicitly generate public file URLs, but each URL is a server-managed record that must specify exactly one expiry choice: `ttlSeconds`, `expires` as a valid ISO string, or `noExpiry: true`; URLs can be revoked before expiry. The SDK also owns private file access through `files.url(fileReference)`, `files.download(fileReference)`, `files.delete(fileReference)`, and replacement via `files.upload(file, { replace: true, fileReference })`, where ADR-0024 defines File references as File IDs or absolute File paths. Replacement preserves the file ID but creates a new file version so stale private and public URLs cannot keep serving cached bytes. This keeps app code independent of local storage, S3-style presigned URLs, and storage backend changes while avoiding table-scoped or field-scoped file semantics in v2.
