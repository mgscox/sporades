...
| --- | --- |
| `blank` | The minimal Capsule layout, the selected framework's connection to `sporades/client`, and a built-in but disabled Stripe payment foundation with no UI or credentials. Its server policy seams, durable Checkout and Customer Portal Jobs, Capsule-owned Customer resolver, and validated client redirect helpers support common hosted payment actions after deliberate activation. Start here when you do not want example domain code. |
