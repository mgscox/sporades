...
- [x] A fresh `photo-library` scaffold can install dependencies and start successfully.
- [x] Browser-level verification uploads an anonymous image and confirms it appears in the public gallery.
...
- Focused regression coverage:
  - `node --test --test-name-pattern 'photo-library|photo library' test/create.test.js`
  - `node --test --test-name-pattern 'photo-library|photo library|Photo Library|scaffolded React photo library|scaffolded Preact photo library' test/dev.test.js`
- The template now scaffolds anonymous and Google providers together, includes server-env placeholders so fresh Capsules start immediately, and documents replacing them with real OAuth credentials via `sporades auth set google`.
