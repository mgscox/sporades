...
| Adapter admission and storage faults | Only runtime-eligible file-backed writable SQLite adapters may acquire a resource connection; busy remains retryable while SQLite setup and receipt failures are redacted. | The same eligibility is checked before outer admission, so `:memory:` run/status callbacks never enter; caught and unawaited acquired constraint failures poison settlement with `RESOURCE_STORAGE_ERROR`. | Focused mutation and endpoint tests cover setup, admitted unique-constraint, memory run/status, caught and unawaited paths. |
...
| Retained effects | Escaped scope effects reject once the resource callback closes. | A retained `notifications.accept` checks lifetime before it can poison a settled transaction, while active acceptance remains unsupported until ticket 06. | Retained mutation and endpoint scope regression preserves committed writes and receipts. |
