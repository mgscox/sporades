...
| 07 | rework | None (04, 05, 06 done) | `b18dae7` | `codex/multi-provider-oauth-07` / `/private/tmp/sporades-multi-provider-oauth-swarm/issue-07` | `/root/oauth_04` | `663cd35` | `/root/oauth_01_review` | REQUEST_CHANGES for `b18dae7...663cd35`: no-redirect bounded Google/Apple exchange; enqueue-time Job provenance; remove legacy user.provider reads/writes; real cross-provider E2E matrix | — | focused gates green; review sandbox listener failures are EPERM only | same worker rework required; replacement SHA will void prior verdict |
