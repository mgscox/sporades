...
Prove that MinIO-backed storage preserves the existing Sporades file lifecycle through current client/server routes and SDK behavior, including the new File path and File reference semantics. App and client code should not know or care whether uploaded bytes live in local filesystem storage or MinIO.

This slice is complete when MinIO can be used as a drop-in storage backend for implemented file features in Dev sessions and local Container sessions.
...
- [ ] Uploads without an explicit File path use the uploaded file name in the Default File bucket.
...
- [ ] Writing new bytes to an existing File path overwrites the existing file, keeps the File ID, creates a new File version, and preserves cache-busting behavior.
...
- .scratch/minio-storage-s3-compatible-adapter/issues/03-expose-file-references-in-acl-storage-helpers.md
- .scratch/minio-storage-s3-compatible-adapter/issues/05-implement-s3-compatible-storage-adapter-for-minio.md
