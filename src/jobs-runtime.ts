...
export async function ensureScheduleStorage(sqlite: LooseRecord) {
...
export async function cancelJob(database: LooseRecord, context: any, id: any) { const sql = database.adapter.dialect.sql; const row = context.__privilegedJobAccess ? await database.adapter.prepare(sql("SELECT * FROM [sporades_jobs] WHERE [id] = ?")).get(id) : await database.adapter.prepare(sql("SELECT * FROM [sporades_jobs] WHERE [id] = ? AND [actorUserId] = ?")).get(id, context.auth.userId); if (!row) return null; const now=database.clock.now().toISOString(); if (["queued","delayed"].includes(row.status)) { await database.adapter.prepare(sql("UPDATE [sporades_jobs] SET [status]='cancelled', [completedAt]=? WHERE [id]=?")).run(now,id); return jobState({...row,status:"cancelled",completedAt:now},true); } if(row.status==="running"){ const runtimeDatabase=database.__rootDatabase ?? database; runtimeDatabase.__jobAbortControllers?.get(id)?.abort(); await database.adapter.prepare(sql("UPDATE [sporades_jobs] SET [cancelRequestedAt]=? WHERE [id]=?")).run(now,id); return jobState({...row,cancelRequestedAt:now},true);} throw jobError("INVALID_JOB_STATE","Job cannot be cancelled from its current state.","Only queued, delayed, or running Jobs can be cancelled."); }
