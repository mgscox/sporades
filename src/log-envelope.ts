...
export function minimumLogPayloadMaxBytes(config: LooseRecord = {}) {
...
export function logPayloadMaxBytes(config: LooseRecord = {}) {
  return config.logs?.payloadMaxBytes ?? config.logging?.payloadMaxBytes ?? 4096;
}

export function validateLogConfig(config: LooseRecord = {}) {
  ...
  for (const key of ["logs", "logging"]) {
    ...
    if (value !== undefined && (!Number.isSafeInteger(value) || value < minimum)) fail(key);
  }
  ...
  if (logPayloadMaxBytes(config) < minimum) fail("logs");
}
