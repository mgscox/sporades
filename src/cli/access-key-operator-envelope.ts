...
export const ACCESS_KEY_OPERATOR_ENVELOPE_BYTE_LIMIT = JSON_STRING_MAX_BYTE_EXPANSION * (
  ((ACCESS_KEY_OPERATOR_LIST_PAGE_LIMIT + 1) * ACCESS_KEY_SCOPE_LIMIT * ACCESS_KEY_SCOPE_BYTE_LIMIT)
  + (ACCESS_KEY_OPERATOR_LIST_PAGE_LIMIT * ACCESS_KEY_GRANT_LIMIT * ACCESS_KEY_GRANT_BYTE_LIMIT)
) + ACCESS_KEY_OPERATOR_ENVELOPE_STRUCTURAL_HEADROOM;
...
function stringList(value: unknown, maximumItems: number, maximumItemBytes: number) {
  return Array.isArray(value) && value.length <= maximumItems
    && value.every((item) => boundedString(item, maximumItemBytes));
}

function optionalRevocationCause(value: unknown) {
