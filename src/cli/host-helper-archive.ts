...
function listArchiveEntries(archivePath: string) {
  ...
  // macOS bsdtar otherwise interprets AppleDouble entries as copyfile metadata:
  // it reports a platform-specific truncation error while still printing the
  // literal entries. Classify that bounded partial listing before falling back
  // to the generic inspector failure; extraction must never become the first
  // place an AppleDouble-only archive is diagnosed.
  ...
  const entries = names.length === verboseLines.length
...
function inspectLiteralArchiveMetadata(archivePath: string) {
  let archive: Buffer;
  try {
  ...
  while (offset + 512 <= archive.length) {
    ...
    entries.push({ name: prefix ? `${prefix}/${name}` : name, type: String.fromCharCode(header[156] || 48) === "0" ? "-" : String.fromCharCode(header[156]) || "-", size });
    offset += 512 + (Math.ceil(size / 512) * 512);
  }
  return entries.some((entry) => isDiscardableArchiveMetadata(entry.name)) ? entries : [];
}

function tarHeaderText(header: Buffer, offset: number, length: number) {
  const end = header.indexOf(0, offset);
  return header.toString("utf8", offset, end >= offset && end < offset + length ? end : offset + length);
}
