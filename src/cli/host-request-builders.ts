...
export function createHostLifecycleRequest(
  alias: string,
  profile: LooseRecord,
  subname: string,
  options: LooseRecord = {},
...
export function createHostStatsRequest(profile: LooseRecord, subname: string) {
  return {
    domain: profile.domain,
    subname,
    hostedUrl: `${profile.scheme}://${subname}.${profile.domain}`,
...
export function createHostRuntimeHealthRequest(profile: LooseRecord, subname: string) {
...
export function createHostBootstrapRequest(profile: LooseRecord) {
...
export function createHostUnregisterRequest(profile: LooseRecord, subname: string) {
  const bootstrap = createHostBootstrapRequest(profile);
  const capsuleDirectory = posixJoin(bootstrap.directories.capsules, subname);
  return {
    subname,
...
export function createHostDeleteRequest(profile: LooseRecord, subname: string) {
  const bootstrap = createHostBootstrapRequest(profile);
  const capsuleDirectory = posixJoin(bootstrap.directories.capsules, subname);
  return {
    subname,
...
function createHostedContainerName(domain: string, subname: string) {
