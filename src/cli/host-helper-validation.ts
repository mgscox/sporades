...
export function missingCapsuleHint(request: HostHelperRequest, purpose: string) {
  if (purpose === "push") {
...
export function hostRegistryRetryCommand(request: HostHelperRequest) {
  return request.action === "host.stats" ? `sporades host stats --host ${request.host.alias}` : `sporades host list --host ${request.host.alias}`;
}

export function validateLifecycleRequest(request: HostHelperRequest) {
...
export function validateSealedEnvRotationRequest(request: HostHelperRequest) {
  const requiredStrings = [
    request.host?.domain,
    request.host?.alias,
    request.host?.remoteRoot,
...
export function validateStatsRequest(request: HostHelperRequest) {
  const requiredStrings = [
    request.host?.domain,
    request.host?.alias,
    request.host?.remoteRoot,
...
export function validateReleaseListRequest(request: HostHelperRequest) {
  const requiredStrings = [
    request.host?.domain,
    request.host?.alias,
    request.host?.remoteRoot,
...
export function validateHealthRequest(request: HostHelperRequest) {
  const requiredStrings = [
    request.host?.domain,
    request.host?.alias,
    request.host?.remoteRoot,
...
export function validateScheduleInspectionRequest(request: HostHelperRequest) {
...
export function validateHostStatsRequest(request: HostHelperRequest) {
  const requiredStrings = [
    request.host?.domain,
    request.host?.alias,
    request.host?.remoteRoot,
...
export function validateRollbackRequest(request: HostHelperRequest) {
  const requiredStrings = [
    request.host?.domain,
    request.host?.alias,
    request.host?.remoteRoot,
...
export function validateHostLogsRequest(request: HostHelperRequest, limits: HostLogLineLimits) {
  const requiredStrings = [
    request.host?.alias,
    request.host?.domain,
    request.host?.remoteRoot,
...
export function validateListRequest(request: HostHelperRequest) {
  const requiredStrings = [
    request.host?.domain,
    request.host?.alias,
    request.host?.remoteRoot,
...
export function validateListRegistryRecord(request: HostHelperRequest, record: unknown, recordPath: string) {
  const capsuleRecord = record as Partial<HostHelperRequest["registration"]> & { subname?: unknown; domain?: unknown; remoteCapsuleId?: unknown };
...
export function validateBootstrapRequest(request: HostHelperRequest) {
  const requiredStrings = [
    request.host?.domain,
    request.host?.alias,
    request.host?.remoteRoot,
...
export function validateRegisterRequest(request: HostHelperRequest) {
  const requiredStrings = [
    request.host?.domain,
    request.host?.alias,
    request.host?.remoteRoot,
...
export function validateUnregisterRequest(request: HostHelperRequest) {
  const requiredStrings = [
    request.host?.domain,
    request.host?.alias,
    request.host?.remoteRoot,
...
export function validateDeleteRequest(request: HostHelperRequest) {
  const requiredStrings = [
    request.host?.domain,
    request.host?.alias,
    request.host?.remoteRoot,
...
export function validateInstallRequest(request: HostHelperRequest) {
  ...
  if (!Array.isArray(release.files) || release.files.some((file: unknown) => !isExpectedClaimedReleaseFile(file, deployFiles.map((entry) => entry.path)))) {
  ...
  if (claimedFiles.length !== sortedExpectedFiles.length || claimedFiles.some((file: string, index: number) => file !== sortedExpectedFiles[index])) {
    throw helperError("Invalid Hosted Capsule release file list.", "Update the Sporades CLI and retry `sporades host push`.");
  }
...
function validateClaimedReleaseFiles(files: string[]) {
