...
export async function readProjectConfig(projectDir: string) {
  ...
  if (config.payments !== undefined) config.payments = validatePaymentsConfig(config.payments);
...
export function validatePasswordResetConfig(auth: LooseRecord) {
  const passwordReset = auth?.email?.passwordReset;
  if (passwordReset === undefined) return;
  ...
  if (passwordReset.ttlMs !== undefined && (
    typeof passwordReset.ttlMs !== "number"
...
function isSameOriginResetPath(value: unknown) {
...
export function validateClientConfig(client: LooseRecord) {
  ...
  if (client.toolchain !== undefined && !isClientToolchain(client.toolchain)) {
...
export function validatePaymentsConfig(payments: unknown) {
  ...
  const paymentRecord = payments as LooseRecord;
  ...
  const stripeRecord = stripe as LooseRecord;
