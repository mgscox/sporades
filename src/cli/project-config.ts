...
import { CLIENT_FRAMEWORKS, CLIENT_TOOLCHAINS, supportsClientCapability } from "../client-capabilities.js";
...
const CLIENT_FRAMEWORK_SET = new Set(CLIENT_FRAMEWORKS);
const CLIENT_TOOLCHAIN_SET = new Set(CLIENT_TOOLCHAINS);
...
export function validateClientConfig(client: LooseRecord) {
  ...
  if (client.framework !== undefined && !CLIENT_FRAMEWORK_SET.has(client.framework as any)) {
    throw commandError(`Unsupported framework: ${client.framework}`, "Use one of: react, preact, inferno, lit, solid, vue, svelte, vanilla.");
  }
  if (client.toolchain !== undefined && !CLIENT_TOOLCHAIN_SET.has(client.toolchain as any)) {
  ...
  if (client.framework !== undefined && client.toolchain !== undefined && !supportsClientCapability(client.framework, client.toolchain)) throw commandError(`Unsupported client framework/toolchain combination: ${client.framework}/${client.toolchain}`, "Choose an admitted pair from the client capability matrix in docs/user-guide.md.");
}
...
export function validateProjectConfigShape(config: unknown) {
...
export function validateSecurityConfig(security: LooseRecord) {
  ...
  if (cors !== undefined) {
    ...
    if (
      cors.allowedOrigins !== undefined &&
      (!Array.isArray(cors.allowedOrigins) || !cors.allowedOrigins.every((origin: unknown) => typeof origin === "string"))
    ) {
...
export function resolveEffectiveSecurityPolicy(config: LooseRecord, session: string) {
...
export async function resolveLocalContainerSshAccess(config: LooseRecord, projectDir: string) {
...
export async function resolveAuthorizedKeyLines(ssh: LooseRecord, projectDir: string) {
  if (ssh === undefined) {
    return [];
  }
...
export function authorizedKeyFingerprint(line: string) {
...
export function withRuntimeSecuritySession(config: LooseRecord, session: string) {
  return {
    ...config,
...
export function readBaseImageUpdatePolicy(config: LooseRecord) {
  return normaliseBaseImageUpdatePolicy(config?.baseImage?.updatePolicy ?? config?.deploy?.baseImageUpdatePolicy);
}

async function readRequiredFile(filePath: string, message: string, hint: string) {
  try {
...
async function readAuthorizedKeysFile(filePath: string, index: number) {
  try {
    return await readFile(filePath, "utf8");
  } catch {
    throw commandError(
...
function resolveProjectFileReference(filePath: string, projectDir: string) {
...
function normaliseAuthorizedKeyMaterial(material: string, source: string) {
...
function looksLikePrivateKey(material: string) {
  return /-----BEGIN [A-Z0-9 ]*PRIVATE KEY-----/.test(material);
}

function validateAuthorizedKeyLine(line: string, source: string) {
  const parts = line.split(/\s+/);
  const keyTypeIndex = parts.findIndex((part) => isOpenSshPublicKeyType(part));
...
function isOpenSshPublicKeyType(value: string) {
...
function malformedAuthorizedKeyError(source: string) {
  return commandError(
...
function decodeAuthorizedKeyBlob(value: string) {
...
function isValidOpenSshPublicKeyBlob(keyType: string, blob: Buffer) {
...
function readSshString(blob: Buffer, offset: number) {
