...
async function registerCapsule(request: HostHelperRequest) {
  ...
  const pendingClaim = await readFile(claimPath, "utf8").then((contents) => JSON.parse(contents)).catch((error) => {
    if (errorDetails(error).code === "ENOENT") return null;
    throw error;
  });
  if (pendingClaim) {
    const aliases = validateAliasDomains(pendingClaim.aliasDomains);
    if (pendingClaim.domain !== request.host.domain || pendingClaim.subname !== request.capsule.subname
      || (request.registration?.aliasDomains !== undefined
        && !isDeepStrictEqual([...request.registration.aliasDomains].sort(), [...aliases].sort()))) {
  ...
  let priorRoute: ReleaseInstallRouteSnapshot | null = null;
  let routeAttempted = false;
  let claimWritten = Boolean(pendingClaim);
  const reserveClaim = async (previous: any = null) => {
    ...
    await publishHostHelperBytes(Buffer.from(JSON.stringify({
      domain: registration.domain, subname: registration.subname,
  ...
  // A failed retry may have restored an already-orphaned route. Retain its claim
  // until registration commits, even if this round's rollback itself succeeded.
...
async function assertRegistrationRecoveryComplete(request: HostHelperRequest) {
  const claimPath = path.join(request.host.remoteRoot, "hosts", request.host.domain, "registry", "registration-claims", `${request.capsule.subname}.json`);
  ...
  if (pending) {
    throw helperError(
      "Hosted Capsule registration recovery is required before teardown.",
      "Repair the reported Host services or storage problem and retry the original registration with the same alias domains before unregistering or deleting this Capsule.",
    );
  }
}
...
async function startCapsule(request: HostHelperRequest, options: LooseRecord = {}) {
  ...
  for (const file of resolveDeployFiles(recordedRelease?.source?.deployFiles)) {
    if (file.update === "preserve") await assertDeployFile(path.join(paths.capsule, "preserved-files"), file.path);
  }
...
function releasePrivateKeyPath(paths: ReleasePaths, releaseId: string) {
  return path.join(paths.data, "sealed-server-env", "releases", `${releaseId}.private.pem`);
}

async function installSealedServerEnvPrivateKey(release: HostHelperRelease, paths: ReleasePaths) {
...
type ReleaseInstallRouteSnapshot = {
  lifecycle: HostedCapsuleLifecycle;
  routeFile: string;
  contents: string | null;
};

async function captureReleaseInstallRoute(request: HostHelperRequest, previousRecord: any): Promise<ReleaseInstallRouteSnapshot> {
...
async function restoreCurrentReleasePointerTarget(currentLink: string, previousTarget: string | null) {
...
async function restoreReleaseInstallRoute(snapshot: ReleaseInstallRouteSnapshot, reloadWhenAbsent = false) {
...
async function restoreFailedReleaseInstall(
  request: HostHelperRequest,
  paths: ReleasePaths & { release: string },
  previousRecord: any,
  previousRegistryContents: string,
  previousCurrentTarget: string | null,
  previousRoute: ReleaseInstallRouteSnapshot,
  priorRuntime: CapsuleRuntimeSettlement | null,
  release: HostHelperRelease,
) {
  try {
    await restoreCurrentReleasePointerTarget(paths.currentLink, previousCurrentTarget);
...
function normaliseLifecycle(request: HostHelperRequest, registryRecord: any = null, options: LooseRecord = {}): HostedCapsuleLifecycle {
  const provided = (options.ignoreProvidedLifecycle === true ? {} : request.lifecycle ?? {}) as HostedCapsuleLifecycle;
  const paths = canonicalReleasePaths(request);
  const subname = request.capsule.subname;
  const domain = request.host.domain;
...
function canonicalLifecycleRouteTls(request: HostHelperRequest, registryRecord: any, provided: HostedCapsuleLifecycle) {
  const suppliedModes = [(provided.routes?.running?.tls as any)?.mode, (provided.routes?.unavailable?.tls as any)?.mode]
...
function assertCanonicalLifecycleAuthority(provided: HostedCapsuleLifecycle, canonical: any) {
  ...
  for (const [route, expected] of [[provided.routes?.running, canonical.routes.running], [provided.routes?.unavailable, canonical.routes.unavailable]] as const) {
    if (!route) continue;
    ...
    for (const [key, value] of Object.entries(route)) {
      ...
      if (key === "log" && value && typeof value === "object") {
        ...
        exactWhenSupplied((value as any).file, expected.log.file);
        continue;
      }
...
function normaliseRegistration(request: HostHelperRequest, existing: any = null) {
...
function routeVariants(route: HostedCapsuleRoute): HostedCapsuleRoute[] {
...
function renderRunningRoute(route: HostedCapsuleRoute) {
...
async function writeUnavailableRoute(lifecycle: HostedCapsuleLifecycle) {
  ...
  await applyManagedRoute(
    lifecycle,
    route.routeFile,
    routeVariants(route).map((variant) => renderRoute(variant, renderUnavailableRouteHandler(variant))).join("\n"),
  );
}
...
async function writeRegistryContentsAtomic(registryRecordPath: any, contents: string) {
