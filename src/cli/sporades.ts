...
const SUPPORTED_TEMPLATES: Set<string> = new Set(CLIENT_TEMPLATES);
  ...
  switch (subcommand) {
    ...
    case "set":
      ...
      if (!disable && !["anonymous", "email"].includes(provider)) {
        ...
        if (missing) {
          throw commandError(
            `Missing ${providerLabel(provider)} OAuth credentials.`,
            provider === "apple"
...
function isValidMicrosoftTenantSelection(value: any) {
...
function readProviderClientCredentials(provider: string, clientJsonPath: string, projectDir: string) {
  ...
  if (!credentials) {
    ...
    throw commandError(
      `OAuth client JSON is missing ${providerLabel(provider)} credentials.`,
      `Use a ${providerLabel(provider)} credentials JSON file with the fields documented by \`sporades auth --help\`.`,
    );
  }
  return credentials;
}

function parseProviderCredentialDocument(provider: string, value: unknown): LooseRecord | null {
...
function hasNonEmptyString(value: LooseRecord, key: string) {
...
async function createProjectFromLocalTemplate(options: LooseRecord) {
...
type LocalTemplateIgnoreRule = { ignored: boolean; pattern: RegExp };

async function readLocalTemplateIgnoreRules(sourceDir: string): Promise<LocalTemplateIgnoreRule[]> {
  let contents = "";
  try {
...
function shouldCopyLocalTemplatePath(
  sourceDir: string,
  sourcePath: string,
  rules: LocalTemplateIgnoreRule[],
) {
...
async function finalizeLocalTemplateProject(options: LooseRecord, projectDir: string) {
  const packagePath = path.join(projectDir, "package.json");
  const configPath = path.join(projectDir, "sporades.json");
  let packageJson: LooseRecord;
...
async function manageAuth(options: LooseRecord) {
  switch (options.subcommand) {
    case "status": {
      ...
      if (options.json) {
        ...
        for (const [provider, state] of Object.entries(status.providers) as [string, any][]) {
  ...
  const previous = existingProviders[options.provider] && typeof existingProviders[options.provider] === "object"
    ...
    : {};
  const providerConfig: LooseRecord = { ...previous, enabled: !options.disable };
  ...
  const enabledSibling = Object.entries(existingProviders).find(([provider, value]: [string, any]) =>
    provider !== "anonymous" && provider !== options.provider && (value === true || value?.enabled !== false),
  )?.[0];
  config.auth = {
...
async function startContainerSession(options: LooseRecord) {
  ...
  const previousFileAccess: Array<{ host: string; uid: number; gid: number; mode: number; dev: number; ino: number }> = [];
  try {
    ...
    if (!candidateRetained) {
      for (const previous of previousFileAccess.reverse()) {
        try {
          const current = await lstat(previous.host).catch((error) => { if (error.code !== "ENOENT") throw error; return null; });
          if (!current) continue;
          const owner = `${previous.uid}:${previous.gid}`;
          runDocker(localPreservedFileAccessArgs(previous.host, owner, owner, SPORADES_BASE_IMAGE.image, previous.mode, previous), options.projectDir,
            "Failed to restore preserved file access.", "Repair the previous runtime's preserved-file permissions before restarting it.");
...
function renderServerEnvValues(existingRaw: string, values: { [s: string]: unknown; } | ArrayLike<unknown>) {
...
function serializeServerEnvValue(value: unknown) {
...
async function writeAuthConfiguration(
  configPath: string,
  envPath: string,
  config: LooseRecord,
...
async function ensureLocalPreservedFileAccess(
  projectDir: string,
  target: string,
  localUser: string,
  fileUser: string,
  failure: { message: string; hint: string },
  record?: Array<{ host: string; uid: number; gid: number; mode: number; dev: number; ino: number }>,
) {
  const info = await lstat(target);
  if (preservedFileAccessMatches(info, localPreservedFileAccess(localUser, fileUser))) return;
