...
async function resolveLocalContainerSshAccess(config: LooseRecord, projectDir: string) {
...
async function resolveAuthorizedKeyLines(ssh: LooseRecord, projectDir: string) {
  if (ssh === undefined) {
    return [];
  }
...
function authorizedKeyFingerprint(line: string) {
  const parts = line.split(/\s+/);
  const keyTypeIndex = parts.findIndex((part) => isOpenSshPublicKeyType(part));
