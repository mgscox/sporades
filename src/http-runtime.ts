...
// monolith's copies the way a `const` would (ADR-0041).
...
// `RuntimeRequestLike` by the three origin predicates and `resolveOAuthRequestOrigin`. Exported
// because `declaration: true` will not emit a signature that names a private type.
...
function boundedRequestTargetPath(target: unknown) {
...
export function writeInvalidHttpRequestTarget(database: LooseRecord, request: IncomingMessage, response: Pick<ServerResponse, "writeHead" | "end">) {
  const error: any = new Error("Invalid HTTP request target.");
  error.code = "INVALID_HTTP_REQUEST_TARGET";
...
export async function readLimitedRequestBody(request: any, limitSource: LooseRecord | number | null = null) {
...
export function resolveHttpMaxBodyBytes(source: LooseRecord | number | null = null) {
...
export function resolveRuntimeSecurityPolicy(config: RuntimeConfig = {}): RuntimeSecurityPolicy {
...
export function websocketOriginAllowed(policy: RuntimeSecurityPolicy, request: RuntimeRequestLike) {
...
export function normalizeOrigin(value: any) {
...
export function resolveOAuthRequestOrigin(policy: LooseRecord, request: RuntimeRequestLike) {
...
export function singleHttpHeader(value: any) {
...
function safeMultipartLimitDetails(error: any) {
  ...
  const kinds = new Set(["maxPartHeaderBytes", "maxFieldCount", "maxFieldBytes", "maxTotalFieldBytes", "maxFiles", "maxFileBytes", "fileMaxSizeBytes", "maxTotalFileBytes"]);
