  ...
  if (node.type === "VariableDeclaration") {
    ...
    for (const declaration of (node.declarations as RendererSyntaxNode[] | undefined) ?? []) {
      if (node.kind === "var" && !declarationScope.parent) {
        const declared: RendererLexicalScope = { functionScope: true, bindings: new Set() };
        addRendererBinding(declared, declaration.id);
        for (const name of declared.bindings) if (!["require", "module", "exports", "__dirname", "__filename", "arguments"].includes(name)) declarationScope.bindings.add(name);
...
function unknownPrerenderMarkerWarning(name: string): ClientPrerenderWarning {
...
export function placeClientPrerenderFragments(html: string, fragments: readonly { name: string; html: string }[]) {
  ...
  if (placement.markers.length > 0) {
    ...
    for (const marker of placement.markers) {
      ...
      if (marker.name === undefined) replaced += fragments.map(expand).join("");
