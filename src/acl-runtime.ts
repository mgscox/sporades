...
export function createPrivilegedAuditEmitter(log: { emit: (input: LooseRecord) => any; }) {
  return {
...
export function emitPrivilegedAuditEvent(target: LooseRecord, details: LooseRecord = {}) {
...
export async function emitPrivilegedRunAudit(database: LooseRecord, context: LooseRecord, details: LooseRecord) {
...
export async function reindexPrivilegedAuditEventsAfterRollback(database: LooseRecord, context: LooseRecord | undefined) {
...
export function normalizePrivilegedRunSignal(value: any) {
...
export function createPrivilegedRunAuditDetails(context: LooseRecord, options: LooseRecord) {
...
export function createPrivilegedRunPublicError(cause: any) {
...
export function createPrivilegedAuditEmissionPublicError(cause: any, context: LooseRecord | undefined = undefined) {
  const error = commandError(
...
export function isPrivilegedAuditEmissionPublicError(error: any) {
...
export function createPrivilegedFileApi(database: LooseRecord, contextGetter: () => LooseRecord) {
...
export function normalizeTableAcl(tableName: any, aclRules: LooseRecord | undefined) {
...
export function grantPrivilegedDbAccess(context: any) {
...
export function revokePrivilegedDbAccess(context: any) {
  if (context && typeof context === "object") {
...
export function runTableWriteWithAcl(database: any, table: LooseRecord, operation: string, previous: any, next: any, contextGetter: any, write: () => any) {
  if (hasPrivilegedDbAccess(contextGetter?.())) {
    return write();
  }
...
export function applyReadAcl(database: any, table: LooseRecord, row: any, context: any) {
...
function createAclHelpers(database: any, context: any) {
  const state = { readCount: 0, maxReads: 32, touchedAsyncRead: false, unconsumedAsyncReads: new Set<Promise<any>>(), pendingAsyncReads: new Set<Promise<any>>() };
  const helpers = {
...
export function emitAclDeniedLog(database: LooseRecord, details: LooseRecord) {
  database.log?.emit?.({
    category: "platform",
    event: "acl.denied",
    level: "warn",
...
export function assertActivePrivilegedJobAccess(contextGetter: () => LooseRecord) {
