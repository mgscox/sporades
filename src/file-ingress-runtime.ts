...
async function clamavInstream(database: RecordLike, bytes: Buffer) {
  ...
  const socketPath = database.__clamavTest?.socketPath ?? "/tmp/sporades-clamd.sock"; const timeoutMs = database.__clamavTest?.timeoutMs ?? 10_000;
