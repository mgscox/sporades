...
async function inspectIngressLease(policy: RecordLike | null, row: RecordLike, bytes: Buffer) {
  ...
  const verdicts = await Promise.all(policy.requiredInspectors.map(async (inspector: string) => Object.freeze({ inspector, outcome: inspector === "content-policy-v1" ? await contentPolicyOutcome(row, bytes) : "inconclusive", leaseId: row.leaseId, size: row.size, digest: row.digest, version: row.version, policyRevision: policy.policyRevision, engine: inspector === "content-policy-v1" ? "sporades-content-policy" : "clamav", signatureVersion: inspector === "content-policy-v1" ? "content-policy-v1" : "unavailable", inspectedAt })));
