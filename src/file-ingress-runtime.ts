...
export function isJavaScriptRawInputWithinBounds(text: string) {
  ...
  const closings: Array<{ closing: string; resumeTemplate: boolean }> = [];
  let mode: "code" | "single" | "double" | "template" | "line-comment" | "block-comment" | "regex" = "code";
  let canStartRegex = true; let regexCharacterClass = false; let regexGroups = 0;
  for (let index = 0; index < text.length; index += 1) {
    const character = text[index]; const next = text[index + 1];
    if (mode === "single" || mode === "double") {
      if (character === "\\") index += 1;
      else if ((mode === "single" && character === "'") || (mode === "double" && character === '"')) { mode = "code"; canStartRegex = false; }
      continue;
    }
    if (mode === "line-comment") { if (character === "\n" || character === "\r") mode = "code"; continue; }
    if (mode === "block-comment") { if (character === "*" && next === "/") { mode = "code"; index += 1; } continue; }
    if (mode === "template") {
      if (character === "\\") index += 1;
      else if (character === "`") { mode = "code"; canStartRegex = false; }
      else if (character === "$" && next === "{") {
        closings.push({ closing: "}", resumeTemplate: true });
        if (closings.length > maximumContentPolicyParserRecursion) return false;
        mode = "code"; canStartRegex = true; index += 1;
      }
      continue;
    }
    if (mode === "regex") {
      if (character === "\\") { index += 1; continue; }
      if (regexCharacterClass) { if (character === "]") regexCharacterClass = false; continue; }
      if (character === "[") { regexCharacterClass = true; continue; }
      if (character === "(") { regexGroups += 1; if (regexGroups > maximumContentPolicyParserRecursion) return false; continue; }
      if (character === ")") { regexGroups = Math.max(0, regexGroups - 1); continue; }
      if (character === "/") {
        mode = "code"; canStartRegex = false;
    ...
    if (character === "/" && next === "/") { mode = "line-comment"; index += 1; continue; }
    if (character === "/" && next === "*") { mode = "block-comment"; index += 1; continue; }
    if (character === "/" && canStartRegex) { mode = "regex"; regexCharacterClass = false; regexGroups = 0; continue; }
    if (/[A-Za-z_$]/.test(character)) {
      let word = character;
      while (/[A-Za-z0-9_$]/.test(text[index + 1] ?? "")) { index += 1; word += text[index]; }
      canStartRegex = expressionPrefixKeywords.has(word); continue;
    }
    if (/[0-9]/.test(character)) {
      while (/[A-Za-z0-9_.]/.test(text[index + 1] ?? "")) index += 1;
      canStartRegex = false; continue;
    }
    const closing = closingFor.get(character);
    if (closing) {
      closings.push({ closing, resumeTemplate: false });
      if (closings.length > maximumContentPolicyParserRecursion) return false;
      canStartRegex = true; continue;
    }
    ...
    if (character === ".") { canStartRegex = false; continue; }
    canStartRegex = true;
  }
  return true;
}
