...
export async function stageMultipartIngress(database: RecordLike, endpoint: RecordLike, request: any, endpointRequest: RecordLike, actor: RecordLike) {
  ...
  for await (const chunk of request) { total += chunk.length; if (total > maxBytes) throw Object.assign(new Error("Multipart body exceeds declared limits."), { code: "MULTIPART_LIMIT_EXCEEDED" }); chunks.push(Buffer.from(chunk)); }
  ...
  for (let part of pieces) {
    ...
    part = part.slice(2); const split = part.indexOf("\r\n\r\n"); if (split < 0) throw Object.assign(new Error("Malformed multipart request."), { code: "INVALID_MULTIPART" });
    const rawHeaders = part.slice(0, split); const body = Buffer.from(part.slice(split + 4).replace(/\r\n$/, ""), "latin1");
