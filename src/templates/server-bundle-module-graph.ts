...
function createBundleInputsModule(options: ServerBundleModuleGraphOptions) {
...
function bundleModuleGraphError(message: string, hint: string) {
...
export async function createServerBundleModuleSource(options: ServerBundleModuleGraphOptions) {
  ...
  try {
    ...
    const message = error?.errors?.map((entry: any) => entry.text).join("; ") || error?.message || String(error);
