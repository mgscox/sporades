...
export default capsule({
  ...
  mutations: {
    seedCampfire: mutation((ctx) => {
      ...
      if (!ctx.db.messages.where("channel", "protect-the-crown").all().length) ctx.db.messages.insert({ channel: "protect-the-crown", body: "Is the crown adequately protected? 👍 All for one · 👎 One more guard, perhaps", authorId: "fixture:dartagnan", authorName: fixtureNames.dartagnan });
