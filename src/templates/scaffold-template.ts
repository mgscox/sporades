...
function compileGitignore(gitignoreContent: string): (relativePath: string) => boolean {
  const patterns: { text: string; isDir: boolean; regex: RegExp | null }[] = [];
    ...
    for (const { text, isDir, regex } of patterns) {
      if (regex) {
        ...
        for (let i = 0; i < parts.length; i++) {
          if (parts[i] === text) {
            if (isDir || i === parts.length - 1) return true;
          }
        }
      }
    }
...
async function readTemplateDirectory(templateDir: string): Promise<Record<string, string>> {
  let ignoreFilter: (relativePath: string) => boolean = () => false;
  try {
    ...
    ignoreFilter = (p: string) => userFilter(p) || alwaysIgnored(p);
  } catch {
...
export async function scaffoldFromDirectory(options: {
  ...
  let toolchain: string | null = options.toolchain ?? null;
