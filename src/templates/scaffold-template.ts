...
const {authAdapter,mutationAdapter,queryAdapter}=createInfernoAdapters();const channels=["general","ideas","random","protect-the-crown"] as const;const musketeers=[{key:"athos",name:"Athos",email:"athos@campfire.example"},{key:"porthos",name:"Porthos",email:"porthos@campfire.example"},{key:"aramis",name:"Aramis",email:"aramis@campfire.example"},{key:"dartagnan",name:"d'Artagnan",email:"dartagnan@campfire.example"}],password="all-for-one-campfire";export const infernoMark=mark;
export class App extends Component {session=authAdapter(this);profiles=queryAdapter<any[]>(this,"profiles");channelQueries={general:queryAdapter<Chat[]>(this,"messagesGeneral"),ideas:queryAdapter<Chat[]>(this,"messagesIdeas"),random:queryAdapter<Chat[]>(this,"messagesRandom"),"protect-the-crown":queryAdapter<Chat[]>(this,"messagesProtectTheCrown")};send=mutationAdapter(this,"sendMessage");reactMutation=mutationAdapter(this,"toggleReaction");seed=mutationAdapter(this,"seedCampfire");register=mutationAdapter(this,"registerFixture");channel="general";draft="";notice="";sharing=false;activities:Activity[]=[];fixturesPrepared=false;fixtureActive=false;generation=0;owner:number|null=null;tail=Promise.resolve();previousUserId:string|undefined;journeySubscription:{unsubscribe():void}|null=null;typing=createTypingPublisher((state)=>journey.set(state));
componentDidMount(){this.session.componentDidMount();this.profiles.componentDidMount();Object.values(this.channelQueries).forEach(q=>q.componentDidMount());this.journeySubscription=journey.subscribe((event:any)=>{const data=event?.data??event;if(data?.type==="snapshot")this.activities=data.states??[];else if(data?.type==="removed"||data?.type==="expired")this.activities=this.activities.filter(a=>a.sessionId!==data.state?.sessionId);else if(data?.state)this.activities=[...this.activities.filter(a=>a.sessionId!==data.state.sessionId),data.state];this.forceUpdate();});this.syncOwner();}
componentDidUpdate(){this.syncOwner();const profiles=this.profiles.state.data;if(!this.fixturesPrepared&&Array.isArray(profiles)){this.fixturesPrepared=true;if(["localhost","127.0.0.1","::1"].includes(window.location.hostname))void this.prepareFixtures(musketeers.filter(person=>!new Set(profiles.map((p:any)=>p.key)).has(person.key)));}}
componentWillUnmount(){this.journeySubscription?.unsubscribe();this.journeySubscription=null;this.previousUserId=undefined;++this.generation;void this.lane(()=>this.retire());Object.values(this.channelQueries).forEach(q=>q.componentWillUnmount());this.profiles.componentWillUnmount();this.session.componentWillUnmount();}
syncOwner(){const userId=this.session.state.auth?.userId;if(userId!==this.previousUserId){const old=this.previousUserId;this.previousUserId=userId;void this.authChanged(userId,old);}}
current(value:number,userId:string){return value===this.generation&&this.session.state.auth?.userId===userId;}async lane<T>(action:()=>Promise<T>){const before=this.tail;let release=()=>{};this.tail=new Promise<void>(resolve=>{release=resolve;});await before;try{return await action();}finally{release();}}
async retire(expected:number|null=null){this.typing.dispose();this.sharing=false;this.forceUpdate();if(this.owner===null||(expected!==null&&this.owner!==expected))return;this.owner=null;const r=await journey.disable();if(r?.error){this.notice=r.error.message;this.forceUpdate();}}
async enable(value:number,userId:string,expose=true){const r=await journey.enable({capture:{navigation:false,focus:false,interactions:false}});if(r.error){this.notice=r.error.message;this.forceUpdate();return false;}this.owner=value;if(!this.current(value,userId)){await this.retire(value);return false;}const published=await journey.set({status:"reading",metadata:{channel:this.channel},ttlSeconds:12});if(published?.error){this.notice=published.error.message;await this.retire(value);return false;}if(!this.current(value,userId)){await this.retire(value);return false;}if(expose){this.sharing=true;this.forceUpdate();}return true;}
async authChanged(userId:string|undefined,old:string|undefined){const value=++this.generation;await this.lane(async()=>{if(old&&old!==userId)await this.retire();if(!userId||this.session.state.auth?.isGuest||this.fixtureActive||!this.current(value,userId))return;const stored=await preferences.get();if(!this.current(value,userId)||stored.error||stored.data?.preferences.campfireShareActivity!==true)return;if(await this.enable(value,userId)&&this.current(value,userId)){this.notice="Activity sharing restored for this Musketeer.";this.forceUpdate();}});}
async setSharing(enabled:boolean){const userId=this.session.state.auth?.userId,value=++this.generation;this.sharing=false;this.notice="";this.forceUpdate();if(!userId){this.notice="Sign in before sharing activity.";this.forceUpdate();return;}await this.lane(async()=>{if(!this.current(value,userId))return;if(enabled){if(!await this.enable(value,userId,false)||!this.current(value,userId)){await this.retire(value);return;}}else{await this.retire();if(!this.current(value,userId))return;}if(!this.current(value,userId)){if(enabled)await this.retire(value);return;}const saved=await preferences.update({campfireShareActivity:enabled});if(saved?.error){this.notice=saved.error.message;if(enabled)await this.retire(value);this.forceUpdate();return;}if(!this.current(value,userId)){if(enabled)await this.retire(value);return;}this.sharing=enabled;this.forceUpdate();});}
async prepareFixtures(people=musketeers){this.fixtureActive=true;++this.generation;await this.lane(()=>this.retire());for(const person of people){let r=await auth.signUp("email",{email:person.email,password,name:person.name});if(r.error&&/already|exists|registered/i.test(r.error.message))r=await auth.signIn("email",{email:person.email,password});if(r.error){this.notice=r.error.message;this.fixtureActive=false;return;}const registered=await this.register.run(person.key);if(registered.error){this.notice=registered.error.message;this.fixtureActive=false;return;}await this.lane(()=>this.retire());await auth.signOut();}this.fixtureActive=false;const seeded=await this.seed.run();this.notice=seeded.error?seeded.error.message:"Development-only fixtures ready.";this.forceUpdate();}
async switchTo(person:any){++this.generation;await this.lane(()=>this.retire());await auth.signOut();const r=await auth.signIn("email",{email:person.email,password});this.notice=r.error?r.error.message:"Signed in as "+person.name+". Restoring activity preference…";this.forceUpdate();}
async choose(next:string){this.typing.dispose();this.channel=next;if(this.sharing)await journey.set({status:"reading",metadata:{channel:next},ttlSeconds:12});this.forceUpdate();}
...
render(){const messages=this.channelQueries[this.channel as keyof typeof this.channelQueries].state.data??[],profiles=this.profiles.state.data??[];return <main className="shell"><aside><p>Sporades exemplar</p><h1>🔥 Campfire</h1><nav>{channels.map(c=><button key={c} onClick={()=>this.choose(c)}># {c}</button>)}</nav></aside><section><header><h2># {this.channel}</h2><p role="status">{this.notice}</p></header><div>{messages.map(m=><article key={m.id}><strong>{m.authorName}</strong><time>{new Date(m.createdAt).toLocaleString()}</time><p>{m.body}</p><button aria-label="Thumbs up" onClick={()=>this.toggle(m.id,"up")}>👍</button><button aria-label="Thumbs down" onClick={()=>this.toggle(m.id,"down")}>👎</button></article>)}</div><form onSubmit={(e)=>this.submit(e)}><input id="message" value={this.draft} onInput={(e)=>{this.draft=(e.currentTarget as HTMLInputElement).value;if(this.sharing)this.typing.input(this.draft,this.channel);this.forceUpdate();}}/><button disabled={!this.draft.trim()||this.send.state.loading}>Send</button></form></section><aside><h2>What's happening</h2><label><input type="checkbox" role="switch" checked={this.sharing} onChange={(e)=>{const input=e.currentTarget as HTMLInputElement,enabled=input.checked;input.checked=this.sharing;void this.setSharing(enabled);}}/>Share my activity</label><p>Never shares drafts, messages, URLs, query strings, emails, passwords, message IDs, or keystrokes.</p><ul>{this.activities.map(a=><li key={a.sessionId}>{profiles.find((p:any)=>p.userId===a.userId)?.name??"A Musketeer"} is {a.status} #{a.metadata?.channel??"campfire"}</li>)}</ul><h3>Switch Musketeer</h3>{musketeers.map(p=><button onClick={()=>this.switchTo(p)}>{p.name}</button>)}</aside></main>}}
export function mountInfernoApp(target:Element){render(<App/>,target);} mountInfernoApp(document.getElementById("app")!);
`;
}
function litTemplateFiles(options: { name: any; template?: any }, files: Record<string, string>) {
  ...
  return {
    ...
    "client/index.ts": (apps[options.template] ?? litBlankTemplate)(),
...
function litPhotoLibraryTemplate() {
  return `import { LitElement, css, html } from "lit";
import { auth, createLitControllers, files } from "sporades/client";
import mark from "./sporades-mark.svg";
import "./styles.css";

const { authController, mutationController, queryController } = createLitControllers();
class SporadesApp extends LitElement {
  session = authController(this); publicPhotos = queryController<any[]>(this, "publicPhotos"); personalPhotos = queryController<any[]>(this, "personalPhotos");
  recordPhoto = mutationController(this, "recordPhoto"); updatePhotoIsPublic = mutationController(this, "updatePhotoIsPublic"); updatePhotoImageUrl = mutationController(this, "updatePhotoImageUrl"); updatePhotoPublicUrlId = mutationController(this, "updatePhotoPublicUrlId");
...
function litCampfireTemplate() {
  return `import { LitElement, css, html } from "lit";
import { auth, createLitControllers, journey, preferences } from "sporades/client";
...
class SporadesApp extends LitElement {
  session = authController(this); profiles = queryController<any[]>(this, "profiles");
  channelQueries = { general: queryController<any[]>(this, "messagesGeneral"), ideas: queryController<any[]>(this, "messagesIdeas"), random: queryController<any[]>(this, "messagesRandom"), "protect-the-crown": queryController<any[]>(this, "messagesProtectTheCrown") };
  sendMessage = mutationController(this, "sendMessage"); toggleReaction = mutationController(this, "toggleReaction"); seedCampfire = mutationController(this, "seedCampfire"); registerFixture = mutationController(this, "registerFixture");
  channel = "general"; draft = ""; notice = ""; sharing = false; activities: any[] = []; fixturesPrepared = false; fixturePreparationActive = false;
  generation = 0; owner: number | null = null; tail = Promise.resolve(); previousUserId: string | undefined; unsubscribe: (() => void) | null = null;
  typingPublisher = createTypingPublisher((state: any) => journey.set(state));
  static styles = css\`:host{display:block;background:#120d0a;color:#fff7ed;font-family:system-ui,sans-serif}.campfire{min-height:100vh;display:grid;grid-template-columns:240px minmax(0,1fr) 300px}.campfire>aside,.conversation>header,.conversation>form{padding:20px;background:#1b120d}.mark{width:2rem}nav,.campfire>aside{display:grid;align-content:start;gap:9px}button,input{border:1px solid #92400e;border-radius:7px;padding:9px 12px;background:#29211d;color:inherit}.active{background:#b45309}.conversation{display:flex;flex-direction:column}.messages{flex:1;padding:20px;display:grid;align-content:start;gap:14px}.messages article{padding:16px;border:1px solid #78350f;border-radius:9px}.conversation form{display:flex;gap:8px}.conversation input{flex:1}@media(max-width:900px){.campfire{grid-template-columns:1fr}}\`;
  connectedCallback() { super.connectedCallback(); if (!this.unsubscribe) this.unsubscribe = journey.subscribe((event: any) => { this.activities = this.apply(this.activities, event); this.requestUpdate(); }).unsubscribe; }
  disconnectedCallback() { super.disconnectedCallback(); this.unsubscribe?.(); this.unsubscribe = null; this.previousUserId = undefined; ++this.generation; void this.lane(() => this.retire()); }
  updated() { const userId = this.session.state.auth?.userId; if (userId !== this.previousUserId) { const old = this.previousUserId; this.previousUserId = userId; void this.authChanged(userId, old); } const profiles = this.profiles.state.data; if (!this.fixturesPrepared && Array.isArray(profiles)) { this.fixturesPrepared = true; if (this.isLocalDemoOrigin()) void this.prepareFixtures(musketeers.filter((person) => !new Set(profiles.map((profile: any) => profile.key)).has(person.key))); } }
  current(value: number, userId: string) { return value === this.generation && this.session.state.auth?.userId === userId; }
  ...
  compose() { if (this.sharing) this.typingPublisher.input(this.draft, this.channel); }
  async submit(event: SubmitEvent) { event.preventDefault(); const result = await this.sendMessage.run({ channel: this.channel, body: this.draft }); if (result.error) this.notice = result.error.message; else { this.draft = ""; if (this.sharing) { this.typingPublisher.dispose(); await journey.set({ status: "posted", metadata: { channel: this.channel }, ttlSeconds: 8 }); } } this.requestUpdate(); }
  async react(messageId: string, kind: string) { const result = await this.toggleReaction.run({ messageId, kind }); if (result.error) this.notice = result.error.message; else if (this.sharing) await journey.set({ status: kind === "up" ? "liked" : "disliked", metadata: { channel: this.channel }, ttlSeconds: 8 }); this.requestUpdate(); }
  async switchTo(person: any) { ++this.generation; await this.lane(() => this.retire()); await auth.signOut(); const result = await auth.signIn("email", { email: person.email, password }); this.notice = result.error ? result.error.message : "Signed in as " + person.name + ". Restoring activity preference…"; this.requestUpdate(); }
  apply(current: any[], event: any) { const data = event?.data ?? event; if (data?.type === "snapshot") return data.states ?? []; if (data?.type === "removed" || data?.type === "expired") return current.filter((item) => item.sessionId !== data.state?.sessionId); return data?.state ? [...current.filter((item) => item.sessionId !== data.state.sessionId), data.state] : current; }
  activityText(item: any) { const name = (this.profiles.state.data ?? []).find((profile: any) => profile.userId === item.userId)?.name ?? "A Musketeer", place = item.metadata?.channel ?? "campfire"; return item.status === "posted" ? name + " posted a message in #" + place : item.status === "liked" ? name + " liked a message in #" + place : item.status === "disliked" ? name + " disliked a message in #" + place : name + " is " + item.status + " #" + place; }
  count(message: any, kind: string) { return Object.keys(message.reactions ?? {}).filter((key) => key.endsWith(":" + kind)).length; }
  render() { const messages = this.channelQueries[this.channel as keyof typeof this.channelQueries].state.data ?? []; return html\`<main class="campfire"><aside><img class="mark" src=\${mark} alt=""><h1>🔥 Campfire</h1><nav>\${channels.map((slug) => html\`<button class=\${this.channel === slug ? "active" : ""} @click=\${() => this.choose(slug)}># \${slug}</button>\`)}</nav><p>Development-only Musketeer identities.</p></aside><section class="conversation"><header><h2># \${this.channel}</h2><p role="status">\${this.notice}</p></header><div class="messages">\${messages.map((message) => html\`<article><strong>\${message.authorName}</strong><p>\${message.body}</p><button @click=\${() => this.react(message.id, "up")}>👍 \${this.count(message, "up")}</button><button @click=\${() => this.react(message.id, "down")}>👎 \${this.count(message, "down")}</button></article>\`)}</div><form @submit=\${(event: SubmitEvent) => this.submit(event)}><input id="message" .value=\${this.draft} @input=\${(event: InputEvent) => { this.draft = (event.currentTarget as HTMLInputElement).value; this.compose(); this.requestUpdate(); }}><button ?disabled=\${!this.draft.trim() || this.sendMessage.state.loading}>Send</button></form></section><aside><h2>What's happening</h2><label><input type="checkbox" .checked=\${this.sharing} @change=\${(event: Event) => { const input = event.currentTarget as HTMLInputElement, enabled = input.checked; input.checked = this.sharing; void this.setShare(enabled); }}>Share my activity</label><p>Never shares drafts, messages, URLs, query strings, emails, passwords, message IDs, or keystrokes.</p><ul>\${this.activities.map((item) => html\`<li>\${this.activityText(item)}</li>\`)}</ul><h3>Switch Musketeer</h3>\${musketeers.map((person) => html\`<button @click=\${() => this.switchTo(person)}>\${person.name}</button>\`)}</aside></main>\`; }
}
customElements.define("sporades-app", SporadesApp);
`;
}
...
function solidStyles(template: string) {
...
function vanillaTemplateFiles(options: { name: any; framework: any; template?: any; }) {
...
function blankPaymentReadme(name: string, introduction: string) {
...
function blankPaymentSupportFiles(capsuleName: string) {
  return {
...
import type { PaymentCheckoutResult, PaymentJobState } from "../shared/payments.js";
...
export async function startStripeCheckout(input: CheckoutInput, onProgress: (progress: CheckoutProgress) => void): Promise<Subscription> {
  const started = await mutations.run<{ intentId: string; jobId: string }>("startStripeCheckout", input);
  if (started.error || !started.data) {
    onProgress({ status: "failed", intentId: input.intentId, jobId: null, error: started.error ?? { message: "Checkout did not start." } });
    return { unsubscribe() {} };
  }
  ...
  return queries.subscribe<PaymentJobState>("paymentJob", (state) => {
    ...
    if (!job) return fail({ message: "Checkout progress is unavailable." });
    if (job.status === "failed" || job.status === "cancelled") return fail(job.failure ?? { message: "Checkout did not complete." });
    if (job.status !== "succeeded") return;
    const result = validCheckoutResult(job.result);
    if (!result) return fail({ message: "Checkout returned an invalid redirect." });
    settled = true;
...
export default capsule({
  ...
  mutations: {
    seedCampfire: mutation(async (ctx) => {
      ...
      await ensure("message", "crown-prompt", async () => (await ctx.db.messages.where("seedKey", "crown-prompt").all()).length > 0, () => ctx.db.messages.insert({ channel: "protect-the-crown", body: "Is the crown adequately protected? 👍 All for one · 👎 One more guard, perhaps", authorId: "fixture:dartagnan", authorName: fixtureNames.dartagnan, seedKey: "crown-prompt", reactions: {} }));
...
${template === "blank" ? "- `server/payments.ts` - payment mutations and Jobs, known-Job query, policy seam, and server-owned Price catalogue\n- `client/payments.ts` - Checkout progress and validated redirect helper\n- `shared/payments.ts` - serializable payment Job state\n" : ""}
