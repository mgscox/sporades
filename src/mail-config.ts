...
export function validateMailConfig(mail: Record<string, any> | undefined) {
  ...
  if (
    ...
    && (
      ...
      || servername.split(".").some((label: string) => label.length < 1 || label.length > 63 || label.startsWith("-") || label.endsWith("-"))
    )
  ) {
  ...
  if (authMethod === "none") {
    ...
    if (
      !["PLAIN", "LOGIN"].includes(authMethod)
  ...
  const tls: Record<string, any> = { mode: tlsMode };
  optional(tls, "rejectUnauthorized", rejectUnauthorized);
  optional(tls, "servername", servername);
  const smtp: Record<string, any> = { vendor, host, port, tls, auth };
