export function validateMailConfig(mail: Record<string, any> | undefined) {
  ...
  const onlyKeys = (value: Record<string, any>, allowed: string[]) => Object.keys(value).every((key) => allowed.includes(key));
  ...
  if (
    ...
    && (
      ...
      || smtp.tls.servername.split(".").some((label: string) => label.length < 1 || label.length > 63 || label.startsWith("-") || label.endsWith("-"))
    )
  ) {
  ...
  if (smtp.auth.method === "none") {
    ...
    if (
      !["PLAIN", "LOGIN"].includes(smtp.auth.method)
  ...
  const validateTimeout = (name: string, value: unknown, maximum: number, label: string) => {
    if (value !== undefined && (!Number.isInteger(value) || (value as number) < 100 || (value as number) > maximum)) {
