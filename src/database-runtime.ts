...
function assertAuthUserProvider(row: LooseRecord) {
  if (Number(row.isAuthenticated) === 1 &&
      (typeof row.provider !== "string" || !row.provider.trim() || ["anonymous", "guest"].includes(row.provider))) {
    throw commandError("Authenticated users require a non-anonymous provider label.",
      "Supply the authentication method used; historical missing evidence is labelled unknown.", "INVALID_AUTH_USER_PROVIDER");
  }
}
...
export function createSharedDatabaseAdapterMethods(dialect: LooseRecord): LooseRecord {
  ...
  return {
    ...
    pruneDeliveredIngressClaimAudits(deliveredBefore: string, limit: number) {
    ...
    issueAccessKeyRecord(row: LooseRecord) {
      ...
      const sequence = chainMaybePromise([
        ...
        ).run(row.ownerUserId, ACCESS_KEY_CURRENT_LIMIT, ACCESS_KEY_RETAINED_LIMIT), (result: LooseRecord) => {
        ...
        () => !reserved || outcome?.status === "issued" ? undefined : this.prepare(
          sql(
            "UPDATE [sporades_auth_access_key_owners] " +
            "SET [currentCount] = [currentCount] - 1, [totalCount] = [totalCount] - 1, " +
    ...
    listAccessKeyRecordsForOwner(ownerUserId: string, options: LooseRecord = {}) {
      ...
      const sequence = chainMaybePromise([
        ...
        () => !sessionEligible ? undefined : thenIfPromise(this.prepare(
          sql(
        ...
        ).all(ownerUserId), (result: LooseRecord[]) => { rows = result; }),
      ]);
    ...
    updateAuthUserProfile(row: { displayName: any; picture: any; isAuthenticated: any; isGuest: any; provider: any; id: any; }) {
      assertNotReservedAuthUserId(row.id);
      assertAuthUserProvider(row);
      return this.prepare(
        sql(
    ...
    [resourceTransactionMechanics]: async function(fn: (transactionAdapter: LooseRecord) => any, beforeCommit?: (adapter: LooseRecord) => void) {
      ...
      return connectionGate.runTransaction(async () => {
        ...
        try {
          ...
          try { dedicated.exec("BEGIN IMMEDIATE"); }
          catch (error: any) {
          ...
          revokeTransactionScopedAdapter(transaction);
          if (begun) { try { dedicated.exec("ROLLBACK"); } catch {} }
...
export async function createPostgresDatabaseAdapter(options: { url: any; }) {
  ...
  const resourceSchemaReady = async (query: (sql: string, params?: any[]) => Promise<any>) => {
    for (const schema of resourceSchemas) {
      ...
      if (primaryKey.length !== schema.primaryKey.length || primaryKey.some((row: any, index: number) =>
        row.column_name !== schema.primaryKey[index]
...
export async function createPostgresConnection(url: any, signal?: AbortSignal) {
  ...
  function executeQueuedPostgresQuery(sql: any, generation: number) {
