...
class Quarantine extends Error {
  ...
  constructor(reason: "catalogue-mismatch" | "provider-state-ambiguous", teamId: string | null = null, rank = 50, objectId: string | null = null) {
    super(reason);
    this.teamId = teamId;
    this.reason = reason;
    this.rank = rank;
...
export async function applyVerifiedTeamBillingObservation(database: LooseRecord, event: any) {
...
function validateEnvelope(database: LooseRecord, event: any) {
  const raw = event?.raw;
...
async function applyCheckout(database: LooseRecord, event: LooseRecord, mode: string) {
  ...
  if (object.object !== "checkout.session" || object.mode !== "subscription" || object.livemode !== event.livemode
    ...
    || object.metadata?.sporades_team_billing_operation !== operationId) throw new Quarantine("provider-state-ambiguous", null, 50, boundedObjectId(object.id));
  const operation = await tx.prepare(tx.dialect.sql(
    "SELECT [id], [teamId], [mode], [status], [providerObjectId], [providerCustomerId], [providerSubscriptionId], [terminalObservedAt] " +
  ...
  if (!operation.terminalObservedAt || nextRank > currentRank) {
    await tx.prepare(tx.dialect.sql(
      "UPDATE [sporades_team_billing_operations] SET [status] = ?, [providerObjectId] = ?, [providerCustomerId] = COALESCE(?, [providerCustomerId]), " +
      "[providerSubscriptionId] = COALESCE(?, [providerSubscriptionId]), [continuationUrl] = NULL, [continuationExpiresAt] = NULL, [terminalObservedAt] = ?, [updatedAt] = ? WHERE [id] = ?",
...
async function applySubscription(database: LooseRecord, event: LooseRecord, mode: string) {
  const tx = database.adapter;
  const object = event.raw.data.object;
  ...
  const existing = await tx.prepare(tx.dialect.sql(
    "SELECT [id], [teamId], [mode], [providerSubscriptionId], [terminalLatch], [currentPeriodStart], [lastEventOccurredAt], [lastEventRank] " +
  ...
  const operation = metadataOperationId && UUID.test(metadataOperationId)
    ? await tx.prepare(tx.dialect.sql(
      "SELECT [id], [teamId], [mode], [productKey], [quantity], [providerCustomerId], [providerSubscriptionId] " +
      "FROM [sporades_team_billing_operations] WHERE [id] = ? AND [kind] = 'checkout'",
  ...
  if (!teamId || (existing && operation && existing.teamId !== operation.teamId) || existing?.mode !== undefined && existing.mode !== mode
    || operation?.mode !== undefined && operation.mode !== mode) throw new Quarantine("provider-state-ambiguous", teamId, 50, subscriptionId);
  ...
  if (existing && !deleted && !winsRatchet(normalized.periodStart, event.occurredAt, normalized.rank,
    existing.currentPeriodStart, existing.lastEventOccurredAt, Number(existing.lastEventRank ?? 0))) {
    return { teamId, objectId: subscriptionId, rank: normalized.rank, outcome: "ignored" };
  }
  await bindCustomer(tx, teamId, mode, customerId, event.occurredAt);
  if (!existing) {
    await tx.prepare(tx.dialect.sql(
      "INSERT INTO [sporades_team_billing_subscriptions] ([id], [teamId], [mode], [providerSubscriptionId], [providerPriceId], [providerSubscriptionItemId], [productKey], [quantity], [state], [cancelAtPeriodEnd], [currentPeriodStart], [currentPeriodEnd], [observedAt], [updatedAt], [lastEventOccurredAt], [lastEventKind], [lastEventRank], [terminalLatch]) " +
      "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    )).run(randomUUID(), teamId, mode, subscriptionId, normalized.priceId, normalized.itemId, normalized.productKey, normalized.quantity,
      normalized.state, normalized.cancelAtPeriodEnd, normalized.periodStart, normalized.periodEnd, event.occurredAt, event.occurredAt,
      event.occurredAt, normalized.kind, normalized.rank, deleted ? 1 : 0);
  } else {
    await tx.prepare(tx.dialect.sql(
      "UPDATE [sporades_team_billing_subscriptions] SET [providerPriceId] = ?, [providerSubscriptionItemId] = ?, [productKey] = ?, [quantity] = ?, [state] = ?, " +
      "[cancelAtPeriodEnd] = ?, [currentPeriodStart] = ?, [currentPeriodEnd] = ?, [observedAt] = ?, [updatedAt] = ?, [lastEventOccurredAt] = ?, [lastEventKind] = ?, [lastEventRank] = ?, [terminalLatch] = ? WHERE [providerSubscriptionId] = ?",
    )).run(normalized.priceId, normalized.itemId, normalized.productKey, normalized.quantity, normalized.state,
      normalized.cancelAtPeriodEnd, normalized.periodStart, normalized.periodEnd, event.occurredAt, event.occurredAt,
      event.occurredAt, normalized.kind, normalized.rank, deleted ? 1 : 0, subscriptionId);
  }
  if (operation) {
    await tx.prepare(tx.dialect.sql(
  ...
  await settleVerifiedTeamBillingTarget(
    { ...database, adapter: tx, __transactionActive: true, __rootDatabase: database.__rootDatabase ?? database },
    { teamId, productKey: normalized.productKey, quantity: normalized.quantity, subscriptionId, occurredAt: event.occurredAt },
  );
...
function normalizeSubscription(definition: LooseRecord, object: LooseRecord, operation: LooseRecord | null, mode: string, deleted: boolean, teamId: string, desiredTeamQuantity: number | null) {
  ...
  const matches = Object.entries(definition.catalogue).filter(([, product]: any) => {
  ...
  const [productKey, product]: any = matches[0];
...
async function inferTeamId(tx: LooseRecord, event: any) {
  const object = event?.raw?.data?.object;
...
async function assertCustomerAssociation(tx: LooseRecord, teamId: string, mode: string, customerId: string) {
  ...
  if ((byTeam && (byTeam.mode !== mode || byTeam.providerCustomerId !== customerId))
    || (byProvider && (byProvider.teamId !== teamId || byProvider.mode !== mode))) throw new Quarantine("provider-state-ambiguous", teamId);
}

async function bindCustomer(tx: LooseRecord, teamId: string, mode: string, customerId: string, now: string) {
  await assertCustomerAssociation(tx, teamId, mode, customerId);
  await tx.prepare(tx.dialect.sql(
    "INSERT INTO [sporades_team_billing_customers] ([teamId], [mode], [providerCustomerId], [createdAt], [updatedAt]) VALUES (?, ?, ?, ?, ?) " +
    "ON CONFLICT ([teamId]) DO UPDATE SET [updatedAt] = excluded.[updatedAt]",
  )).run(teamId, mode, customerId, now, now);
}

async function recordObservation(database: LooseRecord, event: LooseRecord, digest: string, teamId: string | null, objectId: string | null, rank: number, outcome: string, safeReason: string | null) {
  ...
  )).run(randomUUID(), teamId, event.livemode ? "live" : "sandbox", event.providerEventId, objectId, digest,
    event.occurredAt, database.clock?.now?.().toISOString?.() ?? event.occurredAt, event.type, rank, outcome, safeReason);
}

function winsRatchet(periodStart: string, occurredAt: string, rank: number, previousPeriodStart: any, previousAt: any, previousRank: number) {
...
function unixTimestamp(value: any) {
...
function boundedObjectId(value: any) {
  return typeof value === "string" && /^[A-Za-z][A-Za-z0-9_]{1,240}$/.test(value) ? value : null;
}

function safeDigest(raw: any) {
  try { return createHash("sha256").update(JSON.stringify(raw)).digest("hex"); } catch { return null; }
}
