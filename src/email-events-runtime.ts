...
function parsedJsonObject(value: unknown) {
...
function verifiedMailgunRequest(ctx: LooseRecord, secret: string) {
...
function normalizeMailgunWebhook(raw: unknown) {
...
function parseMailgunEvents(body: unknown) {
...
function createProviderEmailEventEndpoint(
  ...
  rejectionStatus = { unverified: 401, malformed: 400 },
) {
  return {
    name,
