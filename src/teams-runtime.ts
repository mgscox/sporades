...
export function createTeamTables(adapter: LooseRecord) {
  ...
  return chainMaybePromise([
    ...
    () => adapter.exec(sql(
      "CREATE TABLE IF NOT EXISTS [sporades_team_join_link_counters] (" +
      "[teamId] TEXT PRIMARY KEY, [activeCount] INTEGER NOT NULL" +
      ")",
    )),
    () => adapter.exec(sql(
      "CREATE TABLE IF NOT EXISTS [sporades_team_join_link_redemptions] (" +
...
export async function validateTeamJoinLink(database: LooseRecord, auth: LooseRecord, code: any) {
  ...
  const expectedSignature = Buffer.from(
    row && secretRow
      ...
      : teamJoinSignature("absent", "absent", parsed.selector, parsed.verifier, "absent"),
    "base64url",
  );
  const actualSignature = Buffer.from(parsed.signature, "base64url");
  const verifierMatches = actualVerifier.length === expectedVerifier.length && timingSafeEqual(actualVerifier, expectedVerifier);
  ...
  const valid = attachedEmails.some((identity: LooseRecord) => normalizeTeamJoinIdentityEmail(identity.email) === targetEmail);
  return { valid };
}
...
function normalizeTeamJoinIdentityEmail(email: any) {
...
async function countTeamAdmins(tx: LooseRecord, teamId: string) {
...
async function pruneExpiredTeamJoinLinks(tx: LooseRecord, now: string) {
  ...
  for (const row of rows) {
    const deleted = await deleteExpiredTeamJoinLink(tx, String(row.id), String((row as LooseRecord).teamId), now);
    if (Number(deleted?.changes ?? 0) === 1) await releaseTeamJoinLinkCapacity(tx, String((row as LooseRecord).teamId));
  }
}
...
async function claimTeamJoinLinkCapacity(tx: LooseRecord, teamId: string, now: string) {
  await tx.prepare(tx.dialect.sql(
...
async function reconcileTeamJoinLinkCapacity(tx: LooseRecord, teamId: string, now: string) {
...
async function deleteExpiredTeamJoinLink(tx: LooseRecord, joinLinkId: string, teamId: string, now: string) {
  await tx.prepare(tx.dialect.sql(
...
async function releaseTeamJoinLinkCapacity(tx: LooseRecord, teamId: string) {
  await tx.prepare(tx.dialect.sql(
    "UPDATE [sporades_team_join_link_counters] SET [activeCount] = [activeCount] - 1 WHERE [teamId] = ? AND [activeCount] > 0",
  )).run(teamId);
}
 ...
 */
export async function promoteTeamMember(database: LooseRecord, auth: LooseRecord, teamId: any, userId: any, eventContext?: LooseRecord) {
  return changeTeamMemberRole(database, auth, teamId, userId, "admin", eventContext);
}

export async function demoteTeamMember(database: LooseRecord, auth: LooseRecord, teamId: any, userId: any, eventContext?: LooseRecord) {
  return changeTeamMemberRole(database, auth, teamId, userId, "member", eventContext);
}

async function changeTeamMemberRole(database: LooseRecord, auth: LooseRecord, teamId: any, userId: any, role: "admin" | "member", eventContext?: LooseRecord) {
  requireAuth({ auth }, { linked: true });
...
export async function removeTeamMember(database: LooseRecord, auth: LooseRecord, teamId: any, userId: any, eventContext?: LooseRecord) {
  requireAuth({ auth }, { linked: true });
  try {
...
export async function leaveCurrentUserTeam(database: LooseRecord, auth: LooseRecord, teamId: any, eventContext?: LooseRecord) {
  requireAuth({ auth }, { linked: true });
  try {
    if (!isOpaqueTeamId(teamId)) throw teamDenied();
    await withTeamTransaction(database, async (tx) => {
...
export async function deleteCurrentUserTeam(database: LooseRecord, auth: LooseRecord, teamId: any, eventContext?: LooseRecord) {
  requireAuth({ auth }, { linked: true });
  try {
    if (!isOpaqueTeamId(teamId)) throw teamDenied();
    await withTeamTransaction(database, async (tx) => {
...
function teamSecurityOperation(event: string) {
...
function teamSecurityMessage(event: string, outcome: "succeeded" | "denied") {
...
export function flushTeamSecurityEvents(database: LooseRecord, context: LooseRecord | undefined, options: LooseRecord = {}) {
