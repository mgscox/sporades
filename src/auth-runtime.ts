...
function sessionFromRow(row: { token: any; userId: any; userKind?: any; displayName: any; email: any; picture: any; isAuthenticated: any; isGuest: any; provider: any; }) {
  return {
    token: row.token,
    auth: {
      userId: row.userId,
...
export function createAnonymousAuthTables(sqlite: LooseRecord, _authConfig: LooseRecord | null = null) {
  ...
  return chainMaybePromise([
    ...
    () => sqlite.exec(sql(
      "CREATE TABLE IF NOT EXISTS [sporades_auth_service_user_locks] " +
...
function ensureAuthUserKindColumns(sqlite: LooseRecord) {
  const sql = sqlite.dialect.sql;
  return chainMaybePromise([
