...
// of a builtin would be lowered to `__require(…)` in the carried IIFE (ADR-0041), which is why the
// four crypto calls in `beginOAuthSignIn` take `nodeCryptoModule` instead.
...
export function authIdentityRowsUnlessReserved(rowsOrPromise: any) {
...
export function assertNotReservedAuthUserId(userId: any) {
...
export function readEndpointSessionToken(headers: { [x: string]: any; }, query: { [x: string]: any; sessionToken?: any; }) {
...
export function emitAuthDeniedLog(database: LooseRecord, details: LooseRecord) {
  database.log?.emit?.({
    category: "platform",
    event: "auth.denied",
    level: "warn",
...
export function normalizeSimulatedText(value: null | undefined) {
...
export function parseOAuthFormBody(body: Buffer) {
...
export function validateConsumedOAuthCallbackParameters(parameters: URLSearchParams) {
...
export function normalizeReturnTo(returnTo: string | URL, origin: string | URL | undefined) {
  if (!returnTo) {
    return origin;
  }
  try {
...
export function oauthProviderAdapter(database: LooseRecord, provider: string) {
...
export function oauthProviderTestEndpoint(override: any, productionUrl: string) {
...
export async function fetchBoundedOAuthJson(
...
export async function completeOpenIdOAuthCodeExchange(database: LooseRecord, context: LooseRecord, contract: LooseRecord) {
...
export function appleOAuthOriginEligible(origin: any) {
  try {
...
export function createAppleClientSecret(database: LooseRecord, nowSeconds = Math.floor(Date.now() / 1000)) {
  const apple = database.authConfig.providers.apple;
...
export async function verifyGoogleIdentityToken(database: LooseRecord, token: string, expectedNonce: string) {
...
export async function discoverMicrosoftOpenIdConfiguration(database: LooseRecord, tenant: string) {
...
export async function fetchMicrosoftOidcJson(
  database: LooseRecord,
  url: string,
  request: LooseRecord,
  policy: LooseRecord,
...
export async function completeMicrosoftOAuth(database: LooseRecord, context: LooseRecord) {
  const microsoft = database.authConfig.providers.microsoft;
  const discovery = await discoverMicrosoftOpenIdConfiguration(database, microsoft.tenant);
  const clientId = database.serverEnv[microsoft.clientIdEnv];
...
export async function verifyAppleIdentityToken(database: LooseRecord, token: string, expectedNonce: string) {
  if (typeof token !== "string" || token.length > 16 * 1024) {
...
export async function loadMicrosoftJwks(
  database: LooseRecord,
  discovery: LooseRecord,
  forceRefresh = false,
...
export function writeRedirect(response: { writeHead: (arg0: number, arg1: { location: any; }) => void; end: () => void; }, location: any) {
...
export function normalizePasswordResetPath(value: any) {
...
export function hashPasswordResetVerifier(verifier: string) {
...
export function serverAuthError(error: LooseRecord | undefined, fallback: string) {
...
export async function confirmPasswordReset(database: LooseRecord, _session: LooseRecord, code: any, newPassword: string) {
  ...
  // may continue.
  const outcome = await database.adapter.withTransaction(async (tx: LooseRecord) => {
...
export function currentEmailSignInThrottleState(database: LooseRecord, email: string, session: LooseRecord, scope = EMAIL_SIGN_IN_THROTTLE_FIELD) {
...
export function recordFailedEmailSignInAttempt(database: LooseRecord, email: string, session: LooseRecord, scope = EMAIL_SIGN_IN_THROTTLE_FIELD) {
  const attempts = createEmailSignInThrottleState(database, scope);
...
export function resetEmailSignInAttempts(database: LooseRecord, email: string, session: LooseRecord, scope = EMAIL_SIGN_IN_THROTTLE_FIELD) {
  const attempts = createEmailSignInThrottleState(database, scope);
...
export function invalidEmailCredentialsError(options: LooseRecord = {}) {
  return {
...
export function normalizeEmailCredentials(credentials: { email: any; password: any; name: null; }) {
...
export function hashEmailPassword(password: BinaryLike) {
...
export function verifyEmailPassword(password: BinaryLike, salt: BinaryLike, expectedHash: WithImplicitCoercion<string>) {
...
export function authStatus(config: LooseRecord, serverEnv: LooseRecord) {
...
export function authProvidersForClient(authConfig: LooseRecord, origin: any = null) {
  const providers: LooseRecord = {};
...
export async function routeSporadesAuth(database: LooseRecord, request: IncomingMessage, response: ServerResponse<IncomingMessage> & { req: IncomingMessage; }) {
  ...
  try {
    ...
    if (stateRow.reauthPurpose) {
      const subject = normalizeSimulatedText(profile?.subject ?? profile?.sub); const identity = subject ? await database.adapter.findAuthIdentityByProviderSubject(provider, subject) : null; const policy = database.reauthenticationPolicy?.[stateRow.reauthPurpose];
      ...
      const now = database.clock.now(); await database.adapter.withTransaction((tx: LooseRecord) => tx.replaceReauthenticationProof({ id: nodeCryptoModule.randomUUID(), userId: session.auth.userId, sessionToken: stateRow.sessionToken, purpose: stateRow.reauthPurpose, createdAt: now.toISOString(), expiresAt: new Date(now.getTime() + policy.maxAgeSeconds * 1000).toISOString() })); writeRedirect(response, stateRow.returnTo); return true;
    }
    let result;
    try {
...
export async function beginOAuthSignIn(database: LooseRecord, session: LooseRecord, provider: string, options: LooseRecord) {
  const adapter = oauthProviderAdapter(database, provider);
  if (!adapter?.enabled) {
    return {
      ok: false,
...
const oauthRegistrationRetentionKey = (keyId: string) => `oauth-registration-key:retain:${keyId}`;
...
function oauthRegistrationInstant(value: unknown) {
  if (typeof value !== "string") return null;
  const instant = Date.parse(value);
  return Number.isFinite(instant) && new Date(instant).toISOString() === value ? instant : null;
}

async function oauthRegistrationOutstandingExpiry(adapter: LooseRecord, keyId: string, now: number) {
  ...
  return rows.reduce((latest: number, row: LooseRecord) => {
...
function oauthRegistrationAlias(value: unknown, now: number) {
  try {
...
async function deleteOAuthRegistrationMetadata(adapter: LooseRecord, key: string) {
  return await adapter.prepare(adapter.dialect.sql("DELETE FROM [sporades] WHERE [key] = ?")).run(key);
}

async function reconcileOAuthRegistrationKeysOnAdapter(adapter: LooseRecord, now: number) {
...
export async function reconcileOAuthRegistrationKeys(database: LooseRecord, options: LooseRecord = {}) {
  const now = Number.isFinite(options.now) ? Number(options.now) : Date.now();
  return await withAuthTransaction(database, async (adapter: LooseRecord) => reconcileOAuthRegistrationKeysOnAdapter(adapter, now));
}
...
export async function rotateOAuthRegistrationKey(database: LooseRecord, options: LooseRecord = {}) {
  const now = Number.isFinite(options.now) ? Number(options.now) : Date.now();
  return await withAuthTransaction(database, async (adapter: LooseRecord) => {
    ...
    const swapped = await adapter.prepare(adapter.dialect.sql(
      "UPDATE [sporades] SET [value] = ? WHERE [key] = ? AND [value] = ?",
    )).run(nextId, OAUTH_REGISTRATION_ACTIVE_KEY, current.keyId);
    const converged = await adapter.readSystemMetadata(OAUTH_REGISTRATION_ACTIVE_KEY);
    if (converged?.value !== nextId) {
      await deleteOAuthRegistrationMetadata(adapter, oauthRegistrationMaterialKey(nextId));
      if (!oauthRegistrationKeyId(converged?.value)) throw new Error("OAuth registration key rotation did not converge.");
...
export async function retireOAuthRegistrationKeys(database: LooseRecord, options: LooseRecord = {}) {
  const now = Number.isFinite(options.now) ? Number(options.now) : Date.now();
  return await withAuthTransaction(database, async (adapter: LooseRecord) => {
    const current = await reconcileOAuthRegistrationKeysOnAdapter(adapter, now);
    const rows = await adapter.prepare(adapter.dialect.sql(
    ...
    for (const row of rows) {
      ...
      try { retainUntil = oauthRegistrationInstant(JSON.parse(String(row.value))?.expiresAt); } catch { retainUntil = null; }
      const outstanding = await oauthRegistrationOutstandingExpiry(adapter, keyId, now);
      if (retainUntil === null || retainUntil > now || outstanding > now) continue;
    ...
    if (aliasRow && !oauthRegistrationAlias(aliasRow.value, now)) await deleteOAuthRegistrationMetadata(adapter, OAUTH_REGISTRATION_ALIAS_KEY);
...
function canonicalOAuthRegistrationBase64url(value: unknown, byteLength?: number) {
  if (typeof value !== "string" || !/^[A-Za-z0-9_-]+$/.test(value)) return null;
  try { const bytes = Buffer.from(value, "base64url"); return bytes.length > 0 && (byteLength === undefined || bytes.length === byteLength) && bytes.toString("base64url") === value ? bytes : null; }
  catch { return null; }
}
async function unsealOAuthRegistration(database: LooseRecord, row?: LooseRecord, transactionAdapter = database.adapter) {
  ...
  try { const segments = String(row.registrationCiphertext).split("."); if (segments.length !== 4) return oauthRegistrationUnsealFailed; const [keyId, ivText, tagText, ciphertext] = segments;
    const iv = canonicalOAuthRegistrationBase64url(ivText, 12); const tag = canonicalOAuthRegistrationBase64url(tagText, 16); const encrypted = canonicalOAuthRegistrationBase64url(ciphertext); if ((!oauthRegistrationKeyId(keyId) && keyId !== "active") || !iv || !tag || !encrypted) return oauthRegistrationUnsealFailed;
...
export function resolvePasswordResetConfig(config: LooseRecord) {
