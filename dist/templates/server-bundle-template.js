...
const MIGRATED_RUNTIME_MODULES = [
    ...
    { file: "maybe-promise.js", loaded: maybePromise },
    { file: "file-storage-runtime.js", loaded: fileStorageRuntime },
    ...
    { file: "http-runtime.js", loaded: httpRuntime },
    ...
    { file: "log-index-storage.js", loaded: logIndexStorage },
    ...
    { file: "database-runtime.js", loaded: databaseRuntime },
];
...
export const MIGRATED_MODULE_STORAGE_SIGNATURE_SKEW_PROBE = [
    ["bucket-head", {
...
export const MIGRATED_MODULE_STORAGE_PATH_SKEW_PROBE = [
...
export const MIGRATED_MODULE_STORAGE_ENGINE_SKEW_PROBE = [
...
export const MIGRATED_MODULE_MAYBE_PROMISE_SKEW_PROBE = [
    ["null", null],
    ["undefined", undefined],
    ["number", 7],
    ["plain-object", { a: 1 }],
...
export const MIGRATED_MODULE_LOG_POLICY_SKEW_PROBE = [
...
export const MIGRATED_MODULE_ROW_DECODING_SKEW_PROBE = [
...
export const MIGRATED_MODULE_HTTP_SECURITY_SKEW_PROBE = [
...
export const MIGRATED_MODULE_HTTP_PLUMBING_SKEW_PROBE = [
    ["default", null],
    ["number", 2048],
    ...
    const adapter = {
        ...
        prepare(sql) {
            return {
                ...
                get: (...params) => { record(sql, params); return null; },
                all: (...params) => { record(sql, params); return []; },
                run: (...params) => { record(sql, params); return { changes: 0, lastInsertRowid: 0 }; },
...
const MIGRATED_MODULE_DATABASE_METHOD_PROBE = [
...
export const MIGRATED_MODULE_DIALECT_SPEC_SKEW_PROBE = [
    ["complete", { name: "x", quoteIdentifier: (i) => `"${i}"`, columnType: () => "TEXT", upsertSql: () => "", listTables: () => [], describeColumns: () => [], addMissingColumn: () => undefined }],
    ...
    ["null-entry", { name: "x", quoteIdentifier: (i) => `"${i}"`, columnType: () => "TEXT", upsertSql: () => "", listTables: null, describeColumns: () => [], addMissingColumn: () => undefined }],
    ["empty", {}],
];
export const MIGRATED_MODULE_NORMALIZATION_SPEC_SKEW_PROBE = [
...
export const MIGRATED_MODULE_NORMALIZATION_VALUE_PROBE = [
    null,
    { type: "null" },
...
export const MIGRATED_MODULE_SQL_TEXT_PROBE = [
...
function describeMigratedModuleAnswers(module) {
    ...
    return [
        ...
        // a chained sequence.
        ...MIGRATED_MODULE_STORAGE_SIGNATURE_SKEW_PROBE.map(([label, request]) => JSON.stringify([label, probedAnswer(() => module.s3Signature(request))])),
        ...MIGRATED_MODULE_STORAGE_PATH_SKEW_PROBE.map(([name, args]) => JSON.stringify([name, args, probedAnswer(() => module[name](...args))])),
        ...
        ...MIGRATED_MODULE_STORAGE_ENGINE_SKEW_PROBE.map(([label, config]) => JSON.stringify([
            label,
            probedAnswer(() => migratedModuleStorageEngineShape(module.createS3CompatibleFileStorageAdapter(config))),
        ])),
        ...[{ storagePath: "/data/files" }, { storagePath: "" }, {}].map((config) => JSON.stringify([
            "createLocalFileStorageAdapter",
            config,
        ...
        ...MIGRATED_MODULE_MAYBE_PROMISE_SKEW_PROBE.map(([label, value]) => JSON.stringify([
            label,
        ...
        ...MIGRATED_MODULE_HTTP_SECURITY_SKEW_PROBE.map(([label, config, request]) => JSON.stringify([label, migratedModuleHttpSecurityAnswer(module, config, request)])),
        ...MIGRATED_MODULE_HTTP_PLUMBING_SKEW_PROBE.map(([label, source]) => JSON.stringify([label, probedAnswer(() => module.resolveHttpMaxBodyBytes(source))])),
        ...
        ].map(([label, html, token]) => JSON.stringify([label, probedAnswer(() => module.injectPageConnectionToken(html, token))])),
        ...
        // not record.
        ...["sqliteDatabaseDialect", "postgresDatabaseDialect"].map((name) => JSON.stringify([name, migratedModuleDialectAnswer(module, name)])),
        // The seam's refusals, with the hint they travel on.
        ...MIGRATED_MODULE_DIALECT_SPEC_SKEW_PROBE.map(([label, spec]) => JSON.stringify([label, dialectProbedAnswer(() => {
                const built = module.createDatabaseDialect(spec);
                return [built.name, typeof built.sql, built.sql("SELECT [a]")];
            })])),
        ...MIGRATED_MODULE_NORMALIZATION_SPEC_SKEW_PROBE.map(([label, spec]) => JSON.stringify([label, dialectProbedAnswer(() => {
        ...
        ...["sqliteRowNormalization", "postgresRowNormalization", "libsqlRowNormalization"].map((name) => JSON.stringify([name, probedAnswer(() => {
        ...
        ...MIGRATED_MODULE_SQL_TEXT_PROBE.map(([label, sql, params]) => JSON.stringify([
            label,
