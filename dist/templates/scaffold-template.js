...
export async function scaffoldFromDirectory(options) {
    ...
    if (templateFiles["sporades.json"]) {
        try {
            ...
            if (!toolchain && !options.framework && config.client?.toolchain)
    ...
    const frameworkDependencies = framework === "react" ? { react: "^19.0.0", "react-dom": "^19.0.0" } :
    ...
    const frameworkDevDependencies = framework === "react" ? { "@types/react": "^19.0.0", "@types/react-dom": "^19.0.0" } :
