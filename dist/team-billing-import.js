...
export async function importLegacyTeamBillingReplayGuard(adapter, input) {
    if (!input || typeof input !== "object" || Object.keys(input).sort().join(",") !== [
        ...
        || !canonicalTime(input.settledAt) || !canonicalTime(input.retainedUntil) || input.retainedUntil <= input.settledAt)
        throw invalid();
    await ensureImportStorage(adapter);
    return await adapter.withTransaction(async (transaction) => {
        await lockImportIdentities(transaction, [input.providerEventId]);
        const sql = transaction.dialect.sql;
        const observation = await transaction.prepare(sql("SELECT * FROM [sporades_team_billing_observations] WHERE [providerEventId] = ?")).get(input.providerEventId);
        const expected = {
        ...
        if (observation && !same(observation, expected))
            throw conflict();
        const replay = await transaction.prepare(sql("SELECT * FROM [sporades_team_billing_replay] WHERE [providerEventId] = ?")).get(input.providerEventId);
        ...
        if (!observation) {
            await transaction.prepare(sql("INSERT INTO [sporades_team_billing_observations] ([id], [teamId], [mode], [providerEventId], [providerObjectId], [payloadDigest], [observedAt], [createdAt], [eventType], [eventRank], [outcome], [safeReason]) VALUES (?, NULL, ?, ?, NULL, ?, ?, ?, ?, NULL, 'ignored', 'LEGACY_REPLAY_GUARD')")).run(stableId("legacy-replay", input.providerEventId), input.mode, input.providerEventId, input.providerEventDigest, input.settledAt, input.settledAt, input.providerEventType);
        }
        if (!replay) {
            await transaction.prepare(sql("INSERT INTO [sporades_team_billing_replay] ([providerEventId], [payloadDigest], [settledAt], [retainedUntil]) VALUES (?, ?, ?, ?)")).run(input.providerEventId, input.providerEventDigest, input.settledAt, input.retainedUntil);
        }
 ...
 */
export async function importLegacyTeamBillingEvidence(adapter, input) {
    ...
    return await adapter.withTransaction(async (transaction) => {
        ...
        const sql = transaction.dialect.sql;
        let changed = false;
        const customer = await transaction.prepare(sql("SELECT * FROM [sporades_team_billing_customers] WHERE [teamId] = ? OR [providerCustomerId] = ?")).all(evidence.teamId, evidence.providerCustomerId);
        if (customer.length > 1 || (customer[0] && !same(customer[0], {
            ...
            throw conflict();
        if (!customer[0]) {
            await transaction.prepare(sql("INSERT INTO [sporades_team_billing_customers] ([teamId], [mode], [providerCustomerId], [createdAt], [updatedAt]) VALUES (?, ?, ?, ?, ?)")).run(evidence.teamId, evidence.mode, evidence.providerCustomerId, evidence.providerObservedAt, evidence.providerObservedAt);
            changed = true;
        }
        const subscription = await transaction.prepare(sql("SELECT * FROM [sporades_team_billing_subscriptions] WHERE [teamId] = ? OR [providerSubscriptionId] = ?")).all(evidence.teamId, evidence.providerSubscriptionId);
        ...
        if (subscription.length > 1 || (subscription[0] && !same(subscription[0], expectedSubscription)))
            throw conflict();
        if (!subscription[0]) {
            await transaction.prepare(sql("INSERT INTO [sporades_team_billing_subscriptions] " +
                ...
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")).run(stableId("legacy-subscription", evidence.providerSubscriptionId), evidence.teamId, evidence.mode, evidence.providerSubscriptionId, evidence.providerPriceId, evidence.providerSubscriptionItemId, evidence.productKey, evidence.quantity, evidence.state, evidence.cancelAtPeriodEnd ? 1 : 0, evidence.currentPeriodStart, evidence.currentPeriodEnd, evidence.providerObservedAt, evidence.providerObservedAt, evidence.providerObservedAt, semantics.kind, semantics.rank, semantics.terminalLatch);
            changed = true;
        }
        const observation = await transaction.prepare(sql("SELECT * FROM [sporades_team_billing_observations] WHERE [providerEventId] = ?")).get(evidence.providerEventId);
        ...
        if (observation && !same(observation, expectedObservation))
            throw conflict();
        if (!observation) {
            await transaction.prepare(sql("INSERT INTO [sporades_team_billing_observations] ([id], [teamId], [mode], [providerEventId], [providerObjectId], [payloadDigest], [observedAt], [createdAt], [eventType], [eventRank], [outcome], [safeReason]) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'applied', NULL)")).run(stableId("legacy-observation", evidence.providerEventId), evidence.teamId, evidence.mode, evidence.providerEventId, evidence.providerSubscriptionId, evidence.providerEventDigest, evidence.providerObservedAt, evidence.providerObservedAt, evidence.providerEventType, semantics.rank);
            changed = true;
        }
        const replay = await transaction.prepare(sql("SELECT * FROM [sporades_team_billing_replay] WHERE [providerEventId] = ?")).get(evidence.providerEventId);
        if (replay && !same(replay, {
            providerEventId: evidence.providerEventId,
            payloadDigest: evidence.providerEventDigest,
            ...
            throw conflict();
        if (!replay) {
            await transaction.prepare(sql("INSERT INTO [sporades_team_billing_replay] ([providerEventId], [payloadDigest], [settledAt], [retainedUntil]) VALUES (?, ?, ?, ?)")).run(evidence.providerEventId, evidence.providerEventDigest, evidence.providerObservedAt, evidence.retainedUntil);
            changed = true;
        }
...
function validate(input) {
    if (!input || typeof input !== "object" || Object.keys(input).sort().join(",") !== [
        ...
        || input.retainedUntil <= input.providerObservedAt)
        throw invalid();
...
async function lockImportIdentities(transaction, identities) {
    if (transaction.dialect?.name !== "postgres")
        return;
...
async function ensureImportStorage(adapter) {
...
//# sourceMappingURL=team-billing-import.js.map