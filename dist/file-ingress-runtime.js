...
function skipPdfTrivia(bytes, offset, limit) {
    while (offset < limit) {
...
function pdfDictionary(bytes, offset, integerKeys) {
...
function pdfXrefSection(bytes, offset) {
    ...
    if (pdfKeywordAt(bytes, offset, "xref")) {
        ...
        while (cursor < bytes.length) {
            ...
            if (!Number.isSafeInteger(count) || count < 1 || entries + count > 1_000_000)
                return null;
            ...
            for (let index = 0; index < count; index += 1) {
                ...
                if (!entry)
                    return null;
                if (entry[0].includes("n"))
                    objectOffsets.add(Number(entry[0].slice(0, 10)));
    ...
    if (!dictionary || !dictionary.values.has("Length"))
        return null;
...
function pdfRevisionObjectsOnly(bytes, start, end, referencedOffsets) {
    let cursor = skipPdfTrivia(bytes, start, end);
    if (cursor < 0)
        return false;
    while (cursor < end) {
        ...
        if (!foundEnd)
            return false;
        cursor = skipPdfTrivia(bytes, cursor, end);
        if (cursor < 0)
            return false;
    }
    return cursor === end;
