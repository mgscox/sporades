...
async function refreshReceiptInspection(database, row, inspection) {
    ...
    if (!current || !sameIngressRetryDescriptor(current, row) || current.leaseId !== row.leaseId || !["leased", "complete"].includes(current.state))
        throw idempotencyConflict("Ingress receipt changed while inspection evidence was refreshed.");
    return current;
}
...
function skipPdfTrivia(bytes, offset, limit, allowBinaryComments = false, deadlineExpired) {
    if (deadlineExpired?.("trivia"))
        return -1;
    let steps = 0;
    while (offset < limit) {
        while (offset < limit && pdfWhitespace(bytes[offset])) {
            if ((steps++ & 255) === 0 && deadlineExpired?.("trivia"))
                return -1;
            offset += 1;
        }
...
function pdfObjectEnd(bytes, offset, limit, deadlineExpired) {
    if (deadlineExpired?.("object"))
        return -1;
    offset = skipPdfTrivia(bytes, offset, limit, false, deadlineExpired);
    if (offset < 0 || offset >= limit)
        return -1;
    if (bytes[offset] === 0x2f) {
        let steps = 0;
        offset += 1;
        while (offset < limit && !pdfDelimiter(bytes[offset])) {
            if ((steps++ & 255) === 0 && deadlineExpired?.("object"))
                return -1;
            offset += 1;
        }
        return offset;
    ...
    if (bytes[offset] === 0x3c && bytes[offset + 1] !== 0x3c) {
        let steps = 0;
        offset += 1;
        while (offset < limit && bytes[offset] !== 0x3e) {
            if ((steps++ & 255) === 0 && deadlineExpired?.("object"))
                return -1;
            offset += 1;
        }
        return offset < limit ? offset + 1 : -1;
    }
    if (bytes[offset] === 0x5b || (bytes[offset] === 0x3c && bytes[offset + 1] === 0x3c)) {
        const stack = [bytes[offset] === 0x5b ? 0x5d : 0x3e];
        offset += bytes[offset] === 0x5b ? 1 : 2;
        let steps = 0;
        while (offset < limit && stack.length && ++steps <= 2_000_000) {
            ...
            if (bytes[offset] === 0x28) {
                const end = pdfObjectEnd(bytes, offset, limit, deadlineExpired);
                if (end < 0)
                    return -1;
                offset = end;
                continue;
            ...
            if (bytes[offset] === stack.at(-1) && (stack.at(-1) !== 0x3e || bytes[offset + 1] === 0x3e)) {
                offset += stack.pop() === 0x3e ? 2 : 1;
                continue;
            }
            offset += 1;
        }
    ...
    while (end < limit && !pdfDelimiter(bytes[end])) {
        if ((tokenSteps++ & 255) === 0 && deadlineExpired?.("object-token"))
            return -1;
        const byte = bytes[end];
        if (byte >= 0x30 && byte <= 0x39)
            digit = true;
        else if (byte === 0x2e && !dot)
            dot = true;
        else
            numeric = false;
        end += 1;
    ...
    if (end === offset)
        return -1;
    if (numeric && digit) {
        if (deadlineExpired?.("object-reference"))
            return -1;
        const after = skipPdfTrivia(bytes, end, limit, false, deadlineExpired);
        if (after < 0)
            return -1;
        let referenceAt = after;
        let generationDigits = 0;
        while (referenceAt < limit && bytes[referenceAt] >= 0x30 && bytes[referenceAt] <= 0x39 && generationDigits <= 10) {
            generationDigits += 1;
            referenceAt += 1;
        }
        if (generationDigits >= 1 && generationDigits <= 10) {
            const whitespaceAt = referenceAt;
            let whitespaceSteps = 0;
            while (referenceAt < limit && pdfWhitespace(bytes[referenceAt])) {
                if ((whitespaceSteps++ & 255) === 0 && deadlineExpired?.("object-reference"))
                    return -1;
                referenceAt += 1;
            }
            if (referenceAt > whitespaceAt && bytes[referenceAt] === 0x52 && (referenceAt + 1 === limit || pdfDelimiter(bytes[referenceAt + 1])))
                end = referenceAt + 1;
        }
    }
    return end;
}
function pdfDictionaryHasTopLevelKey(bytes, offset, key, deadlineExpired) {
    if (bytes[offset] !== 0x3c || bytes[offset + 1] !== 0x3c)
        return false;
    let cursor = offset + 2;
    let pairs = 0;
    while (++pairs <= 100_000) {
        if (deadlineExpired?.("dictionary"))
            return false;
        cursor = skipPdfTrivia(bytes, cursor, bytes.length, false, deadlineExpired);
        if (cursor < 0)
            return false;
        ...
        const start = ++cursor;
        let nameSteps = 0;
        while (cursor < bytes.length && !pdfDelimiter(bytes[cursor])) {
            if ((nameSteps++ & 255) === 0 && deadlineExpired?.("dictionary"))
                return false;
            cursor += 1;
        }
        if (cursor - start > 256)
        ...
        if (name === key)
            return true;
        cursor = pdfObjectEnd(bytes, cursor, bytes.length, deadlineExpired);
        if (cursor < 0)
            return false;
    }
    return false;
}
function pdfDictionary(bytes, offset, integerKeys, nameKeys = new Set(), integerArrayKeys = new Set(), nameListKeys = new Set(), referenceKeys = new Set(), presenceKeys = new Set(), deadlineExpired) {
    if (deadlineExpired?.("dictionary"))
        return null;
    if (bytes[offset] !== 0x3c || bytes[offset + 1] !== 0x3c)
        return null;
    ...
    const present = new Set([...presenceKeys].filter((key) => pdfDictionaryHasTopLevelKey(bytes, offset, key, deadlineExpired)));
    let depth = 0;
    let index = offset;
    let steps = 0;
    if (deadlineExpired?.("dictionary"))
    ...
    while (index < bytes.length && ++steps <= 2_000_000) {
        if ((steps & 255) === 0 && deadlineExpired?.("dictionary"))
            return null;
        ...
        if (byte === 0x3e && bytes[index + 1] === 0x3e) {
            ...
            index += 2;
            if (depth === 0)
                return { end: index, values, names, arrays, nameLists, references, present };
            if (depth < 0)
                return null;
            continue;
        }
        if (byte === 0x2f) {
            ...
            if (depth === 1 && (integerKeys.has(name) || referenceKeys.has(name))) {
                ...
                if (!match || values.has(name) || references.has(name))
                    return null;
                const first = Number(match[1]);
                if (!Number.isSafeInteger(first))
                    return null;
                const after = skipPdfTrivia(bytes, valueAt + match[1].length, bytes.length, false, deadlineExpired);
                if (after < 0)
                    return null;
                ...
                if (reference) {
                    ...
                    if (!referenceKeys.has(name) || first > 999_999 || generation > 65_535)
                        return null;
                    references.set(name, { objectNumber: first, generation });
                }
                else {
                    if (!integerKeys.has(name))
                        return null;
                ...
                while (result.length <= 2_000) {
                    if (deadlineExpired?.("dictionary"))
                        return null;
                    valueAt = skipPdfTrivia(bytes, valueAt, bytes.length, false, deadlineExpired);
                    if (valueAt < 0)
                        return null;
...
function pdfIndirectInteger(bytes, reference, entries, deadlineExpired) {
    if (deadlineExpired?.("object"))
        return null;
    const entry = entries.get(reference.objectNumber);
    if (!entry || entry.type !== 1 || entry.generation !== reference.generation)
        return null;
    ...
    if (!head || Number(head[1]) !== reference.objectNumber || Number(head[2]) !== reference.generation)
        return null;
    let cursor = skipPdfTrivia(bytes, entry.offset + head[0].length, bytes.length, false, deadlineExpired);
    if (cursor < 0)
        return null;
    ...
    if (!Number.isSafeInteger(value) || value > 24_000_000)
        return null;
    cursor = skipPdfTrivia(bytes, cursor + integer[1].length, bytes.length, false, deadlineExpired);
    if (cursor < 0 || !pdfKeywordAt(bytes, cursor, "endobj"))
        return null;
    return value;
}
function pdfStreamLength(bytes, dictionary, entries, deadlineExpired) {
    const direct = dictionary.values.get("Length");
    if (direct !== undefined)
        return direct <= 24_000_000 ? direct : null;
    const reference = dictionary.references.get("Length");
    return reference ? pdfIndirectInteger(bytes, reference, entries, deadlineExpired) : null;
}
function locatePdfIndirectInteger(bytes, reference, limit, deadlineExpired) {
    ...
    let found = null;
    const matches = (at, text) => at + text.length <= limit && bytes.subarray(at, at + text.length).toString("ascii") === text;
    const skipWhitespace = (at) => { let steps = 0; while (at < limit && pdfWhitespace(bytes[at])) {
        if ((steps++ & 255) === 0 && deadlineExpired?.("object"))
            return -1;
        at += 1;
    } return at; };
    for (let cursor = 0; cursor < limit; cursor += 1) {
        if ((cursor & 255) === 0 && deadlineExpired?.("object"))
            return null;
        if ((cursor > 0 && !pdfWhitespace(bytes[cursor - 1])) || !matches(cursor, objectNumber))
            continue;
        let at = cursor + objectNumber.length;
        if (at >= limit || !pdfWhitespace(bytes[at]))
            continue;
        at = skipWhitespace(at);
        if (at < 0)
            return null;
        if (!matches(at, generation))
            continue;
        at += generation.length;
        if (at >= limit || !pdfWhitespace(bytes[at]))
            continue;
        at = skipWhitespace(at);
        if (at < 0)
        ...
        if (at < 0)
            return null;
        const valueStart = at;
        while (at < limit && bytes[at] >= 0x30 && bytes[at] <= 0x39 && at - valueStart <= 20)
            at += 1;
        if (at === valueStart || at - valueStart > 20 || at >= limit || !pdfWhitespace(bytes[at]))
            continue;
        const value = Number(bytes.subarray(valueStart, at).toString("ascii"));
        at = skipWhitespace(at);
        if (at < 0)
            return null;
        if (!matches(at, "endobj") || (at + 6 < limit && !pdfDelimiter(bytes[at + 6])))
            continue;
        if (!Number.isSafeInteger(value) || value > 24_000_000 || found)
            return null;
        found = { offset: cursor, value };
    }
    return found;
}
function pdfTopLevelValueOffset(bytes, dictionaryOffset, dictionaryEnd, wanted, deadlineExpired) {
    let cursor = dictionaryOffset + 2;
    let found;
    let pairs = 0;
    while (++pairs <= 100_000) {
        if (deadlineExpired?.("dictionary"))
            return null;
        cursor = skipPdfTrivia(bytes, cursor, dictionaryEnd, false, deadlineExpired);
        if (cursor < 0)
            return null;
        if (bytes[cursor] === 0x3e && bytes[cursor + 1] === 0x3e)
            return found;
        ...
        let nameSteps = 0;
        while (cursor < dictionaryEnd && !pdfDelimiter(bytes[cursor])) {
            if ((nameSteps++ & 255) === 0 && deadlineExpired?.("dictionary"))
                return null;
            cursor += 1;
        }
        if (cursor - nameStart > 256)
            return null;
        ...
        const valueAt = skipPdfTrivia(bytes, cursor, dictionaryEnd, false, deadlineExpired);
        if (valueAt < 0)
            return null;
        if (name === wanted) {
            if (found !== undefined)
        ...
        cursor = pdfObjectEnd(bytes, valueAt, dictionaryEnd, deadlineExpired);
        if (cursor < 0)
            return null;
    }
    return null;
}
function pdfPredictorParameters(bytes, dictionaryOffset, dictionaryEnd, filterIsArray, deadlineExpired) {
    const parameterAt = pdfTopLevelValueOffset(bytes, dictionaryOffset, dictionaryEnd, "DecodeParms", deadlineExpired);
    if (parameterAt === null)
        return null;
    if (parameterAt === undefined)
    ...
    if (bytes[at] === 0x5b) {
        if (!filterIsArray)
            return null;
        array = true;
        at = skipPdfTrivia(bytes, at + 1, dictionaryEnd, false, deadlineExpired);
        if (at < 0)
            return null;
    }
    if (pdfKeywordAt(bytes, at, "null")) {
        at = skipPdfTrivia(bytes, at + 4, dictionaryEnd, false, deadlineExpired);
        if (at < 0 || (array && bytes[at] !== 0x5d))
            return null;
        return { predictor: 1, colors: 1, bits: 8, columns: 1 };
    }
    ...
    if (!parameters || parameters.end > dictionaryEnd || (parameters.values.has("BitsPerComponent") && parameters.values.has("BPC")))
        return null;
    let end = skipPdfTrivia(bytes, parameters.end, dictionaryEnd, false, deadlineExpired);
    if (end < 0 || (array && bytes[end] !== 0x5d))
        return null;
    const predictor = parameters.values.get("Predictor") ?? 1;
    const colors = parameters.values.get("Colors") ?? 1;
    const bits = parameters.values.get("BitsPerComponent") ?? parameters.values.get("BPC") ?? 8;
    const columns = parameters.values.get("Columns") ?? 1;
    return { predictor, colors, bits, columns };
}
function pdfPredictorLayout(parameters, entryCount, recordBytes) {
    const { predictor, colors, bits, columns } = parameters;
    if (!Number.isSafeInteger(predictor) || !Number.isSafeInteger(colors) || colors < 1 || colors > 32 || ![1, 2, 4, 8, 16].includes(bits) || !Number.isSafeInteger(columns) || columns < 1)
        return null;
    const decodedLength = BigInt(entryCount) * BigInt(recordBytes);
    if (predictor === 1)
        return { encodedLength: Number(decodedLength), decodedLength: Number(decodedLength), rowBytes: recordBytes, rowCount: entryCount };
    if (predictor !== 2 && (predictor < 10 || predictor > 15))
        return null;
    const rowBytes = (BigInt(colors) * BigInt(bits) * BigInt(columns) + 7n) / 8n;
    if (rowBytes < 1n || decodedLength % rowBytes !== 0n)
        return null;
    const rowCount = decodedLength / rowBytes;
    const encodedLength = decodedLength + (predictor >= 10 ? rowCount : 0n);
    return encodedLength <= 25000000n ? { encodedLength: Number(encodedLength), decodedLength: Number(decodedLength), rowBytes: Number(rowBytes), rowCount: Number(rowCount) } : null;
}
function decodePdfPredictor(encoded, parameters, entryCount, recordBytes, deadlineExpired) {
    const { predictor, colors, bits, columns } = parameters;
    const layout = pdfPredictorLayout(parameters, entryCount, recordBytes);
    if (!layout || encoded.length !== layout.encodedLength || layout.decodedLength > 24_000_000 || deadlineExpired?.("predictor-start"))
        return null;
    if (predictor <= 1)
        return encoded;
    const { decodedLength, rowBytes, rowCount } = layout;
    const decoded = Buffer.alloc(decodedLength);
    if (deadlineExpired?.("predictor-start"))
        return null;
    const pixelBytes = Math.ceil(colors * bits / 8);
    let encodedAt = 0;
    for (let row = 0; row < rowCount; row += 1) {
        if (deadlineExpired?.("predictor-row"))
            return null;
        const outputAt = row * rowBytes;
        const previousAt = outputAt - rowBytes;
        if (predictor >= 10) {
            const filter = encoded[encodedAt++];
            if (filter > 4)
                return null;
            for (let column = 0; column < rowBytes; column += 1) {
                if ((column & 255) === 0 && deadlineExpired?.("predictor-work"))
                    return null;
                const raw = encoded[encodedAt++];
                const left = column >= pixelBytes ? decoded[outputAt + column - pixelBytes] : 0;
                const up = row > 0 ? decoded[previousAt + column] : 0;
                const upLeft = row > 0 && column >= pixelBytes ? decoded[previousAt + column - pixelBytes] : 0;
                if (filter === 0)
                    decoded[outputAt + column] = raw;
                else if (filter === 1)
                    decoded[outputAt + column] = raw + left;
                else if (filter === 2)
                    decoded[outputAt + column] = raw + up;
                else if (filter === 3)
                    decoded[outputAt + column] = raw + Math.floor((left + up) / 2);
                else {
                    const p = left + up - upLeft;
                    const pa = Math.abs(p - left);
                    const pb = Math.abs(p - up);
                    const pc = Math.abs(p - upLeft);
                    decoded[outputAt + column] = raw + (pa <= pb && pa <= pc ? left : pb <= pc ? up : upLeft);
                }
            }
            continue;
        }
        const raw = encoded.subarray(encodedAt, encodedAt + rowBytes);
        encodedAt += rowBytes;
        if (bits === 1 && colors === 1) {
            let carry = 0;
            for (let column = 0; column < rowBytes; column += 1) {
                if ((column & 255) === 0 && deadlineExpired?.("predictor-work"))
                    return null;
                let value = raw[column] ^ carry;
                value ^= value >> 1;
                value ^= value >> 2;
                value ^= value >> 4;
                carry = (value & 1) << 7;
                decoded[outputAt + column] = value;
            }
        }
        else if (bits === 8) {
            for (let column = 0; column < rowBytes; column += 1) {
                if ((column & 255) === 0 && deadlineExpired?.("predictor-work"))
                    return null;
                decoded[outputAt + column] = column < colors ? raw[column] : raw[column] + decoded[outputAt + column - colors];
            }
        }
        else if (bits === 16) {
            const pixel = colors * 2;
            for (let column = 0; column < rowBytes; column += 2) {
                if ((column & 255) === 0 && deadlineExpired?.("predictor-work"))
                    return null;
                if (column < pixel) {
                    decoded[outputAt + column] = raw[column];
                    decoded[outputAt + column + 1] = raw[column + 1];
                }
                else {
                    const value = (raw[column] << 8) + raw[column + 1] + (decoded[outputAt + column - pixel] << 8) + decoded[outputAt + column - pixel + 1];
                    decoded[outputAt + column] = value >> 8;
                    decoded[outputAt + column + 1] = value;
                }
            }
        }
        else {
            ...
            for (let column = 0; column < columns; column += 1)
                for (let color = 0; color < colors; color += 1) {
                    if (((column * colors + color) & 255) === 0 && deadlineExpired?.("predictor-work"))
                        return null;
                    if (inputBits < bits) {
                        inputBuffer = inputBuffer << 8 | raw[input++];
                        inputBits += 8;
                    }
                    components[color] = components[color] + (inputBuffer >> (inputBits - bits)) & mask;
                    inputBits -= bits;
                    outputBuffer = outputBuffer << bits | components[color];
                    outputBits += bits;
                    if (outputBits >= 8) {
                        decoded[output++] = outputBuffer >> (outputBits - 8);
                        outputBits -= 8;
                    }
                }
            if (outputBits > 0)
                decoded[output] = (outputBuffer << (8 - outputBits)) + (inputBuffer & ((1 << (8 - outputBits)) - 1));
        }
    }
    return decoded;
}
function pdfXrefSection(bytes, offset, allowHybrid = true, bootstrapEntries, deadlineExpired) {
    if (deadlineExpired?.("xref"))
        return null;
    ...
    if (pdfKeywordAt(bytes, offset, "xref")) {
        ...
        while (cursor < bytes.length) {
            ...
            if (pdfKeywordAt(bytes, cursor, "trailer")) {
                cursor = skipPdfTrivia(bytes, cursor + 7, bytes.length, false, deadlineExpired);
                if (cursor < 0)
                    return null;
                const dictionary = pdfDictionary(bytes, cursor, new Set(["Prev", "Size", "XRefStm"]), new Set(), new Set(), new Set(), new Set(), new Set(["Encrypt"]), deadlineExpired);
                const size = dictionary?.values.get("Size");
                if (!dictionary || dictionary.present.has("Encrypt") || size === undefined || size < 1 || size > 1_000_000 || highestObject >= size)
                    return null;
                for (const entry of entries.values()) {
                    if (deadlineExpired?.("xref") || (entry.type === 0 && entry.nextFree >= size))
                        return null;
                }
                const indirectLengths = [];
                ...
                if (hybridOffset !== undefined) {
                    ...
                    if (!pdfRevisionObjectsOnly(bytes, hybrid.end, offset, objectOffsets, false, undefined, deadlineExpired))
                        return null;
                    highestObject = Math.max(highestObject, hybrid.maxObject);
                }
                return { kind: "classic", end: dictionary.end, prev: dictionary.values.get("Prev"), size, maxObject: highestObject, entries, objectOffsets, indirectLengths };
            }
    ...
    if (!head || Number(head[2]) > 65_535)
        return null;
    let cursor = skipPdfTrivia(bytes, offset + head[0].length, bytes.length, false, deadlineExpired);
    if (cursor < 0)
        return null;
    const dictionaryOffset = cursor;
    ...
    const size = dictionary?.values.get("Size");
    if (!dictionary || dictionary.present.has("Encrypt") || (!dictionary.values.has("Length") && !dictionary.references.has("Length")) || dictionary.names.get("Type") !== "XRef" || size === undefined || size < 1 || size > 1_000_000 || !width || width.length !== 3 || width.some((item) => item > 6) || width[1] === 0 || width[0] + width[1] + width[2] === 0)
        return null;
    ...
    if (entryCount > 1_000_000 || entryCount * recordBytes > 24_000_000)
        return null;
    const filterOffset = pdfTopLevelValueOffset(bytes, dictionaryOffset, dictionary.end, "Filter", deadlineExpired);
    if (filterOffset === null)
        return null;
    const filters = dictionary.nameLists.get("Filter");
    const predictorParameters = pdfPredictorParameters(bytes, dictionaryOffset, dictionary.end, filterOffset !== undefined && bytes[filterOffset] === 0x5b, deadlineExpired);
    const predictorLayout = predictorParameters ? pdfPredictorLayout(predictorParameters, entryCount, recordBytes) : null;
    if (!predictorLayout || (filters === undefined && predictorParameters.predictor !== 1))
        return null;
    const lengthReference = dictionary.references.get("Length");
    const locatedLength = lengthReference ? locatePdfIndirectInteger(bytes, lengthReference, offset, deadlineExpired) : null;
    ...
    if (streamLength === undefined || streamLength > 24_000_000)
        return null;
    cursor = skipPdfTrivia(bytes, dictionary.end, bytes.length, false, deadlineExpired);
    ...
    try {
        if (filters === undefined)
            ...
            if (deadlineExpired?.("inflate-before"))
                return null;
            decoded = zlib.inflateSync(bytes.subarray(streamStart, cursor), { maxOutputLength: predictorLayout.encodedLength + 1 });
            if (deadlineExpired?.("inflate-after"))
                return null;
        }
        else
            return null;
    ...
    const predicted = decodePdfPredictor(decoded, predictorParameters, entryCount, recordBytes, deadlineExpired);
    if (!predicted)
        return null;
    decoded = predicted;
    ...
    if (!pdfKeywordAt(bytes, cursor, "endstream"))
        return null;
    cursor = skipPdfTrivia(bytes, cursor + 9, bytes.length, false, deadlineExpired);
    if (cursor < 0 || !pdfKeywordAt(bytes, cursor, "endobj"))
        return null;
    return { kind: "stream", end: cursor + 6, prev: dictionary.values.get("Prev"), size, maxObject, entries, objectOffsets, indirectLengths: lengthReference ? [{ reference: lengthReference, value: streamLength }] : [] };
}
function pdfFooterAfter(bytes, sectionEnd, limit, deadlineExpired) {
    if (deadlineExpired?.("revision"))
        return null;
    let cursor = skipPdfTrivia(bytes, sectionEnd, limit, false, deadlineExpired);
    if (cursor < 0 || !pdfKeywordAt(bytes, cursor, "startxref"))
        return null;
    const start = cursor;
    cursor = skipPdfTrivia(bytes, cursor + 9, limit, false, deadlineExpired);
    if (cursor < 0)
        return null;
    ...
    while (cursor < limit && pdfWhitespace(bytes[cursor])) {
        if ((whitespaceSteps++ & 255) === 0 && deadlineExpired?.("revision"))
            return null;
        cursor += 1;
    }
...
function pdfRevisionObjectsOnly(bytes, start, end, referencedOffsets, allowBinaryComments = false, entries, deadlineExpired) {
    let cursor = skipPdfTrivia(bytes, start, end, allowBinaryComments, deadlineExpired);
    if (cursor < 0)
        return false;
    while (cursor < end) {
        if (deadlineExpired?.("objects"))
            return false;
        ...
        while (index < end && ++steps <= 2_000_000) {
            if ((steps & 255) === 0 && deadlineExpired?.("objects"))
                return false;
            if (bytes[index] === 0x25) {
            ...
            if (bytes[index] === 0x3c && bytes[index + 1] === 0x3c) {
                const dictionary = pdfDictionary(bytes, index, new Set(["Length"]), new Set(), new Set(), new Set(), new Set(["Length"]), new Set(), deadlineExpired);
                if (!dictionary)
                    return false;
                const after = skipPdfTrivia(bytes, dictionary.end, end, false, deadlineExpired);
                if (after < 0)
                    return false;
                if (pdfKeywordAt(bytes, after, "stream")) {
                    const length = entries ? pdfStreamLength(bytes, dictionary, entries, deadlineExpired) : dictionary.values.get("Length");
        ...
        if (!foundEnd)
            return false;
        cursor = skipPdfTrivia(bytes, cursor, end, allowBinaryComments, deadlineExpired);
        if (cursor < 0)
            return false;
    }
    return cursor === end;
}
function pdfObjectStreamNumbers(bytes, entry, entries, deadlineExpired) {
    if (deadlineExpired?.("objects"))
        return null;
    const head = /^(\d{1,10})[\x00\x09\x0a\x0c\x0d\x20]+(\d{1,10})[\x00\x09\x0a\x0c\x0d\x20]+obj(?:[\x00\x09\x0a\x0c\x0d\x20]|$)/.exec(bytes.subarray(entry.offset, Math.min(bytes.length, entry.offset + 80)).toString("latin1"));
    ...
    const length = dictionary ? pdfStreamLength(bytes, dictionary, entries, deadlineExpired) : null;
    const count = dictionary?.values.get("N");
    const first = dictionary?.values.get("First");
    if (!dictionary || dictionary.names.get("Type") !== "ObjStm" || length === null || count === undefined || count < 1 || count > 100_000 || first === undefined || first > 24_000_000)
        return null;
    cursor = skipPdfTrivia(bytes, dictionary.end, bytes.length, false, deadlineExpired);
    if (cursor < 0 || !pdfKeywordAt(bytes, cursor, "stream"))
        return null;
    ...
    for (let index = 0; index < count; index += 1) {
        if (deadlineExpired?.("objects"))
            return null;
        headerAt = skipPdfTrivia(decoded, headerAt, first, false, deadlineExpired);
        if (headerAt < 0)
            return null;
    ...
    if (previousOffset < 0 || skipPdfTrivia(decoded, headerAt, first, false, deadlineExpired) !== first)
        return null;
    if (bytes[cursor] === 13 && bytes[cursor + 1] === 10)
        cursor += 2;
    else if (bytes[cursor] === 10 || bytes[cursor] === 13)
...
function validPdfEffectiveXref(bytes, size, entries, deadlineExpired) {
    ...
    for (const [objectNumber, entry] of entries)
        if (entry.type === 2) {
            ...
            if (!objectNumbers) {
                objectNumbers = pdfObjectStreamNumbers(bytes, container, entries, deadlineExpired) ?? undefined;
                if (!objectNumbers)
                    return false;
...
function hasPdfFooterBefore(bytes, limit, deadlineExpired) {
    const startxref = Buffer.from("startxref");
    const eof = Buffer.from("%%EOF");
    const skipWhitespace = (at) => { let steps = 0; while (at < limit && pdfWhitespace(bytes[at])) {
        if ((steps++ & 255) === 0 && deadlineExpired("revision"))
            return -1;
        at += 1;
    } return at; };
    for (let cursor = 0; cursor + startxref.length < limit; cursor += 1) {
        if ((cursor & 255) === 0 && deadlineExpired("revision"))
            return true;
        if (!bytes.subarray(cursor, cursor + startxref.length).equals(startxref))
            continue;
        let at = cursor + startxref.length;
        if (!pdfWhitespace(bytes[at]))
            continue;
        at = skipWhitespace(at);
        if (at < 0)
            return true;
        const pointerStart = at;
        while (at < limit && bytes[at] >= 0x30 && bytes[at] <= 0x39 && at - pointerStart <= 20)
            at += 1;
        if (at === pointerStart || at - pointerStart > 20 || at >= limit || !pdfWhitespace(bytes[at]))
            continue;
        at = skipWhitespace(at);
        if (at < 0)
            return true;
        if (at + eof.length <= limit && bytes.subarray(at, at + eof.length).equals(eof))
            return true;
    }
    return false;
}
function validPdfTerminalBoundary(bytes, deadlineExpired) {
    ...
    if (deadlineExpired("terminal"))
        return false;
    let terminalMarker = bytes.lastIndexOf(Buffer.from("%%EOF"));
    if (deadlineExpired("terminal"))
        return false;
    while (terminalMarker >= 0 && ((terminalMarker > 0 && bytes[terminalMarker - 1] !== 10 && bytes[terminalMarker - 1] !== 13) || skipPdfTrivia(bytes, terminalMarker + 5, bytes.length, false, deadlineExpired) !== bytes.length)) {
        if (deadlineExpired("terminal"))
            return false;
    ...
    while (++revisions <= 256) {
        ...
        if (seen.has(offset))
            return false;
        seen.add(offset);
        const section = pdfXrefSection(bytes, offset, true, undefined, deadlineExpired);
        if (!section)
            return false;
        const footer = pdfFooterAfter(bytes, section.end, bytes.length, deadlineExpired);
        ...
        if (section.prev === undefined) {
            ...
            for (let index = chain.length - 1; index >= 0; index -= 1) {
                ...
                if (!pdfRevisionObjectsOnly(bytes, bodyStart, revision.offset, revision.objectOffsets, index === chain.length - 1, effective, deadlineExpired))
                    return false;
                for (const indirect of revision.indirectLengths) {
                    if (deadlineExpired("object"))
                        return false;
                    if (pdfIndirectInteger(bytes, indirect.reference, effective, deadlineExpired) !== indirect.value)
                        return false;
                }
                if (!validPdfEffectiveXref(bytes, revision.size, effective, deadlineExpired))
                    return false;
            }
            return true;
        }
...
function loadPdfJsModule() {
    if (!pdfJsModulePromise) {
        ...
        void pending.catch(() => { if (pdfJsModulePromise === pending)
...
export async function validatePdfIngress(bytes, options = {}) {
    ...
    const monotonicNow = typeof options.monotonicNow === "function" ? options.monotonicNow : () => process.hrtime.bigint();
    const deadline = monotonicNow() + BigInt(timeoutMs) * 1000000n;
    const deadlineExpired = (stage = "pdfjs") => { options.pdfPreflightCheckpoint?.(stage); return expired || monotonicNow() >= deadline; };
    if (deadlineExpired("header") || bytes.length < 16 || bytes.subarray(0, 5).toString("ascii") !== "%PDF-" || !validPdfTerminalBoundary(bytes, deadlineExpired) || deadlineExpired("preflight-complete"))
        return false;
    try {
        const workflow = (async () => {
            ...
            if (deadlineExpired() || document.numPages < 1 || document.numPages > 100)
                return false;
            ...
            const visit = (candidate, depth = 0, role = "ordinary") => {
                if (deadlineExpired() || depth > 128 || ++visitedCount > 100_000)
                    return false;
            ...
            for (const [, object] of parsed.context.enumerateIndirectObjects())
                if (deadlineExpired() || !visit(object))
                    return false;
            ...
            for (let page = 1; page <= document.numPages; page += 1) {
                ...
                if (deadlineExpired() || hasEntries(actions))
                    return false;
        ...
        return await Promise.race([workflow, new Promise((resolve) => { const remaining = deadline - monotonicNow(); timer = setTimeout(() => { expired = true; try {
                task?.destroy?.();
            }
...
function isStaticShellWord(word) {
...
function isRegularExecutableShellPath(candidate) {
...
function shellCommandCanRun(name) {
    ...
    if (/^[A-Za-z][A-Za-z0-9+.-]*:\/\//.test(name))
        return false;
    if (name.includes("/"))
        return isRegularExecutableShellPath(name);
    ...
    for (const entry of pathEntries.slice(0, 128)) {
        ...
        if (Buffer.byteLength(directory, "utf8") > 4096)
            return true;
        if (isRegularExecutableShellPath(`${directory.replace(/\/$/, "")}/${name}`))
            return true;
    }
...
function braceSequenceAlternatives(body) {
    ...
    const format = (value) => alphabetic ? String.fromCharCode(value) : width > 0
...
function shellWordHasPathExpansion(word) {
    ...
    for (let index = 0; index < raw.length; index += 1) {
        ...
        if (character === "{") {
            ...
            if (close > index + 1 && braceBodyIsExpandable(raw.slice(index + 1, close)))
                return true;
        }
    }
    return false;
...
function boundedBraceExpansion(pattern) {
    ...
    for (let depth = 0; depth < 8; depth += 1) {
        ...
        for (const candidate of patterns) {
            ...
            if (!match || alternatives === undefined) {
                next.push(candidate);
                continue;
            }
    ...
    for (let index = 0; index < segment.length; index += 1) {
        ...
        if (character === "[") {
            ...
            while (close < segment.length) {
                ...
                if (segment[close] === "]" && close > index + 1)
                    break;
                const literal = segment[close];
                body += literal === "\\" || literal === "]" ? `\\${literal}` : literal;
                close += 1;
            }
...
function expandedShellCommandCanRun(pattern) {
    if (Buffer.byteLength(pattern, "utf8") > 4096)
        return true;
    const braces = boundedBraceExpansion(pattern);
    if (!braces)
        return true;
    let visited = 0;
    for (let candidate of braces) {
        ...
        if (candidate.startsWith("~")) {
            ...
            roots = [absolute ? "/" : process.cwd()];
            segments = candidate.split("/").filter((segment, index) => !(absolute && index === 0));
        }
        ...
        for (const segment of segments) {
            if (!segment || segment === ".")
                continue;
            if (segment === "..") {
                roots = roots.map((root) => pathRuntime.dirname(root));
                continue;
            }
            ...
            for (const root of roots) {
                if (!matcher)
                    next.push(pathRuntime.join(root, segment));
                else {
                    let names;
                    try {
                    ...
                    for (const name of names) {
                        ...
                        if (visited > 4096)
                            return true;
                        if ((!segment.startsWith(".") && name.startsWith(".")) || !matcher.test(name))
                            continue;
                        next.push(pathRuntime.join(root, name));
                        if (next.length > 64)
                            return true;
                    }
                }
...
function parsedShellNodeHasRunnableCommand(root) {
    ...
    while (pending.length > 0) {
        ...
        if (visited > maximumContentPolicyAstNodes)
            return true;
        if (parsedShellCommandCanRun(value))
            return true;
...
function isNonRunnablePathSentence(text, root) {
...
export function isCurrentClamavSignature(signature, now = Date.now()) { const builtAt = Date.parse(signature?.updatedAt); return Boolean(signature) && typeof signature?.version === "string" && /^daily:\d{1,12}$/.test(signature.version) && Number.isFinite(builtAt) && builtAt <= now && now - builtAt <= clamavSignatureMaxAgeMs; }
...
function clamavNow(database) { return database.__clamavTest?.now?.() ?? Date.now(); }
function clamavDelay(database, timeoutMs) { return database.__clamavTest?.delay?.(timeoutMs) ?? new Promise((resolve) => setTimeout(resolve, timeoutMs)); }
function clamavRemaining(database, deadline) { return Math.max(0, deadline - clamavNow(database)); }
async function verifiedClamavSignature(database, deadline = Number.POSITIVE_INFINITY) {
    ...
    for (const path of ["/app/data/clamav/daily.cld", "/app/data/clamav/daily.cvd"]) {
        ...
        const result = await collectBoundedToolOutput(child, Math.min(5_000, remaining));
        if (!result.ok) {
...
function clamavSocketCommand(database, command, maximumReplyBytes = 4096, timeoutMs = 2_000) {
    if (database.__clamavTest?.socketCommand)
        return database.__clamavTest.socketCommand(command, timeoutMs, maximumReplyBytes);
...
async function loadedClamavSignatureVersion(database, timeoutMs = 500) {
    if (typeof database.__clamavTest?.loadedSignature === "string")
        return /^daily:\d{1,12}$/.test(database.__clamavTest.loadedSignature) ? database.__clamavTest.loadedSignature : null;
...
async function clamavInstream(database, bytes, requestSignature) {
    ...
    if (scanned.outcome !== "clean" && scanned.outcome !== "rejected")
        return scanned;
    ...
    if (scanned.outcome === "rejected")
        return signatureFresh && loadedSignature === signature.version ? scanned : { outcome: "rejected", signatureVersion: "unavailable" };
...
function waitForChild(child, timeoutMs) { if (clamavChildTerminated(child))
    return Promise.resolve(child?.exitCode === 0); return new Promise((resolve) => { let settled = false; const finish = (ok) => { if (settled)
    return; settled = true; clearTimeout(timer); resolve(ok); }; const timer = setTimeout(() => finish(false), Math.max(0, timeoutMs)); child.once("exit", (code) => finish(code === 0)); child.once("error", () => finish(false)); }); }
function signalChildUntil(database, child, signal, deadline) {
    if (clamavChildTerminated(child))
    ...
    return new Promise((resolve, reject) => {
        ...
        const finish = (value) => { if (settled)
            ...
            clearTimeout(timer); remove(); resolve(value || clamavChildTerminated(child)); };
        const fail = (error) => { if (settled)
            return; settled = true; if (timer)
            clearTimeout(timer); remove(); reject(error); };
        ...
        if (database.__clamavTest?.delay)
            Promise.resolve(database.__clamavTest.delay(remaining)).then(() => finish(false), fail);
        else
            timer = setTimeout(() => finish(false), remaining);
    });
}
async function terminateChild(child, timeoutMs = 5_000, database = {}) {
    if (!child || clamavChildTerminated(child))
        return;
    const startedAt = clamavNow(database);
    const deadline = startedAt + Math.max(0, timeoutMs);
    const termDeadline = startedAt + Math.floor(Math.max(0, timeoutMs) / 2);
    if (await signalChildUntil(database, child, "SIGTERM", termDeadline))
        return;
    if (await signalChildUntil(database, child, "SIGKILL", deadline))
        return;
...
async function probeClamavReadiness(database, deadline) {
    ...
    if (remaining <= 0)
        return false;
    if (database.__clamavTest?.readinessProbe)
        return Boolean(await database.__clamavTest.readinessProbe(remaining));
    const current = await currentLoadedClamavSignature(database, deadline);
    if (!current)
        return false;
    const afterSignature = clamavRemaining(database, deadline);
    if (afterSignature <= 0)
        return false;
    return await clamavSocketCommand(database, Buffer.from("zPING\0"), 16, Math.min(500, afterSignature)) === "PONG";
}
export async function waitForClamavReadiness(database, child, deadline, socketPath) {
    while (clamavRemaining(database, deadline) > 0) {
        ...
        if (exists && await probeClamavReadiness(database, deadline) && clamavRemaining(database, deadline) > 0)
            return true;
        const remaining = clamavRemaining(database, deadline);
        if (remaining <= 0)
            return false;
...
async function stopOwnedClamavChildren(database) {
    ...
    if (failures.length)
        throw new AggregateError(failures, "ClamAV child cleanup failed.");
}
...
const clamavRefreshTimeoutMs = 5 * 60 * 1000;
function clamavSpawn(database, command, args) { return database.__clamavTest?.spawn ? database.__clamavTest.spawn(command, args) : childProcess.spawn(command, args, { stdio: "ignore" }); }
function clamavSchedule(database, callback, delayMs) { if (database.__clamavTest?.schedule)
    return database.__clamavTest.schedule(callback, delayMs); const timer = setTimeout(callback, delayMs); timer.unref?.(); return () => clearTimeout(timer); }
async function runFreshclam(database, deadline) {
    ...
    const completed = await waitForChild(update, clamavRemaining(database, deadline));
    if (!completed)
        await terminateChild(update, Math.min(clamavTerminateTimeout(database), clamavRemaining(database, deadline)), database);
    if (database.__clamavUpdateProcess === update)
        database.__clamavUpdateProcess = null;
    return completed;
}
...
function nextClamavRefreshDelay(database, signature, refreshed) { if (!refreshed)
    return clamavRefreshRetryMs; const expiresIn = Date.parse(signature?.updatedAt) + clamavSignatureMaxAgeMs - clamavNow(database); return Math.max(1_000, Math.min(clamavRefreshIntervalMs, expiresIn + 1_000)); }
...
async function refreshClamavSignatures(database) {
    ...
    if (await loadedClamavSignatureVersion(database) !== signature.version)
        await clamavSocketCommand(database, Buffer.from("zRELOAD\0"), 64);
    if (!refreshed)
        await emitClamavRefreshWarning(database, "CLAMAV_SIGNATURE_REFRESH_FAILED");
    return nextClamavRefreshDelay(database, signature, refreshed);
}
async function emitClamavRefreshWarning(database, code) { try {
...
function scheduleClamavRefresh(database, delayMs) {
    if (database.__clamavRefreshStopped)
        return;
    database.__clamavRefreshCancel = clamavSchedule(database, () => {
        database.__clamavRefreshCancel = null;
        const pending = refreshClamavSignatures(database).catch(async () => { await emitClamavRefreshWarning(database, "CLAMAV_SIGNATURE_REFRESH_FAILED"); return clamavRefreshRetryMs; }).then((nextDelayMs) => { if (database.__clamavRefreshPending === pending)
            database.__clamavRefreshPending = null; scheduleClamavRefresh(database, nextDelayMs); });
        database.__clamavRefreshPending = pending;
    }, delayMs);
}
function stopClamavRefresh(database) { database.__clamavRefreshStopped = true; database.__clamavRefreshCancel?.(); database.__clamavRefreshCancel = null; }
export async function initializeClamavRuntime(database) {
    ...
    if (database.__clamavDevSidecar) {
        ...
        database.clamavReady = await waitForClamavReadiness(database, database.__clamavDevSidecar.process, deadline, database.__clamavDevSidecar.socketPath);
        return database.clamavReady;
    }
    if (database.__clamavTest && !database.__clamavTest.managed) {
        database.clamavReady = Boolean(await currentLoadedClamavSignature(database, deadline));
        return database.clamavReady;
    }
    if (!database.__clamavTest && process.env.SPORADES_CLAMAV_MANAGED !== "1")
        return false;
    ...
    for (let attempt = 0;; attempt += 1) {
        refreshed = await runFreshclam(database, deadline);
        signature = await verifiedClamavSignature(database, deadline);
        if (isCurrentClamavSignature(signature, clamavNow(database)))
            break;
        const delayMs = clamavBootRetryDelaysMs[attempt];
        if (delayMs === undefined || clamavRemaining(database, deadline) <= delayMs) {
            database.clamavReady = false;
    ...
    if (!await startClamd(database, deadline))
        return false;
    scheduleClamavRefresh(database, nextClamavRefreshDelay(database, signature, refreshed));
    return true;
}
export async function shutdownClamavRuntime(database) { database.clamavReady = false; stopClamavRefresh(database); if (!database.__clamavDevSidecar?.externallyManaged) {
    await stopOwnedClamavChildren(database);
    await database.__clamavRefreshPending;
    if (database.__clamavProcess || database.__clamavUpdateProcess)
        await stopOwnedClamavChildren(database);
    database.clamavReady = false;
}
else {
...
export async function checkClamavRuntime(database) { if (!database.clamavRequired)
    return { ok: true }; const children = [database.__clamavDevSidecar?.process, database.__clamavProcess]; for (const child of children)
...
async function inspectIngressLease(database, policy, row, bytes, clamavRequestSignature) {
    if (!policy)
        return undefined;
    const verdicts = await Promise.all(policy.requiredInspectors.map(async (inspector) => { const result = inspector === "content-policy-v1" ? { outcome: await contentPolicyOutcome(row, bytes), signatureVersion: "content-policy-v1" } : await clamavInstream(database, bytes, clamavRequestSignature); const inspectedAt = ingressAuditNow(database); return Object.freeze({ inspector, outcome: result.outcome, leaseId: row.leaseId, size: row.size, digest: row.digest, version: row.version, policyRevision: policy.policyRevision, engine: inspector === "content-policy-v1" ? "sporades-content-policy" : "clamav", signatureVersion: result.signatureVersion, inspectedAt }); }));
            ...
            if (state === "body") {
                ...
                if (at < 0) {
                    ...
                    if (take) {
                        ...
                        size += take;
                        if (size > partLimit)
                            throw multipartLimitExceeded(partType, partLimitKind, partLimit);
    ...
    try {
        for await (const part of multipartParts(request, boundary, maxBytes, { file: streamingFileLimit, field: policy.maxFieldBytes, fileKind: streamingFileLimitKind, fieldKind: "maxFieldBytes" }, allowFiles)) {
            ...
            if (filename === undefined) {
                ...
                if (fieldCount > policy.maxFieldCount)
                    throw multipartLimitExceeded("field", "maxFieldCount", policy.maxFieldCount, "Multipart field exceeds declared limits.");
                if (body.length > policy.maxFieldBytes)
                    throw multipartLimitExceeded("field", "maxFieldBytes", policy.maxFieldBytes, "Multipart field exceeds declared limits.");
                if (fieldBytes > policy.maxTotalFieldBytes)
                    throw multipartLimitExceeded("field", "maxTotalFieldBytes", policy.maxTotalFieldBytes, "Multipart field exceeds declared limits.");
            ...
            if (files.length >= policy.maxFiles)
                throw multipartLimitExceeded("file", "maxFiles", policy.maxFiles, "Multipart file exceeds declared limits.");
            if (body.length > policy.maxFileBytes)
                throw multipartLimitExceeded("file", "maxFileBytes", policy.maxFileBytes, "Multipart file exceeds declared limits.");
            if (body.length > database.fileMaxSizeBytes)
                throw multipartLimitExceeded("file", "fileMaxSizeBytes", database.fileMaxSizeBytes, "Multipart file exceeds declared limits.");
            if (fileBytes > policy.maxTotalFileBytes)
                throw multipartLimitExceeded("file", "maxTotalFileBytes", policy.maxTotalFileBytes, "Multipart file exceeds declared limits.");
            ...
            if (!sameIngressRetryDescriptor(row, candidate))
            ...
            if (!acquired.winner && row.state === "staging")
                row = await awaitCompletedStagingReceipt(database, row.key);
            if (!row || !sameIngressRetryDescriptor(row, candidate))
                throw Object.assign(new Error("Multipart retry descriptor conflicts with the original part."), { code: "INGRESS_DESCRIPTOR_CONFLICT" });
