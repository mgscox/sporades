...
export function isJavaScriptRawInputWithinBounds(text) {
    ...
    for (let index = 0; index < text.length; index += 1) {
        ...
        if (mode === "regex") {
            ...
            if (character === "(") {
                regexGroups += 1;
                if (regexGroups > maximumContentPolicyParserRecursion)
                    return false;
                continue;
            }
            if (character === ")") {
        ...
        if (/[A-Za-z_$]/.test(character)) {
            let word = character;
            while (/[A-Za-z0-9_$]/.test(text[index + 1] ?? "")) {
                index += 1;
                word += text[index];
            }
...
function safeUntrustedText(bytes) {
    ...
    if (hasExecutableJavaScriptSemantics(text))
        return false;
    return true;
}
