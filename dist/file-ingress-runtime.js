...
function nextClamavRefreshDelay(database, signature, refreshed) { if (!refreshed)
    return clamavRefreshRetryMs; const expiresIn = Date.parse(signature?.updatedAt) + 24 * 60 * 60 * 1000 - clamavNow(database); return Math.max(1_000, Math.min(clamavRefreshIntervalMs, expiresIn + 1_000)); }
