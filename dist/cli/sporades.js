...
import { SPORADES_BASE_IMAGE, baseImageLabels, baseImageRuntimeUser, normaliseBaseImageUpdatePolicy, } from "../base-image.js";
...
async function startContainerSession(options) {
    ...
    ], options.projectDir, "Failed to start the container session.", "Check Docker is running, then retry `sporades deploy`.");
    const binding = {
        containerId,
        containerName,
        ...(sshAccess.enabled ? {
...
function isValidOpenSshPublicKeyBlob(keyType, blob) {
    ...
    // Certificate blobs carry additional structured fields; checking the embedded
    // OpenSSH key type and non-empty payload still rejects junk and type mismatch.
