#!/usr/bin/env node
import { assertNoPreservedFileAttempt, attemptJournalPath, beginPreservedFileAttempt, finishPreservedFileAttempt, readPreservedFileAttempt, recordPreservedFileAttempt, resolveDeployFiles, assertPreservedDeployFile, deployFileMounts, preparePreservedFiles, rollbackPreservedFiles, rethrowAfterDeployCleanup, localPreservedFileAccess, localPreservedFileAccessArgs, preservedFileAccessMatches, removeDeployFileSnapshot } from "../deploy-files.js";
...
import { commandError, errorDetails, readStdin, writeResult } from "./cli-support.js";
...
function parseCreateArgs(args) {
    ...
    const localTemplateDir = isLocalTemplateReference(template)
    ...
    for (let index = 0; index < args.length; index += 1) {
        ...
        switch (arg) {
            ...
            case "--json":
                ...
                break;
            default:
                throw commandError(`Unknown flag: ${arg}`, "Use `sporades security --session dev|public-dev|container|hosted --json`.");
        }
    }
    if (!SECURITY_SESSIONS.has(session)) {
        throw commandError(`Invalid security session: ${session}`, "Use one of: dev, public-dev, container, hosted.");
    }
    return {
        session,
        json,
...
function parseAccessKeyOperatorArgs(args) {
        ...
        switch (arg) {
            ...
            case "--limit": {
                ...
                if (!Number.isInteger(limit) || limit < 1 || limit > 100)
                    throw commandError("Invalid Access-key list limit.", "Use `--limit` from 1 through 100.");
                break;
            }
            case "--status":
                status = readFlagValue(rest, ++index, arg);
                if (!["active", "expired", "revoked"].includes(status))
                    throw commandError("Invalid Access-key status filter.", "Use active, expired, or revoked.");
                break;
            case "--json":
                json = true;
                break;
            ...
            default:
                if (arg.startsWith("--"))
                    throw commandError(`Unknown flag: ${arg}`, "Use `sporades access-keys --help`.");
                positional.push(arg);
        }
    }
    ...
    if (session === "hosted") {
        if (!hostAlias || !subname)
            throw commandError("Hosted Access-key operation requires a Host and Capsule.", "Pass `--host <alias> --subname <name>`.");
        validateHostAlias(hostAlias);
    ...
    if (ownerCommand) {
        if (!userId || positional.length)
            throw commandError("Access-key owner ID is required.", `Use \`sporades access-keys ${subcommand} --user-id <user-id>\`.`);
    }
...
async function manageOperatorAccessKeys(options) {
    ...
    if (options.session === "dev") {
        ...
        const running = runDocker(["inspect", "--format", "{{.State.Running}}", binding.containerId], options.projectDir, "Unable to inspect the local Container session.", "Check Docker and retry the Access-key operation.");
        if (running !== "true")
    ...
    if (options.json)
        writeResult(envelope, !envelope.ok);
    else if (!envelope.ok)
        throw commandError(envelope.error.message, envelope.error.hint, envelope.error);
    else
    ...
    for (let index = 0; index < args.length; index += 1) {
        ...
        switch (arg) {
            ...
            case "--json":
                ...
                break;
            default:
                throw commandError(`Unknown flag: ${arg}`, "Use `sporades doctor --session dev|public-dev|container|hosted --strict --json`.");
        }
    }
...
function parseEnvArgs(args) {
    ...
    for (let index = 0; index < rest.length; index += 1) {
        ...
        switch (arg) {
            ...
            default:
                if (arg.startsWith("--")) {
                    throw commandError(`Unknown flag: ${arg}`, "Use `sporades env set`, `sporades env has`, `sporades env init`, `sporades env import`, `sporades env status`, `sporades env export`, or `sporades env reencrypt`.");
                }
                positional.push(arg);
        }
    }
    switch (subcommand) {
        ...
        default:
            throw commandError(`Unknown env command: ${subcommand ?? ""}`.trim(), "Use `sporades env set`, `sporades env has`, `sporades env init`, `sporades env import`, `sporades env status`, `sporades env export`, or `sporades env reencrypt`.");
    }
}
...
function parseHostArgs(args) {
    ...
    switch (subcommand) {
        case "add": {
            ...
            if (!alias) {
                throw commandError("Missing Host profile alias.", "Use `sporades host add <alias> --server <ssh-target> --domain <hosted-domain>`.");
            }
            if (extra.length > 0) {
        ...
        case "push":
            ...
            if (fallbackToPreviousRelease && !verify) {
                throw commandError("Release fallback requires verification.", "Use `sporades host push --verify --fallback-to-previous-release`.");
            }
        ...
        case "github": {
            ...
            if (area !== "workflow" || action !== "write") {
                throw commandError("Unknown GitHub Host command.", "Use `sporades host github workflow write --host <alias> --subname <capsule-subname>`.");
            }
            if (extra.length > 0) {
                throw commandError("Too many positional arguments.", "Use `sporades host github workflow write --host <alias> --subname <capsule-subname>`.");
            }
            if (!hostAlias) {
                throw commandError("Missing Host profile alias.", "Pass `--host <alias>`.");
            }
...
function parseProviderCredentialDocument(provider, value) {
    ...
    switch (provider) {
        case "google":
            return isLooseRecord(value.web) &&
                hasNonEmptyString(value.web, "client_id") &&
                hasNonEmptyString(value.web, "client_secret")
                ? { clientId: value.web.client_id, clientSecret: value.web.client_secret }
                : null;
        case "microsoft":
            return hasNonEmptyString(value, "clientId") &&
                hasNonEmptyString(value, "clientSecret") &&
                (value.tenant === undefined || hasNonEmptyString(value, "tenant"))
                ? { clientId: value.clientId, clientSecret: value.clientSecret, tenant: value.tenant ?? "common" }
                : null;
        case "apple":
            return hasNonEmptyString(value, "servicesId") &&
                ...
                : null;
        case "facebook":
            return hasNonEmptyString(value, "appId") &&
                hasNonEmptyString(value, "appSecret") &&
                (value.graphVersion === undefined || hasNonEmptyString(value, "graphVersion"))
...
function parseDbArgs(args) {
    ...
    switch (subcommand) {
        ...
        default:
            throw commandError(`Unknown db command: ${subcommand ?? ""}`.trim(), "Use `sporades db list`, `sporades db dump`, or `sporades db query <sql>`.");
    }
}
...
function readHostLogLineCount(value) {
    if (!/^\d+$/.test(value)) {
        throw commandError("Invalid Host log line count.", `Pass \`--lines <n>\` with a whole number between 1 and ${MAX_HOST_LOG_LINES}.`);
    }
...
function validateHostLogSource(source) {
    if (!HOST_LOG_SOURCES.has(source)) {
        throw commandError("Invalid Host log source.", "Use `sporades host logs [http|stdout|stderr] --host <alias> --subname <capsule-subname> -n <lines>`.");
    }
}
...
async function createProjectFromLocalTemplate(options) {
    const sourceDir = path.resolve(options.localTemplateDir);
    ...
    const relativeDestination = path.relative(sourceDir, projectDir);
...
async function readLocalTemplateIgnoreRules(sourceDir) {
    let contents = "";
    try {
        contents = await readFile(path.join(sourceDir, ".gitignore"), "utf8");
    }
    catch {
        return [];
    }
...
function shouldCopyLocalTemplatePath(sourceDir, sourcePath, rules) {
    ...
    if (first === ".git" || first === "node_modules" || first === ".sporades")
        return false;
    if (relative === ".env.sporades.server")
        return false;
    let ignored = false;
...
async function finalizeLocalTemplateProject(options, projectDir) {
    ...
    try {
        ...
        throw commandError("Local template must include valid package.json and sporades.json files.", "Add both metadata files to the template and try again.");
    }
    ...
    return {
        ...
        async shutdown() {
            ...
            if (failures.length === 1)
                throw failures[0];
            if (failures.length > 1)
                throw new AggregateError(failures, "Dev runtime and scanner shutdown both failed.");
        },
    };
}
    ...
    switch (options.subcommand) {
        ...
        case "as": {
            ...
            if (result.data.delivery) {
                ...
                process.stdout.write(`Delivered to ${result.data.delivery.clients} ${noun} for --client ${result.data.delivery.target}\n`);
            }
    ...
    switch (options.subcommand) {
        ...
        case "has": {
            ...
            const defined = envelope
                ...
                : Object.hasOwn(parseServerEnv(await readServerEnvFile(path.join(options.projectDir, ".env.sporades.server"))), options.name);
            if (options.json) {
        ...
        case "reencrypt": {
            ...
            const hostEnvelopePath = path.join(paths.hosts, options.subname
...
async function readPortableSealedServerEnvEnvelope(filePath) {
    ...
    try {
        ...
        if (errorDetails(error).code === "ENOENT") {
            throw commandError("Sealed Server env export file was not found.", "Pass `--file <path>` pointing at a `sporades env export` JSON file.");
        }
        throw commandError("Invalid Sealed Server env export file.", "Pass a JSON file created by `sporades env export`.");
    }
    if (!envelope ||
    ...
    if (JSON.stringify(envelope).includes("PRIVATE KEY") || Object.hasOwn(envelope, "privateKey")) {
        throw commandError("Invalid Sealed Server env export file.", "Sealed envelope imports must not contain private keys.");
    }
    return envelope;
}
...
async function startContainerSession(options) {
    ...
    if (existingBinding?.containerId && !existingContainer && !options.force) {
        await discardPublicTree(bundle.staticFiles.publicTree).catch(() => { });
        throw commandError("The existing Container binding is stale.", "Retry with `sporades deploy --force`; through npm, use `npm run deploy -- --force`.");
    }
    ...
    try {
        ...
        await rethrowAfterDeployCleanup(error, [
            ...
            async () => { if (seedsRemoved && snapshotRemoved)
    ...
    try {
        ...
        for (const relative of new Set([...activePreserved, ...previouslyPreserved])) {
            const target = await assertPreservedDeployFile(preservedRoot, relative).catch((error) => {
                if (error.code === "ENOENT" && !activePreserved.has(relative))
                    return null;
                throw error;
            });
            if (!target)
        ...
        containerId = runDocker(dockerRunArgs, options.projectDir, "Failed to start the container session.", "Check Docker is running, then retry `sporades deploy`.");
        const candidateContainer = inspectDockerContainer(options.projectDir, containerId);
        candidateOwnershipProven = Boolean(candidateContainer?.Config?.Labels?.["com.sporades.container-transaction"] === containerTransactionToken
        ...
        if (committedConsumer) {
            try {
                await restorePublicTreeConsumer(bundle.buildDir, "container", previousConsumer, { token: committedConsumer.token, identity: committedConsumer.identity });
            }
            catch {
        ...
        try {
            if (existingBinding)
                await replaceContainerBinding(bindingPath, existingBinding);
            else
...
function inspectDockerContainer(projectDir, containerId) {
    const output = runDocker(["inspect", "--format", "{{json .}}", containerId], projectDir, "Failed to inspect the local Container session.", "Check Docker is running and the bound container still exists. If it was removed manually, run `sporades deploy remove` and deploy again.");
...
async function printLogs(options) {
    const result = (await tryFetchInspectionJson(options, options.subcommand === "tail" ? "/__sporades/debug/logs/tail" : "/__sporades/debug/logs")) ?? readContainerLogs(options);
    if (options.json) {
...
async function fetchInspectionDatabase(options) {
    return ((await tryFetchInspectionJson(options, options.subcommand === "query" ? "/__sporades/debug/db/query" : `/__sporades/debug/db/${options.subcommand}`, options.subcommand === "query"
        ? {
            method: "POST",
...
async function readDevSession(projectDir) {
    ...
    const raw = await readRequiredFile(sessionPath, "No running Sporades dev session found.", "Start one with `sporades dev` from this project, then retry the command.");
    try {
        return JSON.parse(raw);
    }
    catch {
...
function resolveLocalContainerTarget(options) {
    ...
    if (!binding?.containerId) {
        throw commandError("No running Sporades session found.", "Start `sporades dev`, run `sporades deploy`, or pass `--port <number>` for a running local Container session.");
    }
...
async function readContainerBinding(bindingPath) {
    try {
        ...
        if (error instanceof SyntaxError) {
            throw commandError("Invalid container binding metadata.", "Delete .sporades/binding.json or fix its JSON, then retry `sporades deploy`.");
        }
        throw error;
    }
}
async function readRemoteBinding(projectDir) {
    try {
        ...
        if (error instanceof SyntaxError) {
            throw commandError("Invalid project remote binding metadata.", "Delete .sporades/remote-binding.json or fix its JSON, then retry the command.");
        }
        throw error;
    }
}
async function resolveHostPushTarget(config, options) {
    ...
    if (!subname) {
        throw commandError("No Hosted Capsule binding found.", "Run `sporades host register <subname> --host <alias>` or pass `--host <alias> --subname <subname>`.");
    }
    validateCapsuleSubname(subname);
...
async function readHostConfig() {
    try {
        ...
        if (error instanceof SyntaxError) {
            throw commandError("Invalid Host profile configuration.", "Fix or delete the Sporades Host profile config, then retry the command.");
        }
        throw error;
    }
}
...
function hostConfigPath() {
    const configDir = process.env.SPORADES_CONFIG_DIR ??
...
function normaliseHostProfiles(value) {
    ...
    return Object.fromEntries(Object.entries(value).map(([alias, profile]) => [
        alias,
        {
            ...profile,
...
function resolveHostProfile(config, explicitAlias) {
    ...
    if (!alias) {
        throw commandError("No current Host profile selected.", "Run `sporades host use <alias>` or pass `--host <alias>`.");
    }
...
function requireHostProfile(config, alias) {
    ...
    if (!profile) {
        throw commandError(`Unknown Host profile alias: ${alias}`, `Add it with \`sporades host add ${alias} --server <ssh-target> --domain <hosted-domain>\`.`);
    }
    return profile;
}
...
async function prepareHostPushSealedServerEnv(options) {
    ...
    if (!envelope) {
        ...
        if (Object.keys(legacyValues).length > 0) {
            throw commandError("Hosted Capsule push requires Sealed Server env.", "Run `sporades env import --file .env.sporades.server --json` explicitly, then retry `sporades host push`.", {
...
function missingLocalSealedServerEnvSourceError(details = {}) {
    return commandError("Local Sealed Server env source values are unavailable.", "Restore the local Sealed Server env private key, or run `sporades env import --file .env.sporades.server --json` explicitly from source-of-truth values, then retry. Legacy Server env files are imported only by that explicit command.", {
...
async function readHostedCapsuleSealedEnvPublicKey(alias, profile, subname, projectDir) {
    ...
    if (!sealedServerEnv?.publicKey || !sealedServerEnv?.publicKeyFingerprint) {
        throw commandError("Hosted Capsule Sealed Server env public key is unavailable.", `Run \`sporades host register ${subname} --host ${alias} --json\` or inspect \`sporades host list --host ${alias} --json\`, then retry.`, {
            capsule: {
                subname,
                domain: profile.domain,
...
async function createHostReleaseArchive(options) {
    ...
    const requiredInspectors = [...new Set(Object.values(capsuleDefinition?.endpoints ?? {})
            .flatMap((endpoint) => endpoint?.options?.body?.multipart?.inspection?.requiredInspectors ?? []))]
    ...
    if (sealedServerEnv) {
        await writeFile(path.join(packageDir, ".sporades", "sealed-server-env", "server-env.sealed.json"), `${JSON.stringify(sealedServerEnv.envelope, null, 2)}\n`);
    }
    ...
    if (result.error || result.status !== 0) {
        throw commandError("Failed to package Hosted Capsule release.", "Check that tar is available and the Capsule runtime files are readable, then retry `sporades host push`.");
    }
...
async function listHostedPublicFiles(root, directory = root) {
    ...
    for (const entry of await readdir(directory, { withFileTypes: true })) {
        ...
        if (entry.isDirectory())
            files.push(...await listHostedPublicFiles(root, entryPath));
        else if (entry.isFile())
            files.push(`public/${path.relative(root, entryPath).split(path.sep).join("/")}`);
        else
            throw commandError("Invalid Hosted Capsule public tree.", "Rebuild a normalized public tree containing regular files only.");
    }
    return files.sort();
}
...
function uploadHostReleaseArchive(options) {
    ...
    if (result.error || result.status !== 0) {
        throw commandError("Failed to upload Hosted Capsule release archive.", "Check the Host profile SSH target, network connectivity, SSH key access, and remote incoming directory.");
    }
}
...
function formatHostedCapsuleList(data, profile) {
    ...
    const widths = Object.fromEntries(keys.map((key) => [key, Math.max(headers[key].length, ...rows.map((row) => row[key].length))]));
    const line = (row) => [row.subname, row.hostedUrl, row.registry, row.release, row.docker]
        .map((value, index) => {
...
function formatHostedCapsuleReleases(data) {
    ...
    const widths = Object.fromEntries(keys.map((key) => [key, Math.max(headers[key].length, ...rows.map((row) => row[key].length))]));
    const line = (row) => [row.id, row.state, row.current, row.createdAt]
        .map((value, index) => {
        const key = keys[index];
...
function upgradeHostHelper(options) {
    ...
    if (chmod.error || chmod.status !== 0) {
        throw commandError("Failed to activate the Host helper upgrade.", "Check the Host profile SSH target, SSH key access, and remote root permissions.");
    }
    return {
        ok: true,
        data: {
...
async function writeGithubAutodeployWorkflow(options) {
    ...
    if (relativeFile === ".." || relativeFile.startsWith(`..${path.sep}`) || path.isAbsolute(relativeFile)) {
        throw commandError("Invalid GitHub workflow file path.", "Pass a relative path inside the project, such as `.github/workflows/sporades-autodeploy.yml`.");
    }
    const github = {
    ...
    try {
        ...
        if (!options.force) {
            throw commandError("GitHub Actions workflow already exists.", "Pass `--force` to overwrite it, or choose another path with `--file <path>`.");
        }
    }
    catch (error) {
        if (errorDetails(error).code !== "ENOENT") {
...
function validateHostAlias(alias) {
    if (!/^[A-Za-z0-9][A-Za-z0-9._-]*$/.test(alias)) {
        throw commandError("Invalid Host profile alias.", "Use letters, numbers, dots, underscores, or dashes, starting with a letter or number.");
    }
}
function validateHostedDomain(domain) {
    ...
    if (!valid) {
        throw commandError("Invalid Hosted domain.", "Pass a DNS domain such as `example.com` without a scheme, path, or wildcard.");
    }
}
function validateHostRemoteRoot(remoteRoot) {
    const segments = remoteRoot.split("/").filter(Boolean);
    const valid = remoteRoot.startsWith("/") &&
        remoteRoot !== "/" &&
...
function validateHostTlsMode(tlsMode) {
    if (!HOST_TLS_MODES.has(tlsMode)) {
        throw commandError("Invalid Host TLS mode.", "Use `--tls automatic` for Caddy-managed certificates or `--tls cloudflare-origin` for preinstalled Cloudflare origin certificates.");
    }
}
function validateHostReleaseId(releaseId) {
    if (!/^\d{8}T\d{6}Z-[a-f0-9]{8}$/.test(releaseId)) {
        throw commandError("Invalid Hosted Capsule release ID.", "Use a recorded release ID from `sporades host releases <subname> --json`.");
    }
}
...
function validateCapsuleSubname(subname) {
    if (!/^[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?$/.test(subname)) {
        throw commandError("Invalid Capsule subname.", "Use a lowercase DNS-safe label such as `notes` or `team-notes`.");
    }
    if (RESERVED_CAPSULE_SUBNAMES.has(subname)) {
        throw commandError("Reserved Capsule subname.", "Choose a Capsule subname other than www, api, admin, root, or host.");
    }
}
function validateRemoteHelperAction(action) {
    if (!/^[a-z][a-z0-9.-]*$/.test(action)) {
        throw commandError("Invalid remote Host helper action.", "Use a lowercase action name such as `contract.echo`.");
    }
}
...
// Repair one preserved file's owner, runtime group, and mode through the Docker
// helper only when the current inode differs from the bound runtime's policy.
// The helper reports the inode it actually opened before mutation, even on a
// later chmod/chown failure. Docker's inode namespace may differ from the CLI
// host, so rollback reuses that same namespace via the recorded identity.
async function ensureLocalPreservedFileAccess(projectDir, target, localUser, fileUser, failure, record) {
    const info = await lstat(target);
    if (preservedFileAccessMatches(info, localPreservedFileAccess(localUser, fileUser)))
        return;
    const access = spawnSync("docker", localPreservedFileAccessArgs(target, localUser, fileUser, SPORADES_BASE_IMAGE.image), { cwd: projectDir, encoding: "utf8" });
    if (access.stdout?.trim())
        record?.push({ host: target, ...JSON.parse(access.stdout) });
    if (access.status !== 0)
        throw commandError(failure.message, failure.hint);
}
...
async function assertNoLocalDeployFileAttempt(options, action) {
    try {
        ...
        throw commandError(`Interrupted deploy.files attempt requires recovery before \`sporades deploy ${action}\`.`, "Run `sporades deploy reconcile` to settle the interrupted deployment, then retry.", { journal: attemptJournalPath(localPreservedFilesRoot(options)), cause: errorDetails(error).message });
    }
}
...
async function reconcileLocalContainerSession(options) {
    ...
    if (committed) {
        ...
        if (typeof candidate?.transaction === "string" && /^[a-f0-9]{32}$/.test(candidate.transaction)) {
            const listed = runDockerCleanup(["ps", "--all", "--quiet", "--filter", `label=com.sporades.container-transaction=${candidate.transaction}`], options.projectDir, "Failed to locate the interrupted candidate Container.", "Check Docker is running, then retry `sporades deploy reconcile`.");
            for (const containerId of listed.split(/\s+/).filter(Boolean)) {
                runDockerCleanup(["rm", "-f", containerId], options.projectDir, "Failed to remove the interrupted candidate Container.", "Retry `sporades deploy reconcile` after Docker can remove the candidate Container.", true);
        ...
        if (previous?.rollbackName && previous.name && typeof previous.containerId === "string" && binding?.containerId === previous.containerId) {
            ...
            if (staged && String(staged.Name ?? "").replace(/^\//, "") === previous.rollbackName) {
                runDocker(["rename", previous.rollbackName, previous.name], options.projectDir, "Failed to restore the previous Container name.", "Retry `sporades deploy reconcile` after Docker can rename the bound Container.");
...
async function removeLocalContainerSession(options) {
    ...
    if (currentConsumer || bindingExpectation) {
        ...
        claimedConsumer = await writePublicTreeConsumer(buildDir, "container", path.join(buildDir, ".public-trees", currentConsumer.tree), currentConsumer.identity, bindingExpectation);
    }
    try {
        ...
        for (const file of resolveDeployFiles(binding.deployFiles).filter((entry) => entry.update === "preserve")) {
            const target = await assertPreservedDeployFile(localPreservedFilesRoot(options), file.path)
        ...
        if (claimedConsumer && currentConsumer) {
            await restorePublicTreeConsumer(buildDir, "container", currentConsumer, { token: claimedConsumer.token, identity: claimedConsumer.identity }).catch(() => { });
        }
        throw error;
    }
...
function removeSporadesOwnedCapsuleImages(capsuleServices, projectDir) {
    ...
    if (images.size > 0) {
        runDocker(["rmi", ...images], projectDir, "Failed to remove Sporades-owned Capsule images.", "Check Docker is running, then retry the reset command.");
    }
    return [...images];
}
...
async function waitForCapsuleDatabaseService(capsuleServices, projectDir) {
    ...
    const error = commandError("Capsule database service did not become ready.", "Run `docker compose -f .sporades/compose/capsule-services.compose.yml ps` and inspect the service logs.");
    error.diagnostics = diagnostics;
    throw error;
}
...
function capsuleServiceStatus(capsuleServices, projectDir, serviceName) {
    const output = runDocker(["compose", "-f", capsuleServices.path, "ps", "--format", "json", serviceName], projectDir, "Failed to inspect Capsule service readiness.", "Check Docker is running and supports `docker compose ps --format json`, then retry the command.");
...
function ensureLocalBaseImage(cwd) {
    ...
    try {
        ...
        throw commandError("Unable to prepare the Sporades Base image.", `Check Docker can pull ${SPORADES_BASE_IMAGE.image}, then retry \`sporades deploy\`.`);
    }
    ...
    if (build.status !== 0) {
        throw commandError("Unable to prepare the Sporades Base image.", "Check Docker is running and can build the local Sporades Base image, then retry `sporades deploy`.");
    }
}
...
function verifyContainerReplacementOwnership(binding, consumer, expectedContainerName) {
    if (binding === null && consumer === null)
        return;
    const owned = Boolean(binding
        && consumer
    ...
    if (!owned) {
        throw commandError("Container replacement ownership could not be verified.", "Preserve the current Container state and reconcile its binding and public-tree consumer before retrying deployment.");
    }
}
        ...
        try {
            ...
            return async () => {
                ...
                if (owner?.token !== token)
                    throw commandError("Container lifecycle lock ownership changed.", "Preserve the successor lifecycle lock.");
...
async function prepareRuntimeDataPath(targetPath) {
    ...
    if (stats.isSymbolicLink()) {
        throw commandError("Container session data path contains a symbolic link.", `Remove the symbolic link at ${targetPath}, then retry \`sporades deploy\`.`);
    }
    if (stats.isDirectory()) {
...
//# sourceMappingURL=sporades.js.map