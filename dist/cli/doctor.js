...
function containerRuntimePolicyCheck(container) {
    ...
    const loopbackOnlyPublishedPorts = Object.values(ports).every((entries) => !Array.isArray(entries) || entries.length === 0 || entries.every((entry) => entry?.HostIp === "127.0.0.1"));
    const ok = Boolean(baseImageLabels.name &&
    ...
    const checks = [
        {
            ...
            details: {
                declared: true,
                services: Object.fromEntries(Object.entries(capsuleServices.services).map(([name, service]) => [
                    name,
                    {
                        kind: service.kind,
...
async function generatedComposeCheck(capsuleServices, projectDir, scope) {
    ...
    const missingServices = Object.values(capsuleServices.services)
        .filter((service) => !raw.includes(service.name))
...
async function capsuleServicesRuntimeStateCheck(capsuleServices, projectDir, scope) {
    ...
    const unhealthy = Object.values(services).some((service) => service.status && service.status !== "running" || service.health === "unhealthy" || !service.network.exists || !service.volume.exists);
