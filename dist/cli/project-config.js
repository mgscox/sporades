...
export function validateClientConfig(client) {
    ...
    if (client.framework !== undefined && !CLIENT_FRAMEWORK_SET.has(client.framework)) {
        throw commandError(`Unsupported framework: ${client.framework}`, "Use one of: react, preact, inferno, lit, solid, vue, svelte, vanilla.");
    }
    if (client.toolchain !== undefined && !CLIENT_TOOLCHAIN_SET.has(client.toolchain)) {
    ...
    if (client.framework !== undefined && client.toolchain !== undefined && !supportsClientCapability(client.framework, client.toolchain))
        throw commandError(`Unsupported client framework/toolchain combination: ${client.framework}/${client.toolchain}`, "Choose an admitted pair from the client capability matrix in docs/user-guide.md.");
}
