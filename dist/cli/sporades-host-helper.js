...
async function dockerRunArgs(lifecycle, releaseId) {
    ...
    args.push("--volume", formatMount(lifecycle.mounts.data), "--workdir", "/app", "--env", "PORT=4000", "--env", "SPORADES_LOG_STDOUT=1", "--env", `SPORADES_RELEASE_ID=${releaseId}`);
...
function validateHostLogsRequest(request) {
    ...
    if (!Number.isInteger(lines) || lines < 1 || lines > MAX_HOST_LOG_LINES) {
        throw helperError("Invalid Host log line count.", `Pass \`--lines <n>\` with a whole number between 1 and ${MAX_HOST_LOG_LINES}.`);
    }
}
