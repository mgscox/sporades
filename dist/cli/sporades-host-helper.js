...
async function prepareHostedRuntimeFileAccess(target, mode, failure) {
    ...
    try {
        ...
        if (!identity.isFile() || identity.nlink !== 1)
            throw helperError(failure.message, failure.hint);
...
async function preparePreservedReleaseFiles(request, recordedRelease) {
    ...
    if (!recordedRelease) {
        ...
        if (interrupted)
            throw helperError("Current Hosted release is not recorded.", "Run `sporades host reconcile <subname>` to settle the interrupted release install before starting the Capsule.");
        return;
    }
    if (interrupted) {
...
async function reconcileReleaseAttempt(request) {
    ...
    const data = { capsule: capsuleData(request, lifecycle), journal, reconciled: false, committed: null, release: null, actions: [] };
    if (!attempt) {
        writeEnvelope({ ok: true, data, error: null });
        return;
    }
