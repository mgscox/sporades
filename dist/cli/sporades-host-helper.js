...
function normaliseLifecycle(request, registryRecord = null) {
    ...
    return {
        ...
        routes: {
            running: withRouteAccessLog(provided.routes?.running ?? {
            ...
            unavailable: withRouteAccessLog(provided.routes?.unavailable ?? {
                hostname: `${subname}.${domain}`,
...
async function provisionCaddyAccessLog(request, bootstrap) {
    ...
    if (chown.error || chown.status !== 0) {
        throw helperError("Failed to provision the Caddy access log for the service user.", `Ensure the Host helper runs with permission to chown ${logDirectory} and ${logFile}, then rerun \`sporades host bootstrap --host ${request.host.alias}\`.`);
    }
    return {
        file: logFile,
...
async function provisionRouteLogFile(route) {
    ...
    const chown = spawnSync("chown", [`${caddyUser.uid}:${caddyUser.gid}`, logDirectory, logFile], { encoding: "utf8" });
    if (chown.error || chown.status !== 0) {
        throw helperError("Failed to provision the Hosted Capsule HTTP log for the Caddy service user.", `Ensure the Host helper runs with permission to chown ${logDirectory} and ${logFile}, then retry the Hosted Capsule command.`);
    }
}
...
async function acquireManagedRouteOsLock(lockFile, timeoutMs) {
    ...
    await new Promise((resolve, reject) => {
        ...
        child.once("error", () => fail(helperError("Hosted Capsule route locking is unavailable.", "Install Python 3 with POSIX fcntl support on the Host server, then retry the lifecycle command.")));
        ...
        child.once("close", (code) => {
            ...
            settled = true;
            if (code === 75) {
                reject(helperError("Hosted Capsule route is locked.", "Wait for the other Host server operation to finish, then retry the lifecycle command."));
                return;
            }
            reject(helperError("Hosted Capsule route locking is unavailable.", "Install Python 3 with POSIX fcntl support on the Host server, then retry the lifecycle command."));
        });
    });
    let released = false;
    return {
        release: async () => {
            ...
            if (closed.code !== 0 || closed.signal !== null) {
                throw helperError("Hosted Capsule route lock did not release cleanly.", "Check the Host server Python runtime, then retry the lifecycle command.");
            }
        },
    };
}
...
async function prepareWritableDataPath(targetPath) {
    ...
    if (stats.isSymbolicLink()) {
        throw helperError("Hosted Capsule data path contains a symbolic link.", `Remove the symbolic link at ${targetPath}, then retry the Host lifecycle command.`);
    }
