...
export function validateEmailWebhooksConfig(webhooks) {
    if (webhooks === undefined)
        return undefined;
    const webhooksData = captureMailConfigData(webhooks, ["mailjet", "smtp2go", "postmark", "mailgun"], "Invalid email webhook configuration.", "Configure only supported providers under `mail.webhooks`.");
    const result = {};
    for (const [provider, defaultPath, defaultSecretEnv] of [
        ...
        if (input === undefined)
            continue;
        const data = captureMailConfigData(input, ["enabled", "path", "secretEnv"], `Invalid ${provider} webhook configuration.`, `Configure \`mail.webhooks.${provider}\` with optional enabled, path, and secretEnv values.`);
        ...
        if (!sameOriginWebhookPath(path) || runtimeOwnedHttpPath(path)) {
            invalidMailConfig(`Invalid ${provider} webhook path.`, `Set \`mail.webhooks.${provider}.path\` to a same-origin absolute path outside Sporades runtime-owned HTTP namespaces.`);
        }
        if (!isServerEnvReference(secretEnv)) {
            invalidMailConfig(`Invalid ${provider} webhook Server env reference.`, `Set \`mail.webhooks.${provider}.secretEnv\` to an uppercase Server env key without the reserved \`SPORADES_\` prefix.`);
        }
        result[provider] = { enabled, path, secretEnv };
    }
    return result;
}
//# sourceMappingURL=email-events-config.js.map