...
export function publicAccessKeyManagementError(error) {
    if (!PUBLIC_ACCESS_KEY_MANAGEMENT_ERROR_CODES.has(error?.code))
        return null;
    return {
        code: error.code,
...
export function createCurrentUserAccessKeysApi(database, contextGetter) {
    return {
        ...
        async list(options = {}) {
            ...
            const rows = await withAccessKeyTransaction(database, (adapter) => adapter.listAccessKeyRecordsForOwner(context.auth.userId, {
                sessionToken: accessKeyOwnerSessionTokens.get(context),
                sessionValidationTime: () => database.clock.now().toISOString(),
            }));
            if (!Array.isArray(rows) && rows?.status === "session-ineligible")
                throwAccessKeyOwnerSessionInactive("listing");
        ...
        async revoke(id) {
            ...
            if (outcome?.status === "session-ineligible")
                throwAccessKeyOwnerSessionInactive("revoking");
            if (!outcome)
                throw accessKeyNotFoundError();
        ...
        async rotate(id, options) {
            ...
            for (let attempt = 0; attempt < 5; attempt += 1) {
                ...
                if (outcome.status === "selector-conflict")
                    continue;
                if (outcome.status === "session-ineligible")
                    throwAccessKeyOwnerSessionInactive("rotating");
                if (outcome.status === "not-found")
                    throw accessKeyNotFoundError();
                if (outcome.status === "not-active")
                    throw commandError("Access key is not active.", "Issue a new Access key.", "ACCESS_KEY_NOT_ACTIVE");
                if (outcome.status === "revision-conflict")
                    throw commandError("Access-key revision changed.", "Refresh the key list and retry rotation.", "ACCESS_KEY_REVISION_CONFLICT");
        ...
        async delete(id) {
            ...
            if (outcome.status === "session-ineligible")
                throwAccessKeyOwnerSessionInactive("deleting");
            if (outcome.status === "not-found")
                throw accessKeyNotFoundError();
...
export function createPrivilegedAccessKeysApi(database, contextGetter, transactionDatabaseFactory) {
...
async function runPrivilegedAccessKeyOperation(database, context, operation, target, callback, options = {}) {
    ...
    if (!options.atomicWrite || database.__transactionActive)
...
async function emitOwnerAccessKeyAudit(database, event, context, accessKey) {
    ...
    if (database.__transactionActive) {
        ...
        if (events.length === 0)
            accessKeyLifecycleAuditEventsByContext.set(context, events);
        events.push(input);
        return;
    }
    try {
...
export function transferAccessKeyRuntimeState(previousContext, nextContext) {
