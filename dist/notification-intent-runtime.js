...
export async function ensureNotificationIntentStorage(adapter) {
    if (adapter.engine === "postgres") {
        const bootstrap = adapter[Symbol.for("sporades.database.resourceBootstrapMechanics")];
        if (typeof bootstrap !== "function")
            throw Object.assign(new Error("Resource operation could not complete."), { code: "RESOURCE_ADAPTER_UNSUPPORTED" });
        await bootstrap();
        return;
    }
...
export async function notificationIntentStorageExists(adapter) {
    if (adapter.engine === "postgres") {
...
function exactPlainObject(input, allowed) {
    return input && Object.getPrototypeOf(input) === Object.prototype
        && Object.getOwnPropertySymbols(input).length === 0
        && Object.values(Object.getOwnPropertyDescriptors(input)).every((descriptor) => Object.hasOwn(descriptor, "value"))
        && Object.keys(input).sort().join(",") === allowed.sort().join(",");
}
export async function stageNotificationIntent(adapter, database, identity, input, canonicalJson) {
    ...
    if (!Array.isArray(input.to) || input.to.length < 1 || input.to.length > 100 || input.to.some((value) => typeof value !== "string"))
        invalid();
    if (typeof input.subject !== "string" || typeof input.text !== "string" || (input.html !== undefined && typeof input.html !== "string"))
        invalid();
    if (input.text.length === 0 && (input.html === undefined || input.html.length === 0))
        invalid();
    let normalized;
    try {
    ...
    if (normalized.to.length !== input.to.length)
        invalid();
    const payload = { id: input.id, to: normalized.to.map((entry) => entry.email), subject: normalized.subject,
    ...
    if (existing) {
        ...
        if (digest !== payloadDigest)
            throw Object.assign(new Error("Resource operation could not complete."), { code: "RESOURCE_OPERATION_CONFLICT" });
...
export async function readNotificationIntentStatuses(adapter, identity, intentIds) {
    const intents = [];
    for (const intentId of intentIds) {
        ...
        const publicRecipients = recipients.map((row) => ({
        ...
        const pending = publicRecipients.some((row) => ["accepted", "submitting", "unknown", "retry-wait"].includes(row.state));
        const allAcknowledged = publicRecipients.length > 0 && publicRecipients.every((row) => row.state === "acknowledged");
        intents.push({ id: intentId, state: pending ? "pending" : allAcknowledged ? "acknowledged" : "rejected", recipients: publicRecipients });
    }
    return intents;
}
export function notificationRetryDelay(attemptNumber) {
...
async function recoverExpiredReservations(database, now) {
    await database.adapter.withTransaction(async (tx) => {
        ...
        for (const row of expired) {
            ...
            const token = row.currentAttemptToken ?? row.currentattempttoken;
...
async function moveUnknownRecipientsToRetryWait(database) {
    await database.adapter.prepare(sql(database.adapter, "UPDATE [sporades_notification_recipients] SET [state]='retry-wait' WHERE [state]='unknown'")).run();
}
async function reserveDueRecipient(database, now) {
...
export async function runNotificationIntentDeliveryPass(database) {
...
async function nextWakeAt(database) {
...
export function startNotificationIntentWorker(database) {
    ...
    const run = async () => {
        if (database.__notificationIntentStopped || database.__notificationIntentWorkerPromise)
            return;
        const work = (async () => {
            try {
        ...
        try {
            ...
            if (database.__notificationIntentWorkerPromise === work)
                database.__notificationIntentWorkerPromise = null;
        }
    };
    return run();
}
export function stopNotificationIntentWorker(database) {
    database.__notificationIntentStopped = true;
    if (database.__notificationIntentTimer) {
        if (database.__notificationIntentNativeTimer)
            clearTimeout(database.__notificationIntentTimer);
        else
            database.clock.clearTimer(database.__notificationIntentTimer);
    }
    database.__notificationIntentTimer = null;
    database.__notificationIntentNativeTimer = false;
    return database.__notificationIntentWorkerPromise;
}
//# sourceMappingURL=notification-intent-runtime.js.map