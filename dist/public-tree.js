...
export async function replacePublicTree(buildDir, files) {
    ...
    try {
        ...
        try {
            ...
            if (previousMoved)
                await rename(previousDir, publicDir);
            throw error;
        }
        if (previousMoved)
...
export async function readPublicAsset(root, rawPathname) {
    ...
    if (!stats.isFile() || stats.size > PUBLIC_TREE_LIMITS.fileBytes)
        return null;
    return {
