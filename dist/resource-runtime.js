...
async function upgradeFoldedResourceColumns(adapter, table, columns) {
    ...
    const existing = new Set((await adapter.prepare(adapter.dialect.sql("SELECT [column_name] FROM [information_schema].[columns] WHERE [table_schema]=current_schema() AND [table_name]=?")).all(table)).map((row) => row.column_name));
...
export function bindOuterResources(database, context, hooks) {
    ...
    const outerAborted = new Promise((_, reject) => { rejectOuterAbort = reject; });
    void outerAborted.catch(() => { });
    const parentDb = context.db;
    const parentJobs = context.jobs;
    const pending = new Set();
    const executions = new Set();
    const normalizeStorageError = (error) => error?.code === "RESOURCE_BUSY" || error?.code === "SQLITE_BUSY" || error?.errcode === 5 || error?.errcode === 6
    ...
    const execute = async (options, callback, status) => {
        ...
        try {
            ...
            try {
                ...
                if (error?.errcode === 5 || error?.errcode === 6 || error?.code === "SQLITE_BUSY" || error?.code === "55P03" || error?.code === "57014")
                    throw resourceError("RESOURCE_BUSY");
            ...
            if (acquired || normalized?.code === "RESOURCE_STORAGE_ERROR")
