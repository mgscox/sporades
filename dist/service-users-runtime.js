import { accessKeyListPage, accessKeyNotFoundError, accessKeySummary, accessKeyVerifierDigest, createAccessKeySecret, markAccessKeySecretDisclosed, normalizeAccessKeyIssue, normalizeAccessKeyListOptions, throwAccessKeyIssueError, } from "./access-keys-runtime.js";
...
async function requireCurrentHumanSession(database, context, sessionToken) {
    ...
    await adapter.prepare(sql("UPDATE [sporades_auth_sessions] SET [token] = [token] WHERE [token] = ? AND [userId] = ?")).run(sessionToken, context.auth?.userId);
    const row = await adapter.prepare(sql("SELECT [s].[token] FROM [sporades_auth_sessions] [s] " +
        ...
        "AND [u].[isAuthenticated] = 1 AND [u].[isGuest] = 0")).get(sessionToken, context.auth?.userId, database.clock.now().toISOString());
    if (!row) {
...
async function lockServiceUser(database, userId, requireActive = true) {
    ...
    const adapter = database.adapter;
    const sql = adapter.dialect.sql;
    const locked = await adapter.prepare(sql("UPDATE [sporades_auth_service_user_locks] SET [operationRevision] = [operationRevision] + 1 WHERE [userId] = ?")).run(userId);
    if (Number(locked?.changes ?? 0) !== 1) {
        throw serviceUserError("SERVICE_USER_NOT_FOUND", "Service User not found.", "Refresh the service-User list.");
    }
    const row = await adapter.prepare(sql("SELECT [id], [createdAt], [displayName], [email], [picture], [isAuthenticated], [isGuest], [provider], " +
        "[userKind], [lifecycleStatus], [disabledAt] FROM [sporades_auth_users] WHERE [id] = ?")).get(userId);
    if (!row || row.userKind !== "service" || (requireActive && row.lifecycleStatus !== "active")) {
        throw serviceUserError(requireActive ? "SERVICE_USER_NOT_ACTIVE" : "SERVICE_USER_NOT_FOUND", requireActive ? "Service User is not active." : "Service User not found.", requireActive ? "Use an active service User." : "Refresh the service-User list.");
    }
    return row;
}
    ...
    return {
        create(input) {
            return tracked(inContext(async (context) => {
                ...
                await database.adapter.prepare(sql("INSERT INTO [sporades_auth_users] " +
                    "([id], [createdAt], [displayName], [email], [picture], [isAuthenticated], [isGuest], [provider], [userKind], [lifecycleStatus], [disabledAt]) " +
                    "VALUES (?, ?, ?, NULL, NULL, 1, 0, 'service', 'service', 'active', NULL)")).run(id, createdAt, displayName);
        ...
        rotateAccessKey(userId, id, options) {
            return tracked(inContext(async (context) => {
                ...
                if (!options || typeof options !== "object" || Array.isArray(options)
                    ...
                    || !Number.isInteger(options.lifecycleRevision)
                    || options.lifecycleRevision < 1) {
                ...
                for (let attempt = 0; attempt < 5; attempt += 1) {
                    ...
                    if (outcome.status === "not-active")
                        throw serviceUserError("ACCESS_KEY_NOT_ACTIVE", "Access key is not active.", "Issue a new Access key.");
                    if (outcome.status === "revision-conflict")
                        throw serviceUserError("ACCESS_KEY_REVISION_CONFLICT", "Access-key revision changed.", "Refresh the key list and retry rotation.");
                    markAccessKeySecretDisclosed(context);
                    return {
        ...
        disable(userId) {
            return tracked(inContext(async () => {
                ...
                const sql = database.adapter.dialect.sql;
                const result = await database.adapter.prepare(sql("UPDATE [sporades_auth_users] SET [lifecycleStatus] = 'disabled', [disabledAt] = ?, " +
                    "[isAuthenticated] = 0 WHERE [id] = ? AND [userKind] = 'service' AND [lifecycleStatus] = 'active'")).run(disabledAt, serviceUser.id);
...
//# sourceMappingURL=service-users-runtime.js.map