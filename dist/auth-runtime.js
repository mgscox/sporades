...
export async function unlinkCurrentAuthUser(database, context) {
    ...
    return runAccessKeyOwnerSecurityTransition(database, {
        ...
        await tx.prepare(tx.dialect.sql("UPDATE [sporades_auth_users] SET [email] = NULL, [isAuthenticated] = ?, [isGuest] = ?, [provider] = ? WHERE [id] = ?")).run(0, 1, "anonymous", auth.userId);
    });
}
...
function requireOwnerSecuritySessionContext(context) {
    ...
    if (!["query", "mutation", "endpoint", "message"].includes(context?.kind) || context?.credential?.kind !== "session") {
        throw commandError("Owner security changes require a linked Session.", "Sign in interactively and retry the owner operation.", "FORBIDDEN");
    }
    return auth;
}
...
export async function signInWithEmail(database, session, credentials) {
    ...
    return await withAuthTransaction(database, async (tx) => {
        ...
        await tx.prepare(tx.dialect.sql("UPDATE [sporades_auth_users] SET [provider] = 'email' WHERE [id] = ? AND [isAuthenticated] = 1 AND [isGuest] = 0")).run(auth.userId);
        return { ok: true, sessionToken: await rotateSessionOnAdapter(database, tx, session, auth.userId, "email"), auth };
    });
}
...
export async function beginOAuthSignIn(database, session, provider, options) {
    ...
    if (reauthentication?.purpose && adapter.reauthenticationFreshness !== "signed-auth-time")
        return { ok: false, error: { code: "REAUTHENTICATION_FAILED", message: "Reauthentication failed.", hint: "Verify the current linked identity and retry." } };
    let started;
    try {
...
// stay there.
export function createAnonymousAuthTables(sqlite, _authConfig = null) {
...
function createProviderIdentityTables(sqlite) {
    ...
    return chainMaybePromise([
        ...
        () => sqlite.exec(sql("CREATE INDEX IF NOT EXISTS [sporades_auth_identities_user_id] " +
...
function migrateAuthUserProviderLabels(sqlite) {
    return sqlite.exec(sqlite.dialect.sql("UPDATE [sporades_auth_users] SET [provider] = CASE " +
