...
async function oauthRegistrationKey(database, keyId = "active", adapter = database.adapter) {
    ...
    if (typeof row?.value !== "string" || !/^[A-Za-z0-9_-]{43}$/.test(row.value))
...
async function sealOAuthRegistration(database, value, binding) {
    ...
    const cipher = nodeCryptoModule.createCipheriv("aes-256-gcm", await oauthRegistrationKey(database), iv);
