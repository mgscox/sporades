...
export async function settleExhaustedTeamBillingCheckoutJob(transaction, handler, payloadJson, now) {
    if (handler !== TEAM_BILLING_CHECKOUT_JOB || typeof payloadJson !== "string")
        return;
    let payload;
    try {
    ...
    if (!operationId)
        return;
    await transaction.prepare(transaction.dialect.sql("UPDATE [sporades_team_billing_operations] SET [status] = 'failed', [safeFailureCode] = 'PROVIDER_REJECTED', [updatedAt] = ? " +
        "WHERE [id] = ? AND [kind] = 'checkout' AND [status] IN ('queued', 'running', 'retrying')")).run(now, operationId);
}
