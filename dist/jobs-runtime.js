...
export async function cleanupExpiredStripeEventPayloads(database, options = {}) {
    ...
    const unassigned = await adapter.prepare(sql("SELECT [id], [completedAt] FROM [sporades_jobs] WHERE [handler]=? AND [status]='succeeded' " +
        "AND [payloadRedactedAt] IS NULL AND [payloadRetentionUntil] IS NULL " +
        "ORDER BY [completedAt] ASC, [id] ASC LIMIT ?")).all(STRIPE_EVENT_JOB, batchSize);
    for (const row of unassigned) {
        ...
        if (deadline === null) {
            ...
            await adapter.prepare(sql("UPDATE [sporades_jobs] SET [payloadRetentionUntil]='' WHERE [id]=? AND [handler]=? " +
                ...
                "AND [claimToken] IS NULL AND [leaseExpiresAt] IS NULL")).run(row.id, STRIPE_EVENT_JOB);
            continue;
        }
        const changed = await adapter.prepare(sql("UPDATE [sporades_jobs] SET [payloadRetentionUntil]=? WHERE [id]=? AND [handler]=? " +
            ...
            "AND [payloadRetentionUntil] IS NULL AND [claimToken] IS NULL AND [leaseExpiresAt] IS NULL")).run(deadline, row.id, STRIPE_EVENT_JOB, row.completedAt);
    ...
    const due = await adapter.prepare(sql("SELECT [id], [completedAt], [payloadRetentionUntil] FROM [sporades_jobs] WHERE [handler]=? " +
        ...
        "AND [payloadRetentionUntil] <= ? ORDER BY [payloadRetentionUntil] ASC, [id] ASC LIMIT ?")).all(STRIPE_EVENT_JOB, nowIso, batchSize);
