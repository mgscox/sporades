...
export function stripeEventPayloadRetentionDeadline(settledAt) {
    if (!isCanonicalJobTimestamp(settledAt))
        return null;
...
export async function cleanupExpiredStripeEventPayloads(database, options = {}) {
    ...
    const due = await adapter.prepare(sql("SELECT [id], [completedAt], [payloadRetentionUntil] FROM [sporades_jobs] WHERE [handler]=? " +
        ...
        "AND [payloadRetentionUntil] <= ? ORDER BY [payloadRetentionUntil] ASC, [id] ASC LIMIT ?")).all(STRIPE_EVENT_JOB, nowIso, remaining);
    for (const row of due) {
        ...
        const changed = await adapter.prepare(sql("UPDATE [sporades_jobs] SET [payload]=?, [result]=NULL, [payloadRedactedAt]=? " +
            "WHERE [id]=? AND [handler]=? AND [status]='succeeded' AND [completedAt]=? " +
            "AND [payloadRedactedAt] IS NULL AND [payloadRetentionUntil]=? " +
            "AND [payloadRetentionUntil] <= ? AND [claimToken] IS NULL AND [leaseExpiresAt] IS NULL")).run(REDACTED_STRIPE_EVENT_PAYLOAD, nowIso, row.id, STRIPE_EVENT_JOB, row.completedAt, row.payloadRetentionUntil, nowIso);
    ...
    if (recheckIsWaiting)
        sentinelRecheckAt = sentinelMaintenance.recheckAt;
    const sentinelCandidates = remaining === 0 || recheckIsWaiting ? [] : await adapter.prepare(sql("SELECT [id], [completedAt] FROM [sporades_jobs] WHERE [handler]=? AND [status]='succeeded' " +
        "AND [payloadRedactedAt] IS NULL AND [payloadRetentionUntil]='' AND [id]>? " +
        "ORDER BY [id] ASC LIMIT ?")).all(STRIPE_EVENT_JOB, sentinelMaintenance.afterId, STRIPE_EVENT_PAYLOAD_CLEANUP_BATCH_SIZE + 1);
    ...
    for (const observed of sentinelCandidates.slice(0, STRIPE_EVENT_PAYLOAD_CLEANUP_BATCH_SIZE)) {
        ...
        while (row && remaining > 0) {
            ...
            if (deadline === null)
                break;
            const changed = await adapter.prepare(sql("UPDATE [sporades_jobs] SET [payloadRetentionUntil]=? WHERE [id]=? AND [handler]=? " +
                "AND [status]='succeeded' AND [completedAt]=? AND [payloadRedactedAt] IS NULL " +
                "AND [payloadRetentionUntil]='' AND [claimToken] IS NULL AND [leaseExpiresAt] IS NULL")).run(deadline, row.id, STRIPE_EVENT_JOB, row.completedAt);
            const mutations = Number(changed?.changes ?? 0);
            if (mutations > 0) {
            ...
            if (retried)
                break;
            row = await adapter.prepare(sql("SELECT [id], [completedAt] FROM [sporades_jobs] WHERE [id]=? AND [handler]=? " +
    ...
    if (remaining === 0 && processedSentinelCount === 0) {
        ...
        sentinelScanPending = recheckIsWaiting ? false : Boolean(await adapter.prepare(sql("SELECT [id] FROM [sporades_jobs] WHERE [handler]=? AND [status]='succeeded' " +
        ...
        const unresolvedSentinel = pageHasMore || Boolean(await adapter.prepare(sql("SELECT [id] FROM [sporades_jobs] WHERE [handler]=? AND [status]='succeeded' " +
            "AND [payloadRedactedAt] IS NULL AND [payloadRetentionUntil]='' LIMIT 1")).get(STRIPE_EVENT_JOB));
        ...
        const advanced = await adapter.prepare(sql("UPDATE [sporades] SET [value]=? WHERE [key]=? AND [value]=?")).run(nextMaintenanceValue, STRIPE_EVENT_PAYLOAD_SENTINEL_CURSOR_KEY, observedMaintenanceValue);
        if (Number(advanced?.changes ?? 0) === 0)
            sentinelScanPending = true;
        else {
        ...
        const unresolvedSentinel = Boolean(await adapter.prepare(sql("SELECT [id] FROM [sporades_jobs] WHERE [handler]=? AND [status]='succeeded' " +
            "AND [payloadRedactedAt] IS NULL AND [payloadRetentionUntil]='' LIMIT 1")).get(STRIPE_EVENT_JOB));
        ...
        const wrapped = await adapter.prepare(sql("UPDATE [sporades] SET [value]=? WHERE [key]=? AND [value]=?")).run(nextMaintenanceValue, STRIPE_EVENT_PAYLOAD_SENTINEL_CURSOR_KEY, observedMaintenanceValue);
        if (Number(wrapped?.changes ?? 0) === 0)
            sentinelScanPending = true;
        else {
            observedMaintenanceValue = nextMaintenanceValue;
    ...
    const unassigned = remaining === 0 ? [] : await adapter.prepare(sql("SELECT [id], [completedAt] FROM [sporades_jobs] WHERE [handler]=? AND [status]='succeeded' " +
        ...
        while (row && remaining > 0) {
            ...
            const changed = deadline === null
                ? await adapter.prepare(sql("UPDATE [sporades_jobs] SET [payloadRetentionUntil]=? WHERE [id]=? AND [handler]=? " +
                    ...
                    "AND [payloadRedactedAt] IS NULL AND [payloadRetentionUntil] IS NULL " +
                    "AND [claimToken] IS NULL AND [leaseExpiresAt] IS NULL")).run(unresolvedDeadline, row.id, STRIPE_EVENT_JOB, row.completedAt, row.completedAt)
                : await adapter.prepare(sql("UPDATE [sporades_jobs] SET [payloadRetentionUntil]=? WHERE [id]=? AND [handler]=? " +
                    "AND [status]='succeeded' AND ([completedAt]=? OR ([completedAt] IS NULL AND ? IS NULL)) " +
                    "AND [payloadRedactedAt] IS NULL AND [payloadRetentionUntil] IS NULL " +
                    "AND [claimToken] IS NULL AND [leaseExpiresAt] IS NULL")).run(deadline, row.id, STRIPE_EVENT_JOB, row.completedAt, row.completedAt);
            const mutations = Number(changed?.changes ?? 0);
            if (mutations > 0) {
                if (deadline === null)
                    classifiedCount += mutations;
                else
                    assignedCount += mutations;
                remaining -= mutations;
                break;
    ...
    const moreUnassigned = await adapter.prepare(sql("SELECT [id] FROM [sporades_jobs] WHERE [handler]=? AND [status]='succeeded' " +
        "AND [payloadRedactedAt] IS NULL AND [payloadRetentionUntil] IS NULL LIMIT 1")).get(STRIPE_EVENT_JOB);
    if (!sentinelScanPending && sentinelMaintenance.afterId === "" && sentinelRecheckAt === null && classifiedCount > 0) {
        ...
        const nextMaintenanceValue = serializeStripeEventPayloadSentinelMaintenance("", nextRecheckAt);
        const scheduled = await adapter.prepare(sql("UPDATE [sporades] SET [value]=? WHERE [key]=? AND [value]=?")).run(nextMaintenanceValue, STRIPE_EVENT_PAYLOAD_SENTINEL_CURSOR_KEY, observedMaintenanceValue);
        if (Number(scheduled?.changes ?? 0) === 0)
            sentinelScanPending = true;
        else
            sentinelRecheckAt = nextRecheckAt;
    }
    const next = await adapter.prepare(sql("SELECT [payloadRetentionUntil] FROM [sporades_jobs] WHERE [handler]=? AND [status]='succeeded' " +
...
function installStripeEventPayloadCleanupTimer(database, dueAt) {
    if (database.__stripeEventPayloadCleanupTimer)
        database.clock.clearTimer(database.__stripeEventPayloadCleanupTimer);
    ...
    database.__stripeEventPayloadCleanupTimer = database.clock.setTimer(async () => {
        ...
        if (!database.__jobStopped)
            await startStripeEventPayloadCleanup(database);
    }, Math.min(STRIPE_EVENT_PAYLOAD_TIMER_CHUNK_MS, Math.max(0, dueAt - database.clock.now().getTime())));
}
export function startStripeEventPayloadCleanup(database) {
    if (database.__jobStopped)
        return undefined;
    if (database.__stripeEventPayloadCleanupPromise) {
    ...
    cleanup.finally(() => {
        if (database.__stripeEventPayloadCleanupPromise === cleanup)
            database.__stripeEventPayloadCleanupPromise = null;
    }).catch(() => { });
    return cleanup;
}
...
export function stopStripeEventPayloadCleanup(database) {
    if (database.__stripeEventPayloadCleanupTimer)
        database.clock.clearTimer(database.__stripeEventPayloadCleanupTimer);
    database.__stripeEventPayloadCleanupTimer = null;
    database.__stripeEventPayloadCleanupDueAt = null;
...
export async function ensureScheduleStorage(sqlite, scheduleStorageFault) {
    ...
    const migrateLegacyScheduleIdentity = async (adapter) => {
        ...
        const lineageMigration = await adapter.prepare(migrationSql("SELECT [value] FROM [sporades] WHERE [key]='schedule-legacy-adoption-lineage-v1'")).get();
        ...
        for (const row of schedules) {
            ...
            await adapter.prepare(migrationSql("UPDATE [sporades_schedules] SET [name]=[name] WHERE [name]=?")).run(row.name);
            ...
            if (initializeLegacyLineage) {
                await adapter.prepare(migrationSql("INSERT INTO [sporades_schedule_legacy_adoption] ([scheduleName], [definitionFingerprint], [adoptionOpen]) VALUES (?, ?, ?) ON CONFLICT ([scheduleName]) DO NOTHING")).run(row.name, row.definitionFingerprint, wasLegacySchedule ? 1 : 0);
            }
            const lineage = await adapter.prepare(migrationSql("SELECT [definitionFingerprint], [adoptionOpen] FROM [sporades_schedule_legacy_adoption] WHERE [scheduleName]=?")).get(row.name);
        ...
        if (initializeLegacyLineage) {
            await adapter.prepare(migrationSql("INSERT INTO [sporades] ([key], [value]) VALUES ('schedule-legacy-adoption-lineage-v1', 'complete') ON CONFLICT ([key]) DO NOTHING")).run();
        }
    };
    ...
    await sqlite.exec(sql("CREATE INDEX IF NOT EXISTS [sporades_schedule_legacy_pending_discovery] " +
...
export async function ensureJobStorage(sqlite) {
    ...
    await sqlite.prepare(sql("INSERT INTO [sporades] ([key], [value]) VALUES (?, ?) ON CONFLICT ([key]) DO NOTHING")).run(STRIPE_EVENT_PAYLOAD_SENTINEL_CURSOR_KEY, serializeStripeEventPayloadSentinelMaintenance("", null));
    ...
    for (const [name, type] of [["retryJson", "TEXT"], ["attemptHistory", "TEXT"], ["cancelRequestedAt", "TEXT"], ["leaseExpiresAt", "TEXT"], ["claimToken", "TEXT"], ["scheduleName", "TEXT"], ["scheduledFor", "TEXT"], ["actorProvider", "TEXT"], ["authSnapshotJson", "TEXT"], ["credentialJson", "TEXT"], ["payloadRetentionUntil", "TEXT"], ["payloadRedactedAt", "TEXT"]])
        await sqlite.dialect.addMissingColumn(sqlite, "sporades_jobs", name, type);
    await sqlite.exec(sql("CREATE INDEX IF NOT EXISTS [sporades_jobs_stripe_payload_retention] " +
    ...
    const legacyRows = await sqlite.prepare(sqlite.dialect.sql("SELECT [id], [actorUserId], [enqueuedByUserId], [actorProvider] FROM [sporades_jobs] " +
        "WHERE [scheduleName] IS NULL AND [actorUserId] <> ? AND ([authSnapshotJson] IS NULL OR [credentialJson] IS NULL)")).all(privilegedAuthUserId());
    for (const row of legacyRows) {
        let user = null;
        try {
            user = await sqlite.prepare(sqlite.dialect.sql("SELECT [id], [displayName], [email], [picture], [isAuthenticated], [isGuest] FROM [sporades_auth_users] WHERE [id] = ?")).get(row.actorUserId);
        }
        catch (error) {
            if (!/no such table|does not exist|unknown table/i.test(String(error?.message ?? error)))
                throw error;
        }
        ...
        await sqlite.prepare(sqlite.dialect.sql("UPDATE [sporades_jobs] SET [authSnapshotJson] = COALESCE([authSnapshotJson], ?), [credentialJson] = COALESCE([credentialJson], ?) WHERE [id] = ?")).run(JSON.stringify(authSnapshot), JSON.stringify({ kind: "session" }), row.id);
    }
}
export async function scheduleSummary(sqlite, row) {
    ...
    if (latestOutcome !== null && !isCanonicalJobTimestamp(row.latestScheduledFor))
 ...
 */
export function captureJobAuthSnapshot(auth) {
    if (typeof auth?.displayName !== "string" ||
    ...
    try {
        ...
        if (error?.code !== "INVALID_JOB_IDENTITY")
            throw error;
...
export function canonicalJobCredentialProvenance(credential) {
    if (credential?.kind === "session" && Object.keys(credential).every((key) => key === "kind"))
        return { kind: "session" };
...
export function legacyJobAuthFallback(userId, provider) {
...
export function readJobAuthSnapshot(row) {
    let snapshot;
...
export function readJobCredentialProvenance(row) {
...
function stripeEventPayloadRetentionState(row) {
    if (String(row.handler) !== STRIPE_EVENT_JOB)
        return undefined;
