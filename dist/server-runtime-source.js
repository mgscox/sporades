...
export function createWebSocketHub(getDatabase) {
    ...
    function broadcastJourneyEvent(event) {
        for (const recipient of clients) {
            if (recipient.journeySubscribed)
                sendJson(recipient, { id: null, type: "journey.event", data: event, error: null });
        }
    }
    ...
    async function handleClientMessage(client, rawMessage) {
        ...
        if (message.type === "journey.enable") {
            ...
            if (typeof message.resumeCredential === "string") {
                ...
                if (resumed && resumed.userId === client.session.auth.userId)
                    client.journey = resumed;
            }
            if (!client.journey) {
