...
function resolveFileMaxSizeBytes(config = {}) {
    ...
    if (!Number.isInteger(configured) || configured <= 0) {
        throw commandError("Invalid File size configuration.", "Set `files.maxSizeBytes` to a positive integer byte count.", "INVALID_FILE_CONFIG");
    }
    return configured;
}
    ...
    database = {
        ...
        authorizeReauthentication: typeof capsuleDefinition?.auth?.reauthentication?.authorize === "function" ? async function (transaction, auth, purpose) {
            ...
            const assertActive = () => { if (!active)
                throw commandError("Reauthentication access is no longer active.", "Start a new reauthentication attempt.", "REAUTHENTICATION_ACCESS_INACTIVE"); };
            ...
            return await database.adapter.withTransaction(async (transactionAdapter) => {
                ...
                for (const row of persisted) {
                    if (!declaredNames.has(String(row.name))) {
                        await transactionAdapter.prepare(sql("UPDATE [sporades_schedule_legacy_adoption] SET [definitionFingerprint]=?, [adoptionOpen]=0 WHERE [scheduleName]=?")).run(row.definitionFingerprint, row.name);
                        await transactionAdapter.prepare(sql("INSERT INTO [sporades_schedule_legacy_adoption] ([scheduleName], [definitionFingerprint], [adoptionOpen]) VALUES (?, ?, 0) ON CONFLICT ([scheduleName]) DO NOTHING")).run(row.name, row.definitionFingerprint);
                ...
                for (const { definition, row, nextOccurrence, exhausted, generationToken } of plans) {
                    ...
                    if (row) {
                        ...
                        await transactionAdapter.prepare(sql("INSERT INTO [sporades_schedules] ([name], [definitionFingerprint], [generationToken], [expression], [effectiveTimezone], [missedRunPolicy], [enabled], [exhausted], [nextOccurrence]) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)")).run(definition.name, definition.fingerprint, generationToken, definition.expression, definition.effectiveTimezone, definition.missedRun, definition.enabled ? 1 : 0, exhausted ? 1 : 0, nextOccurrence);
                    }
                    if (!legacyAdoptionOpen) {
                        await transactionAdapter.prepare(sql("UPDATE [sporades_schedule_legacy_adoption] SET [definitionFingerprint]=?, [adoptionOpen]=0 WHERE [scheduleName]=?")).run(definition.fingerprint, definition.name);
                        await transactionAdapter.prepare(sql("INSERT INTO [sporades_schedule_legacy_adoption] ([scheduleName], [definitionFingerprint], [adoptionOpen]) VALUES (?, ?, 0) ON CONFLICT ([scheduleName]) DO NOTHING")).run(definition.name, definition.fingerprint);
                    }
                    if (sameEnabledDefinition) {
                        await transactionAdapter.prepare(sql("UPDATE [sporades_schedule_occurrences] SET [definitionFingerprint]=?, [generationToken]=? WHERE [scheduleName]=? AND [status]='pending' AND [definitionFingerprint]=? AND ([generationToken]=? OR ([generationToken] IS NULL AND ? IS NULL))")).run(definition.fingerprint, generationToken, definition.name, definition.fingerprint, row.generationToken ?? null, row.generationToken ?? null);
                    }
                    if (legacyAdoptionOpen) {
                        await transactionAdapter.prepare(sql("UPDATE [sporades_schedule_occurrences] SET [definitionFingerprint]=?, [generationToken]=? WHERE [scheduleName]=? AND [status]='pending' AND [definitionFingerprint] IS NULL AND [generationToken] IS NULL")).run(definition.fingerprint, generationToken, definition.name);
                    }
...
function preflightStaticScheduleTimers(database) {
    for (const definition of database.schedules) {
        if (!definition.enabled)
            continue;
...
function startStaticSchedules(database, timerPlans) {
    database.__scheduleTimers ??= new Set();
    database.__activeScheduleOccurrences ??= new Set();
    for (const { definition } of timerPlans) {
        const arm = () => {
            if (database.__scheduleStopped || !definition.enabled || definition.exhausted || definition.nextOccurrence == null)
                return;
    ...
    try {
        ...
        const committed = await database.adapter.withTransaction(async (transactionAdapter) => {
            ...
            try {
                ...
                const successor = nextScheduleCursor(definition, occurrence);
                const summary = await transactionAdapter.prepare(sql("UPDATE [sporades_schedules] SET [nextOccurrence]=?, [exhausted]=?, [latestScheduledFor]=?, [latestOutcome]=?, [latestJobId]=?, [latestErrorCode]=? WHERE [name]=? AND [enabled]=1 AND [definitionFingerprint]=? AND [generationToken]=?")).run(successor.nextOccurrence, successor.exhausted ? 1 : 0, scheduledFor, outcome, state?.id ?? null, errorCode, definition.name, definition.fingerprint, definition.generationToken);
...
async function recoverLateLegacyScheduleOccurrences(database) {
    const sql = database.adapter.dialect.sql;
    const rows = await database.adapter.prepare(sql("SELECT [id], [scheduleName], [definitionFingerprint], [generationToken], [scheduledFor], [claimToken], [claimExpiresAt] " +
        "FROM [sporades_schedule_occurrences] WHERE [status]='pending' AND [definitionFingerprint] IS NULL AND [generationToken] IS NULL " +
        "ORDER BY [scheduledFor] ASC, [scheduleName] ASC LIMIT ?")).all(LEGACY_SCHEDULE_DISCOVERY_LIMIT);
    for (const row of rows) {
        if (!validRetainedScheduleOccurrenceIdentity(database, row)
            || (row.claimExpiresAt !== null && !isCanonicalJobTimestamp(row.claimExpiresAt))) {
            await finishInvalidRetainedScheduleOccurrence(database, row);
...
function scheduleLateLegacyOccurrenceDiscovery(database) {
    if (database.__scheduleStopped || database.__scheduleLegacyDiscoveryTimer)
        return;
    if (!database.schedules.some((definition) => definition.enabled && definition.__adoptLegacyPendingOccurrences === true))
        return;
    const timer = database.clock.setTimer(() => {
        database.__scheduleTimers?.delete(timer);
        database.__scheduleLegacyDiscoveryTimer = null;
        if (database.__scheduleStopped)
            return;
        const active = recoverLateLegacyScheduleOccurrences(database).catch((error) => {
...
async function finishInvalidRetainedScheduleOccurrence(database, row, adapter) {
    if (adapter)
        return finishRetainedScheduleOccurrence(database, row, "SCHEDULE_OCCURRENCE_INVALID", adapter);
    const result = await database.adapter.withTransaction((transactionAdapter) => finishRetainedScheduleOccurrence(database, row, "SCHEDULE_OCCURRENCE_INVALID", transactionAdapter));
    armRetainedScheduleRecoveryAfterCommit(database, result.recoveryAt);
    return result;
}
async function finishSupersededRetainedScheduleOccurrence(database, row, adapter) {
    if (adapter)
        return finishRetainedScheduleOccurrence(database, row, "SCHEDULE_OCCURRENCE_SUPERSEDED", adapter);
    const result = await database.adapter.withTransaction((transactionAdapter) => finishRetainedScheduleOccurrence(database, row, "SCHEDULE_OCCURRENCE_SUPERSEDED", transactionAdapter));
    armRetainedScheduleRecoveryAfterCommit(database, result.recoveryAt);
    return result;
}
function earliestScheduleRecoveryAt(current, candidate) {
    if (candidate === null)
        return current;
    if (current === null)
        return candidate;
...
function armRetainedScheduleRecoveryAfterCommit(database, recoveryAt) {
    if (recoveryAt !== null)
        schedulePendingOccurrenceRecovery(database, recoveryAt);
}
async function finishRetainedScheduleOccurrence(database, row, errorCode, adapter) {
    const sql = adapter.dialect.sql;
    if (typeof row.scheduleName === "string") {
        await adapter.prepare(sql("UPDATE [sporades_schedules] SET [name]=[name] WHERE [name]=?")).run(row.scheduleName);
    }
    const completedAt = database.clock.now().toISOString();
    ...
    if (Number(result.changes) === 1)
        return { finished: true, recoveryAt: null };
...
function scheduleJobLeaseRecoveryAt(database, dueAt) {
    ...
    if (dueAt !== null && database.__jobLeaseRecoveryTimer && database.__jobLeaseRecoveryDueAt !== null && database.__jobLeaseRecoveryDueAt <= dueAt)
        return;
...
async function runJobLeaseRecoveryChain(database) {
    ...
    while (!database.__jobStopped) {
        ...
        database.__jobLeaseRecoveryRequestedAt = null;
        if (requestedAt !== null && requestedAt <= database.clock.now().getTime())
            continue;
        const dueAt = requestedAt === null
            ? nextRecoveryAt
            : nextRecoveryAt === null ? requestedAt : Math.min(nextRecoveryAt, requestedAt);
        scheduleJobLeaseRecoveryTimer(database, dueAt);
        if (wakeWorker && !database.__jobStopped)
            scheduleCurrentUserJobWorker(database);
        return;
    }
}
function scheduleJobLeaseRecoveryTimer(database, dueAt) {
    ...
    database.__jobLeaseRecoveryTimer = database.clock.setTimer(async () => {
        ...
        if (!database.__jobStopped)
            await startJobLeaseRecovery(database);
    }, Math.min(MAX_NATIVE_TIMER_DELAY_MS, Math.max(0, dueAt - database.clock.now().getTime())));
}
...
function invalidStoredJobFailure(row, referenceInstant) {
    ...
    if (firstAttempt === null)
        return invalidJobRetryPolicyFailure();
    if (!jobRetryHorizonFits(new Date(firstAttempt), retry, remainingAttempts, Boolean(row.scheduleName && row.scheduledFor && retry.maxAttempts === 1)))
        return invalidJobRetryPolicyFailure();
    return null;
}
function jobRetryHorizonFits(firstAttempt, retry, attemptCount, allowShortFinalScheduleLease = false) {
    ...
    return bindJobResources(database, context, claim, {
        ...
        async authorize(scoped, identity) {
            ...
            const stored = database.adapter.engine === "postgres"
                ? await scopedDatabase.adapter.prepare(`SELECT * FROM ${scopedDatabase.adapter.dialect.quoteIdentifier(table.name)} WHERE ${scopedDatabase.adapter.dialect.quoteIdentifier("id")} = ? FOR UPDATE NOWAIT`).get(identity.id)
...
function trackObservedMutationContextWork(context, start) {
    ...
    return Object.freeze({
        then(onFulfilled, onRejected) { return observed().then(onFulfilled, onRejected); },
        catch(onRejected) { return observed().catch(onRejected); },
        finally(onFinally) { return observed().finally(onFinally); },
...
export async function withTrustedRead(database, options, callback) {
    if (!isActiveTransactionScopedAdapter(options?.transaction, database?.adapter)) {
        throw commandError("Trusted app-database reads require an active transaction.", "Start the trusted policy from the runtime-owned transition transaction.", "TRUSTED_READ_TRANSACTION_REQUIRED");
    }
    if (!trustedReadPurposes.has(options?.purpose)) {
        throw commandError("Trusted app-database read purpose is invalid.", "Use a runtime-owned trusted policy purpose.", "INVALID_TRUSTED_READ_PURPOSE");
    }
    const signal = options?.signal;
    const abortError = () => commandError("Trusted app-database read was aborted.", "Retry the runtime-owned trusted policy if cancellation was not intended.", "TRUSTED_READ_ABORTED");
    if (signal?.aborted)
        throw abortError();
    ...
    const assertActive = () => {
        if (!active) {
            throw commandError("Trusted app-database read access is no longer active.", "Start a new runtime-owned trusted policy callback before reading app data.", "TRUSTED_READ_ACCESS_INACTIVE");
        }
        if (signal?.aborted)
            throw abortError();
    };
...
export function createWebSocketHub(getDatabase, trustedRefresh = null, options = {}) {
                ...
                if (reserved) {
                    ...
                    try {
                        await database.adapter.withTransaction(async (tx) => {
                            ...
                            if (!current || current.token !== client.session.token || current.userId !== client.session.auth.userId || !current.isAuthenticated || current.isGuest || Date.parse(current.expiresAt) <= now.getTime() || credential?.userId !== current.userId || !verifyEmailPassword(normalized.password, credential.passwordSalt, credential.passwordHash))
                                return;
                            ...
                            if (!await tx.claimEmailCredentialVersion(normalized.email, credential.passwordHash, credential.passwordSalt))
                                return;
                            if (!await database.authorizeReauthentication(tx, currentAuth, purpose))
                                return;
                            await tx.replaceReauthenticationProof({ id: randomUUID(), userId: current.userId, sessionToken: current.token, purpose, createdAt: now.toISOString(), expiresAt });
                            ok = true;
                        ...
                        try {
                            await database.log?.emit?.({ category: "platform", event: "auth.reauthentication.authorization_failed", level: "error", message: "Reauthentication authorization policy failed.", data: { provider: "email", purpose } });
                        }
                        catch { }
                        ok = false;
                    }
    ...
    async function signOutSession(database, client) {
        try {
            await database.adapter.withTransaction((tx) => tx.deleteAuthSession(client.session.token));
...
function createMutationContext(database, auth, options = {}) {
    ...
    context.teamBilling = createCurrentUserTeamBillingErasureApi(database, auth, () => holder.current, (candidate) => database.__transactionActive
        ...
        : holder.current === candidate, (operation) => trackObservedMutationContextWork(context, operation));
    context.accessKeys = createCurrentUserAccessKeysApi(database, () => holder.current);
        ...
        while (true) {
            ...
            try {
                ...
                const settled = row.handler === STRIPE_EVENT_JOB
                    ? await database.adapter.prepare(sql("UPDATE [sporades_jobs] SET [status] = 'succeeded', [result] = ?, [completedAt] = ?, [leaseExpiresAt] = NULL, [claimToken] = NULL, [attemptHistory] = ?, [payloadRetentionUntil] = ? " +
