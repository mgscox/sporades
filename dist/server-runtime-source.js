            ...
            return await database.adapter.withTransaction(async (transactionAdapter) => {
                ...
                for (const { definition, row, nextOccurrence, generationToken } of plans) {
                    ...
                    if (row && Boolean(row.enabled) && definition.enabled && row.definitionFingerprint === definition.fingerprint) {
                        await transactionAdapter.prepare(sql("UPDATE [sporades_schedule_occurrences] SET [generationToken]=? WHERE [scheduleName]=? AND [status]='pending' AND [definitionFingerprint]=? AND ([generationToken]=? OR ([generationToken] IS NULL AND ? IS NULL))")).run(generationToken, definition.name, definition.fingerprint, row.generationToken ?? null, row.generationToken ?? null);
                    }
                    if (row) {
