    ...
    const boundedRendererRoots = [...projectRoots, ...rendererDependencyRoots];
    {
        ...
        for (const request of outcome.packageImports ?? []) {
            if (request.specifier.startsWith("#"))
                ...
                recordRendererPackageManifests(request.specifier, path.dirname(request.filename), onDependency);
        }
        ...
        setup(pluginBuild) {
            ...
            pluginBuild.onResolve({ filter: /.*/ }, async (args) => {
                ...
                if (args.path.startsWith("#"))
                    ...
                    recordRendererPackageManifests(args.path, args.resolveDir || projectRoot, onDependency);
...
function recordRendererPackageManifests(specifier, directory, onDependency) {
    ...
    for (const base of localRequire.resolve.paths(specifier) ?? [])
        onDependency(path.join(base, packageName, "package.json"));
}
