...
const writeTablePattern = new RegExp(String.raw `^\s*(?:INSERT(?:\s+OR\s+\w+)?\s+INTO|REPLACE\s+INTO|UPDATE(?:\s+OR\s+\w+)?|DELETE\s+FROM)\s+${quotedIdentifier}`, "i");
...
export function trackLiveQueryReads(tables, run) {
    return liveQueryReads.run(tables, run);
}
export function recordLiveQueryStatementRead(sql) {
...
export function recordLiveQueryTableRead(table) {
 ...
 */
export function liveQueryNeedsRefresh(readTables, dirty) {
    if (!readTables || dirty.has(LIVE_QUERY_ANY_TABLE))
        return true;
...
//# sourceMappingURL=live-query-invalidation.js.map