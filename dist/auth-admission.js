...
export function invalidAuthRequirements(hint) {
    return commandError("Invalid Auth requirements.", hint, "INVALID_AUTH_REQUIREMENTS");
}
export function normalizeRequireUserAuthOptions(options = {}) {
...
export function decorateRequireAuth(options, handler) {
...
export function readAuthRequirements(handler) {
    return typeof handler === "function" ? handler[AUTH_REQUIREMENTS] ?? null : null;
}
export function normalizeAuthRequirements(options = {}) {
    ...
    if (reauthentication !== undefined && (typeof reauthentication !== "string" || !/^[a-z][a-z0-9-]{0,63}$/.test(reauthentication)))
        throw invalidAuthRequirements("reauthentication must be a declared concrete purpose.");
...
export function normalizeCapsuleAuthDefinition(definition) {
    ...
    if (("accessKeys" in definition) && definition.accessKeys !== undefined) {
        ...
        if (!isPlainObject(accessKeys) || Object.keys(accessKeys).some((key) => key !== "scopes") || !("scopes" in accessKeys)) {
            throw commandError("Invalid Capsule Access-key declaration.", "Declare accessKeys as { scopes: readonly string[] } with no additional fields.", "INVALID_ACCESS_KEY_DECLARATION");
        }
    ...
    for (const item of Object.values(normalized.mutations ?? {})) {
        ...
        if (purpose && !declaredPurposes.has(purpose))
            throw invalidAuthRequirements("Every reauthentication purpose must be declared by the Capsule.");
    }
    return normalizeFileAccessKeyPolicy(normalized);
}
function normalizeFileAccessKeyPolicy(definition) {
    if (definition.files?.accessKeys === undefined)
        return definition;
    const policy = definition.files.accessKeys;
    if (!isPlainObject(policy) || Object.keys(policy).some((key) => key !== "read") || !("read" in policy)) {
        throw commandError("Invalid private File Access-key policy.", "Declare files.accessKeys as { read: { scopes?: readonly string[] } }.", "INVALID_FILE_ACCESS_KEY_POLICY");
    }
    const read = policy.read;
    if (!isPlainObject(read) || Object.keys(read).some((key) => key !== "scopes")) {
        throw commandError("Invalid private File Access-key read policy.", "Declare files.accessKeys.read as { scopes?: readonly string[] }.", "INVALID_FILE_ACCESS_KEY_POLICY");
    }
    ...
    if (scopes.some((scope) => !declaredScopes.has(scope))) {
        throw commandError("Invalid private File Access-key read policy.", "Every File read scope must be declared in capsule({ accessKeys: { scopes } }).", "INVALID_FILE_ACCESS_KEY_POLICY");
    }
    return {
        ...definition,
        files: {
...
export function validateCapsuleAuthRequirements(definition) {
    const declaredScopes = new Set(definition.accessKeys?.scopes ?? []);
...
export function scopeGrantMatches(grant, requiredScope) {
    const parts = grant.split("*");
    if (parts.length === 1)
        return grant === requiredScope;
    let offset = grant.startsWith("*") ? 0 : parts[0].length;
    if (!grant.startsWith("*") && !requiredScope.startsWith(parts[0]))
        return false;
    const suffix = grant.endsWith("*") ? "" : parts.at(-1) ?? "";
    const limit = requiredScope.length - suffix.length;
    if (limit < offset || (suffix && !requiredScope.endsWith(suffix)))
        return false;
    ...
    for (const part of parts.slice(firstInterior, lastInterior)) {
        ...
        if (foundAt === -1 || foundAt + part.length > limit)
            return false;
        offset = foundAt + part.length;
    }
    return true;
}
export function accessKeyGrantsSatisfyScopes(grants, requiredScopes) {
...
//# sourceMappingURL=auth-admission.js.map