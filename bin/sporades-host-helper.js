...
import { access, chmod, chown, link, lstat, mkdir, readdir, readFile as readFile2, readlink, rename, rm, statfs, symlink, writeFile } from "node:fs/promises";
...
function routeLockProcessIdentity(pid) {
  const result = spawnSync2("ps", ["-o", "lstart=", "-p", String(pid)], { encoding: "utf8" });
  if (result.status !== 0) {
    return null;
  }
...
async function readManagedRouteLockOwner(lockDir) {
  let parsed;
  try {
    parsed = JSON.parse(await readFile2(lockDir, "utf8"));
  } catch (error) {
    if (errorDetails(error).code === "ENOENT") {
...
async function acquireManagedRouteReclaimGuard(reclaimGuard, claimantOwner, claimFile) {
  ...
  try {
    ...
    return errorDetails(error).code === "ENOENT" ? acquireManagedRouteReclaimGuard(reclaimGuard, claimantOwner, claimFile) : false;
  }
