...
var DEFAULT_HOST_LOG_LINES_VALUE = 200;
...
var HOSTED_CAPSULE_DOCKER_IMAGE = DEFAULT_HOSTED_CAPSULE_DOCKER_IMAGE;
var HOSTED_CAPSULE_DOCKER_NETWORK = DEFAULT_HOSTED_CAPSULE_DOCKER_NETWORK;
var HOSTED_CAPSULE_GRACE_CHECK_MS = DEFAULT_HOSTED_CAPSULE_GRACE_CHECK_MS;
var DEFAULT_HOST_LOG_LINES = DEFAULT_HOST_LOG_LINES_VALUE;
var MAX_HOST_LOG_LINES = DEFAULT_MAX_HOST_LOG_LINES;
main().catch((error) => {
  writeEnvelope(
    {
      ok: false,
...
async function provisionCaddyAccessLog(request, bootstrap) {
  ...
  const chown2 = spawnSync("chown", [`${caddyUser.uid}:${caddyUser.gid}`, logDirectory, logFile], { encoding: "utf8" });
...
function validateListRegistryRecord(request, record, recordPath) {
  ...
  const valid = record && typeof record.subname === "string" && record.subname.length > 0 && record.subname === expectedSubname && record.domain === request.host.domain && (record.remoteCapsuleId ?? expectedRemoteCapsuleId) === expectedRemoteCapsuleId;
  if (!valid) {
    throw helperError(
      "Hosted Capsule registry record is invalid.",
      `Repair the Host server registry record at ${recordPath}, then retry \`${hostRegistryRetryCommand(request)}\`.`
