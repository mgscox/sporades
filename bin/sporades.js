...
function parseHostArgs(args) {
  ...
  for (let index = 0; index < rest.length; index += 1) {
    ...
    if (arg.startsWith("--")) {
      throw commandError(
        `Unknown flag: ${arg}`,
        "Use `sporades host add`, `sporades host use`, `sporades host current`, `sporades host bind`, `sporades host register`, `sporades host push`, `sporades host bootstrap`, `sporades host logs`, or `sporades host invoke`.",
      );
    }
    positional.push(arg);
  }
