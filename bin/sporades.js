...
export default function App() {
  ...
  return <main class="campfire"><aside><img class="mark" src={mark} alt="" /><h1>\u{1F525} Campfire</h1><nav><For each={channels}>{(slug) => <button classList={{ active: channel() === slug }} onClick={() => choose(slug)}># {slug}</button>}</For></nav><p>Development-only Musketeer identities. Never expose known credentials publicly.</p></aside>
    <section class="conversation"><header><h2># {channel()}</h2><p role="status">{notice()}</p></header><div class="messages"><For each={messages().data ?? []}>{(message) => <article><strong>{message.authorName}</strong><time>{new Date(message.createdAt).toLocaleString()}</time><p>{message.body}</p><button disabled={toggleReaction.state().loading} aria-label={"Thumbs up: " + reactionCount(message, "up")} onClick={() => react(message.id, "up")}>\u{1F44D} {reactionCount(message, "up")}</button><button disabled={toggleReaction.state().loading} aria-label={"Thumbs down: " + reactionCount(message, "down")} onClick={() => react(message.id, "down")}>\u{1F44E} {reactionCount(message, "down")}</button></article>}</For></div><form onSubmit={submit}><input id="message" value={draft()} onInput={(event) => { setDraft(event.currentTarget.value); compose(); }} placeholder={"Message #" + channel()} /><button disabled={sendMessage.state().loading || !draft().trim()}>Send</button></form></section>
...
<main class="campfire"><aside><img class="mark" src={mark} alt=""/><h1>\u{1F525} Campfire</h1><nav>{#each channels as slug}<button class:active={channel===slug} onclick={()=>choose(slug)}># {slug}</button>{/each}</nav><p>Development-only Musketeer identities. Never expose known credentials publicly.</p></aside>
<section class="conversation"><header><h2># {channel}</h2><p role="status">{notice}</p></header><div class="messages">{#each messages.data??[] as message(message.id)}<article><strong>{message.authorName}</strong><time>{new Date(message.createdAt).toLocaleString()}</time><p>{message.body}</p><button disabled={$toggleReaction.loading} aria-label={"Thumbs up: "+reactionCount(message,"up")} onclick={()=>react(message.id,"up")}>\u{1F44D} {reactionCount(message,"up")}</button><button disabled={$toggleReaction.loading} aria-label={"Thumbs down: "+reactionCount(message,"down")} onclick={()=>react(message.id,"down")}>\u{1F44E} {reactionCount(message,"down")}</button></article>{/each}</div><form onsubmit={(event)=>{event.preventDefault();submit();}}><input id="message" bind:value={draft} oninput={compose} placeholder={"Message #"+channel}/><button disabled={$sendMessage.loading||!draft.trim()}>Send</button></form></section>
