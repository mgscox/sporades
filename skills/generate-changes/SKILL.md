...
The repository `npm run package` command runs this skill's script before bumping the package version, committing release metadata, and publishing, so `CHANGES.md` reflects source changes since the previous release rather than the release commit itself. Before publishing, the package script fetches the configured upstream and refuses an untracked, ahead, behind, or diverged release branch. It atomically reserves the release commit and annotated `vX.Y.Z` tag on the remote before the irreversible npm publication. If Git synchronization or publication is interrupted, `npm run package -- --resume` repacks the exact release commit, verifies npm shasum and integrity when already published, and safely completes whichever side is absent. Recovery also accepts detached HEAD, but requires `--branch=<name>` and optionally `--remote=<name>` rather than guessing the original release branch from the remote default. The next `/generate-changes` run uses that tag as its release baseline.
