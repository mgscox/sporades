...
An opt-in, server-only `ctx.resources.run/status` boundary for ordinary SQLite Jobs, Custom mutations, and Custom endpoints. Jobs own a dedicated engine writer transaction; mutations and endpoints join their existing outer transaction. Each binds current anchor ACL/Team authorization, application writes, child enqueues, and an indefinitely retained canonical replay receipt. Non-Job scopes use the outer transaction's 30-second budget with a one-second admission reserve; results remain provisional until that outer transaction commits. No resource lease or SMTP fence is introduced, and durable notification acceptance remains fail-closed until ticket 06. See [the reference](docs/reference/jobs-and-schedules.md#sqlite-resource-transactions-ticket-03).
