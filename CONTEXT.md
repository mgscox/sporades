...
An opt-in, server-only `ctx.resources.run/status` boundary for ordinary SQLite and PostgreSQL Jobs, Custom mutations, and Custom endpoints. PostgreSQL verifies the exact resource lock and receipt schema first; if initialization is needed it publishes both tables or a folded-legacy upgrade in a short separate transaction whose advisory guard remains held through commit. Normal scopes then lock one runtime resource row with `FOR UPDATE NOWAIT`, and Jobs lock the exact Job claim before protected reads and commit. Bootstrap therefore never becomes scope-wide admission or releases a guard before its DDL is visible. Each binds current anchor ACL/Team authorization, application writes, child enqueues, and an indefinitely retained canonical replay receipt. A lost PostgreSQL COMMIT acknowledgement closes its dedicated connection and callers reconcile after fresh acquisition through the receipt. No resource lease or SMTP fence is introduced, and durable notification acceptance remains fail-closed until ticket 06. See [the reference](docs/reference/jobs-and-schedules.md#sqlite-resource-transactions-ticket-02).
