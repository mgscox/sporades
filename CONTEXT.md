...
An opt-in, server-only `ctx.resources.run/status` boundary for ordinary SQLite and PostgreSQL Jobs, Custom mutations, and Custom endpoints. PostgreSQL Jobs own a dedicated READ COMMITTED connection that briefly serializes resource-schema bootstrap, then locks one runtime resource row with `FOR UPDATE NOWAIT`, then locks the exact Job claim before protected reads and commit. The bootstrap lock is released before resource-row authority and callbacks, so independent resources do not contend on it. Each binds current anchor ACL/Team authorization, application writes, child enqueues, and an indefinitely retained canonical replay receipt. A lost PostgreSQL COMMIT acknowledgement closes its dedicated connection and callers reconcile after fresh acquisition through the receipt. No resource lease or SMTP fence is introduced, and durable notification acceptance remains fail-closed until ticket 06. See [the reference](docs/reference/jobs-and-schedules.md#sqlite-resource-transactions-ticket-02).
