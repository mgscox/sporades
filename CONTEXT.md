...
The optional top-level `teamBilling` Capsule declaration. It is dormant when omitted. When present, it binds stable product keys to distinct exact sandbox/live Stripe Price identities and either a fixed quantity or the Team's exact accepted-member count. Every operation first requires a current linked administrator of the explicit Team, then invokes the Capsule's `authorize` policy with transaction-bound read-only app data; the policy capability expires when the callback settles and no reusable billing capability is issued. Sporades owns private Customer, Subscription, operation, observation, replay, durable Checkout Job, retry, idempotency, and verified completion correlation. The client exposes `teamBilling.get(teamId)` for closed provider-free product-key state and `teamBilling.startCheckout(...)` for one authorized operation; only a validated, expiring Checkout URL capability may cross that command. Browser return and provider acknowledgement never apply entitlement. The Capsule owns all rendered UI, product copy, entitlement mapping, and local business policy.
