...
**Headless Team Billing**:
The optional top-level `teamBilling` Capsule declaration. It is dormant when omitted. When present, it binds stable product keys to distinct exact sandbox/live Stripe Product and Price identities, explicit reviewed Customer Portal configurations, and either a fixed quantity or the Team's accepted-member count with an optional positive integer billable-seat floor. Omitting the floor preserves the existing one-seat minimum. Every operation first requires a current linked administrator of the explicit Team, then invokes the Capsule's `authorize` policy with transaction-bound read-only app data; the policy capability expires when the callback settles and no reusable billing capability is issued. Sporades owns private Customer, Subscription, operation, observation, replay, durable Checkout, Portal, managed Plan-transition, and Team-seat convergence Jobs, retry, idempotency, configuration attestation, and verified billing-state convergence. Compatible switches within one quantity policy remain in the reviewed Customer Portal. `teamBilling.requestPlanTransition(...)` handles fixed-to-Team-counted, Team-counted-to-fixed, or differently floored Team-counted switches by updating the exact Subscription item Price and quantity together. Accepted-member changes commit independently, then stage latest-wins seat intent; a durable per-Team provider lane prevents overlapping provider writes across runtimes, and restart repair reconstructs missing work. Supported Checkout, Subscription, and failed-invoice observations enter the same per-Capsule atomic Stripe fence, validate exact association and catalogue facts, and advance through semantic ratchets and terminal latches; malformed, stale, unknown-catalogue, below-floor, or ambiguous evidence cannot grant entitlement and is reduced to provider-free attention state. The client exposes only closed provider-free product and operation state plus validated, expiring Stripe-hosted URL capabilities. Browser return, Checkout completion alone, and provider acknowledgement never apply entitlement. The Capsule owns all rendered UI, product copy, entitlement mapping, downgrade policy, and other local business rules.
_Avoid_: billing page, Stripe state hook, Customer ID API, app-owned provider bridge

**Provider-safe Team erasure**:
The durable headless Team Billing operation started by `teamBilling.prepareErasure`. Sporades fences provider work, resolves issued Checkout Sessions exactly, immediately cancels every known or newly discovered live Subscription, and persists only provider-free keyed identity tombstones before `ctx.teamBilling.admitLocalErasure(teamId)` may authorize the Capsule's separate local deletion transaction. It never deletes the Stripe Customer, renders confirmation UI, or exposes provider evidence.
_Avoid_: Stripe Customer deletion, cascade delete, provider cleanup callback, deletion webhook

**Verified Stripe event**:
A bounded server-only value produced only after the official Stripe verifier accepts the exact callback bytes and `Stripe-Signature` header within its five-minute tolerance. It preserves the stable provider Event identity, type, creation time, live/test mode, relevant object identity, and frozen verified provider value. Admission stores it only as one idempotent runtime-owned Privileged Job payload within the retained Capsule database; changing the configured Capsule name does not change that Job identity. Successful delivery with a canonical completion time starts a fixed 30-day payload deadline when that exact deadline fits the canonical four-digit timestamp range, after which the runtime leaves only a non-sensitive terminal replay tombstone. Unresolved Jobs, including legacy successes with absent or malformed completion times and canonical near-ceiling settlements whose deadline is unrepresentable, retain the value and expose only safe retention state through operator inspection. It grants no user, Session, Team, Capsule role, or browser authority and does not itself change Capsule billing state.
...
The optional single Capsule policy handler declared with `stripeEvents: stripeEvent(handler)`. The runtime-owned Stripe Event Job invokes it with a Verified Stripe event under the userless Privileged server role and its existing audit, retry, cancellation, and revocation lifecycle. The handler owns idempotent, order-independent Capsule consequences; unknown event types may be ignored and no app-table subscription, entitlement, access, invoice, Customer, Team, order, export, erasure, or retention state is created automatically. A declared headless Team Billing control plane independently converges its private runtime-owned projection before a legacy handler runs.
_Avoid_: Stripe webhook handler, callback endpoint, billing-state synchronizer

**Atomic Stripe consequence**:
The opt-in `stripeEvent({ consequence: "atomic" }, handler)` policy form. One Job attempt enters a runtime-owned, per-Capsule serialization fence before any app read, then runs the handler's database writes and Job enqueues in one transaction through a narrow userless context. That context may read only the exact accepted-membership count for an explicitly identified Team through `teams.countMembers(teamId)`; it receives no Team identities, directory, administration, or mutation authority. Return commits and dispatches enqueued Jobs afterward; throw, cleanup failure, or pre-commit cancellation rolls back and revokes every captured capability. The legacy Stripe-event subscription remains non-transactional for long-lived cooperative handlers.
...
**Access key**:
A runtime-owned, user-owned, named credential for scoped programmatic access. Its attribution ID, single User owner, human-readable name, grant expressions, and optional expiry are fixed at issuance; moving automation, renaming it, or changing its authority requires a new key. The name is unique among its owner's current Access keys. Active and expired keys remain current and reserve their names; irrevocably revoked keys are historical and release their names for reuse. A linked, non-guest human User may own one through the current-user lifecycle, while a Service User owns keys issued through the separate `ctx.serviceUsers` lifecycle. It authenticates that exact owner with no greater authority than the User, the key's granted scopes, and Capsule policy allow; it does not create a Team member or Capsule role.
_Avoid_: passwordless login, Session token, unscoped API key, Capsule-owned secret

**Service User**:
A first-class non-human Sporades User for named automation. It has `ctx.auth.userKind === "service"`, an active-or-disabled lifecycle, and one or more runtime-owned Access keys, but no email, password, OAuth Provider identity, browser Session, or implicit human owner. A current human Session may atomically create one with its initial key, issue or rotate a named key, revoke a key, list safe key metadata, or irreversibly disable the User through `ctx.serviceUsers`. Disabling retires every current key; safe User and key metadata remain for historical attribution. The Capsule owns application roles, Team membership, and resource policy, so a Service User and its scopes never create application authority by themselves.
_Avoid_: shared human account, synthetic Session, provider identity, Capsule-specific credential table
...
The runtime-owned lifecycle interfaces through which a current linked human User manages only their own keys with `ctx.accessKeys`, or manages Service Users and their keys with `ctx.serviceUsers`, using an interactive current Session. An Access key cannot manage Access keys or Service Users. Capsules render their own management experience over the interfaces rather than receiving a framework-owned page. Service-user calls recheck the Session inside the current transaction; plaintext is returned only by a committed issue or rotation. Inside an explicit audited `ctx.privileged.run(...)`, the separate Privileged projection may list one exact owner's metadata, inspect or revoke an exact key, revoke one exact owner's current keys, and delete revoked history; it cannot issue, rotate, or receive bearer tokens, and cannot create or disable Service Users. The `sporades access-keys` operator commands reach that projection only through a running Dev, Container, or Hosted Capsule using immutable owner/key IDs and the shared generated-Bundle action seam. Read operations require no confirmation; destructive operations require an interactive confirmation or `--yes`, with exact owner-ID confirmation for bulk revocation. `--json` never implies consent, and stopped-Capsule database access is not supported.
...
An opt-in, server-only `ctx.resources.run/status` boundary for ordinary SQLite and PostgreSQL Jobs, Custom mutations, and Custom endpoints. PostgreSQL verifies the exact ordered resource lock and receipt columns, text types, nullability, absence of extras, primary keys, and absence of every index except the primary-key backing index first; if initialization is needed it publishes both tables or a folded-legacy upgrade in a short separate transaction whose advisory guard remains held through commit. Normal scopes then lock one runtime resource row with `FOR UPDATE NOWAIT`; Jobs lock the exact Job claim, and every scope locks its authorization anchor before the current ACL decision and protected reads. Bootstrap therefore never becomes scope-wide admission or releases a guard before its DDL is visible. Each binds current anchor ACL/Team authorization, application writes, child enqueues, durable notification intents, and an indefinitely retained canonical replay receipt through commit. A lost PostgreSQL COMMIT acknowledgement closes its dedicated connection and callers reconcile after fresh acquisition through the receipt. No resource lease or SMTP fence is introduced. A separate worker reserves and submits one SMTP envelope per recipient after commit; uncertain retries can duplicate receiver acceptance and this is not an exactly-once guarantee. See [the reference](docs/reference/jobs-and-schedules.md#sqlite-resource-transactions-ticket-02).

Resource adapter support is explicit: SQLite is supported, PostgreSQL is supported, and libSQL is unsupported. libSQL `run` and `status` reject with `RESOURCE_ADAPTER_UNSUPPORTED` before callback or receipt work.
