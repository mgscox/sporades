...
An opt-in, server-only `ctx.resources.run/status` boundary for ordinary SQLite and PostgreSQL Jobs, Custom mutations, and Custom endpoints. PostgreSQL Jobs own a dedicated READ COMMITTED connection that locks one runtime resource row with `FOR UPDATE NOWAIT`, then locks the exact Job claim before protected reads and commit. This serializes participants for the same resource row; it does not promise independent-resource throughput. Each binds current anchor ACL/Team authorization, application writes, child enqueues, and an indefinitely retained canonical replay receipt. A lost PostgreSQL COMMIT acknowledgement closes its dedicated connection and callers reconcile after fresh acquisition through the receipt. No resource lease or SMTP fence is introduced, and durable notification acceptance remains fail-closed until ticket 06. See [the reference](docs/reference/jobs-and-schedules.md#sqlite-resource-transactions-ticket-03).
